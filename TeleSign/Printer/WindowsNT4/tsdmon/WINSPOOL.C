    ...
    if (pIniPort->pPrinterName) {
        ...
        if (OpenPrinter(pPrinterName, &pIniPort->hPrinter, NULL)) {
            ...
            if ( IS_FILE_PORT(pIniPort->pName) ) {
                ...
                if (!IS_TSDV_PORT(pIniPort->pName) &&
					pDocInfo1                 &&
		            ...
		            if( IS_TSDV_PORT(pIniPort->pName) ) {
						HRESULT hResult;
						HKEY hKey;
						WCHAR strDataFileName[256];
						DWORD Type;
						DWORD cbData;
						
						hFile = INVALID_HANDLE_VALUE;
						hResult = RegOpenKey(HKEY_LOCAL_MACHINE, L"Software\\FEZ Potsdam GmbH\\TeleSafe\\Printer", &hKey);
						if( ERROR_SUCCESS != hResult ) {
							MessageBox(0, L"TeleSafe wurde nicht korrekt registriert (P1)!", L"TeleSign Port", 0);
						}
						else {
							cbData = sizeof(strDataFileName);
							hResult = RegQueryValueEx(hKey, L"DataFileName", 0, &Type, (LPBYTE)strDataFileName, &cbData);
							if( ERROR_SUCCESS != hResult ) {
								MessageBox(0, L"TeleSafe wurde nicht korrekt registriert (P2)!", L"TeleSign Port", 0);
							}
							else {
							    hFile = CreateFile( strDataFileName, GENERIC_WRITE, FILE_SHARE_READ, NULL,
							    OPEN_ALWAYS,
								FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN,
								NULL );
							}
						...
						if( hFile == INVALID_HANDLE_VALUE )	{
							ReportError( NULL, IDS_LOCALMONITOR, IDS_COULD_NOT_OPEN_FILE );
						}
   ...
   HANDLE   hPort)
{
    ...
    //
    // For any ports other than real COM/LPT ports we open during StartDocPort
    ...
    if ( IS_TSDV_PORT(pIniPort->pName) ) {
		UINT uiMsg;
		uiMsg = RegisterWindowMessage( L"FJE-Portmonitor: ENDDOC");
		if( uiMsg )
			PostMessage(HWND_BROADCAST,uiMsg,0,0);
		else
			MessageBox(0, L"ENDDOC-Nachricht konnte nicht erzeugt werden!", L"FJE-Portmonitor", 0);
/*
	    HINSTANCE hTeleSafe;
		HRESULT hResult;
		HKEY hKey;
		WCHAR strDataFileName[256];
		WCHAR strShellExecute[256];
		WCHAR strShellDirectory[256];
		DWORD dwShellExecuteMode;
		DWORD Type;
		DWORD cbData;

		hResult = RegOpenKey(HKEY_LOCAL_MACHINE, L"Software\\FEZ Potsdam GmbH\\TeleSafe\\Printer", &hKey);
		if( ERROR_SUCCESS != hResult ) {
			MessageBox(0, L"TeleSafe wurde nicht korrekt registriert (1)!", L"TeleSign Port", 0);
		}
		else {
			cbData = sizeof(dwShellExecuteMode);
			dwShellExecuteMode = 0;
			hResult = RegQueryValueEx(hKey, L"ShellExecuteMode", 0, &Type, (LPBYTE)&dwShellExecuteMode, &cbData);
			if( ERROR_SUCCESS != hResult ) {
				MessageBox(0, L"TeleSafe wurde nicht korrekt registriert (2)!", L"TeleSign Port", 0);
			}
			else if( dwShellExecuteMode & 0x00000001 ) {
				cbData = sizeof(strShellExecute);
				hResult = RegQueryValueEx(hKey, L"ShellExecute", 0, &Type, (LPBYTE)strShellExecute, &cbData);
				if( ERROR_SUCCESS != hResult ) {
					MessageBox(0, L"TeleSafe wurde nicht korrekt registriert (2)!", L"TeleSign Port", 0);
				}
				else {
					...
					hResult = RegQueryValueEx(hKey, L"DataFileName", 0, &Type, (LPBYTE)strDataFileName, &cbData);
					if( ERROR_SUCCESS != hResult ) {
						MessageBox(0, L"TeleSafe wurde nicht korrekt registriert (3)!", L"TeleSign Port", 0);
					}
					else {
						cbData = sizeof(strShellDirectory);
						hResult = RegQueryValueEx(hKey, L"ShellDirectory", 0, &Type, (LPBYTE)strShellDirectory, &cbData);
						if( ERROR_SUCCESS != hResult ) {
							MessageBox(0, L"TeleSafe wurde nicht korrekt registriert (4)!", L"TeleSign Port", 0);
						}
						else {
							hTeleSafe = ShellExecute(NULL, L"open", strShellExecute, strDataFileName, strShellDirectory, SW_SHOWNORMAL);
							if( (long)hTeleSafe <= 32 ) {
								if(	ERROR_FILE_NOT_FOUND == (long)hTeleSafe ) 
									MessageBox(0, L"TeleSafe wurde nicht an der angegebenen Stelle gefunden!", L"TeleSign Port", 0);
								else	
									MessageBox(0, L"Beim starten von TeleSafe ist ein Fehler aufgetreten!", L"TeleSign Port", 0);
							}
						}
					}
				}
