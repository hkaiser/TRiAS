...
// lokale Helperfunktionen
	...
	HRESULT OpenPropSeqStorage (LPCOLESTR pcoleStrmName, DWORD dwMode, IStorage **ppStorage) 
	{
	WStorage StgRoot;
	WStorage Stg;
	...
	HRESULT hr = OpenStorage (pcoleStrmName, dwMode, StgRoot, Stg.ppi());
...
// Laden einer bestimmten PropertySequence
	HRESULT LoadPropertySequence (
		BSTR bstrName, IPropertyActionSequence **ppIPropertyActionSequence, 
		LPCOLESTR pcoleStrmName)
	{
		TEST_OUT_PARAM(ppIPropertyActionSequence);
		COM_TRY {
		WStorage Stg;

			if (FAILED(OpenPropSeqStorage (pcoleStrmName, STGM_READ, Stg.ppi())))
				return E_FAIL;
...
// Schreiben einer bestimmten PropertySequence
		...
		LPCOLESTR pcoleStrmName)
	{
		...
		WStorage Stg;
			...
			if (FAILED(OpenPropSeqStorage (pcoleStrmName, STGM_READWRITE, Stg.ppi())))
				return E_FAIL;
