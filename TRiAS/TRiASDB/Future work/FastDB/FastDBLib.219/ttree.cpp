		     ...
		     int type, int offs)
{
	...
	oid_t nodeId = rootId;
	dbTtreeNode::insert(db, nodeId, recordId, key, type, offs);
	if (rootId != nodeId) { 
	    ((dbTtree*)db->put(treeId))->root = nodeId;
	}
    }
		     ...
		     int type, int offs)
{
    ...
    oid_t nodeId = rootId;
    int h = dbTtreeNode::remove(db, nodeId, recordId, key, type, offs);
    assert(h >= 0);
    if (nodeId != rootId) { 
	((dbTtree*)db->put(treeId))->root = nodeId;
    }
...
bool dbTtreeNode::find(dbDatabase* db, dbSearchContext& sc)
{
	...
	if (sc.firstKey != NULL) { 
		...
		while (r < n) { 
		    ...
		    if (!sc.condition 
			|| db->evaluate(sc.condition, item[r], table)) 
                    {
	...
	for (l = 0; l < n; l++) { 
	    ...
	    if (!sc.condition || db->evaluate(sc.condition, item[l], table)) {
	...
	if (sc.firstKey != NULL) { 
	    rec = (char*)db->getRow(item[0]);
	    diff = keycmp(sc.firstKey, rec+sc.offs, sc.type);
	    if (diff >= sc.firstKeyInclusion) {	    
		rec = (char*)db->getRow(item[n-1]);
		diff = keycmp(sc.firstKey, rec+sc.offs, sc.type);
		if (diff >= sc.firstKeyInclusion) {
		...
		while (r < n) { 
		    rec = (char*)db->getRow(item[r]);
		    if (sc.lastKey != NULL 
			&& keycmp(rec+sc.offs, sc.lastKey, 
				  sc.type) >= sc.lastKeyInclusion) 
		    { 
			return false;
		    }
		    if (!sc.condition 
	...
	for (l = 0; l < n; l++) { 
	    rec = (char*)db->getRow(item[l]);
	    if (sc.lastKey != NULL && keycmp(rec+sc.offs, sc.lastKey, sc.type) 
		>= sc.lastKeyInclusion) 
	    {
		return false;
	    }
			 ...
			 void* key, int type, int offs)
{
	...
	if (leftId == 0) { 
	    ...
	    bool grow = insert(db, childId, recordId, key, type, offs);
	    if (childId != leftId) { 
	...
	return insert(db, nodeId, reinsertId, key, type, offs);
    }
}
			...
			void* key, int type, int offs)
{
	...
	if (leftId != 0) { 
	    oid_t childId = leftId;
	    int h = remove(db, childId, recordId, key, type, offs);
	    if (childId != leftId) { 
				  ...
				  dbExprNode* condition)
{
    ...
    for (int i = 0, n = nItems; i < n; i++) { 
	if (db->evaluate(condition, item[i], table)) { 
	    if (!cursor->add(item[i])) { 
		return false;
	    }
	}
