/**
 **     $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imgiflzw.c,v 1.11 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER  "    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imgiflzw.c,v 1.11 1995/06/29 00:28:04 bduggan Exp $"
...
#endif
{
	...
	if ( ImBinRead( ioType,fd,fp,&count, IM_UCHAR_TYPE, 1, 1 ) == -1 )
	    ImReturnBinError( );
	if (count == 0)
	    done = TRUE;
	else if ( ImBinRead( ioType,fd,fp,&buf[2],IM_UCHAR_TYPE,1,count ) == -1 ) 
	    ImReturnBinError( );
	last_byte = 2 + count;
...
#endif
{
	...
	imLzwGetCode(ioType,fd,fp,0,TRUE);
	...
	if (code == end_code) 
	{
	    while ( ImBinRead(ioType,fd,fp,&count,IM_UCHAR_TYPE,1,1) != -1  && (count!=0))
		while (count-->0 && ImBinRead( ioType,fd,fp,&junk,IM_UCHAR_TYPE,1,1) != -1);
	    if (count!=0)
...
#endif
{
                ...
                ((cur_bit & 7) != 0))<<3) - cur_bit) != 0) 
	{
        ...
        if (block[0]) 
	   if ( ImBinWrite( ioType, fd, fp, block, IM_UCHAR_TYPE, 1, block[0]+1) == -1)
                        ImReturnBinError();
