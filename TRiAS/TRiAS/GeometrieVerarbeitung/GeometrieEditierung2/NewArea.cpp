// Definitionen: Editierung eines neuen Flächenobjektes
// File: NewArea.cpp
// W. Mörtl
...
HRESULT CNewArea::MouseMove (LPARAM lParam)
{
    ...
    if (m_LeftButtonSem || m_RightButtonSem || m_bIsPaint)
   	...
   	// Hilfslinie, die auf dem Bildschirm die editierte Fläche immer gleich zuzieht, einmalig
	// löschen
...
HRESULT hrRet = m_wEdExSite->ShowEditDialog (resCapt.Addr(), g_cbGeoEditNewArea, ulCnt,
    ...
    // m_wObj würde erst durch den Destruktor ~CNewArea zerlegt werden; damit aber mehrere
    // Flächenobjekte editiert werden können, ohne das zwischendurch ~CNewArea gerufen wird,
...
*/
    ...
    // Hilfslinie beim nächsten MouseMove() löschen (s. a. LeftMouseUp), wenn zusätzlich der
    // nächste editierte Punkt nicht Beginn einer Insel ist
    // da der DeleteButton (mehrmals) gedrückt worden sein konnte, sind nachfolgende Werte
    // zu aktualisieren
    if (m_wObj.IsValid())
    {
    long* plCnt;
...
{
    m_bIsland = true;	// der nächste editierte Punkt ist Beginn einer Insel
