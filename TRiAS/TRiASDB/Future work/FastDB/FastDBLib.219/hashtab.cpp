...
{
    dbHashTable* hash = (dbHashTable*)db->get(hashId);
    unsigned keylen = keySize[sc.type];
    if (sc.type == dbField::tpString) { 
	keylen = strlen(sc.firstKey);
    }
    unsigned hashkey = hashCode((byte*)sc.firstKey, keylen);
    unsigned h = hashkey % hash->size;
    oid_t itemId = 
	...
	if (item->hash == hashkey) { 
		...
		|| (sc.type != dbField::tpString
		    && memcmp(sc.firstKey, rec + sc.offs, keylen) == 0)) 
	    { 
		if (!sc.condition 
		    || db->evaluate(sc.condition, item->record, table))
                {
