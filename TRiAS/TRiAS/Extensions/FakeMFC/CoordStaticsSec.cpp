...
CCompoundStaticSec::CCompoundStaticSec()
{
	...
	TRY {
		VERIFY(m_strDistance.LoadString (IDS_DISTANCE));
...
CString str;
	...
	TRY {
		VERIFY(str.LoadString (IDS_RECHTSWERT));
		m_ftCoordX.SetWindowText (str);
		VERIFY(str.LoadString (IDS_HOCHWERT));
...
double XOut, YOut;
char cbOutBuff[64];		// temporäre Zeichenkette

	if (!m_pCTF -> UnScale (Pkt.GetX(), Pkt.GetY(), &XOut, &YOut))
...
char cbInBuff[64];
	...
	m_ftCoordX.GetWindowText (cbInBuff, sizeof(cbInBuff));
	if (strcmp (cbInBuff, cbOutBuff))
		m_ftCoordX.SetWindowText (cbOutBuff);

	m_pCTF -> CoordTransToStringEx (XOut, YOut, false, cbOutBuff, sizeof(cbOutBuff));
	m_ftCoordY.GetWindowText (cbInBuff, sizeof(cbInBuff));
	if (strcmp (cbInBuff, cbOutBuff))
	...
	UINT nID = pNMHDR->idFrom;
		...
		LPTSTR pBuffer = NULL;
			...
			TRY {
				...
				VERIFY(strDesc.LoadString ((ID_HOCHWERT == nID) ? IDS_HOCHWERT : IDS_RECHTSWERT));
			} CATCH (CMemoryException, e) {
				return false;
			} END_CATCH;
...
{
	if (uiMsg == WM_CORRECTSHOWNPOSITION)	// wParam enthält ursprünglichen MsgID
...
HRESULT CInstCoordStatics::RouteMouseEvents (HWND hWnd, UINT uiMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (m_fDrag && WM_LBUTTONUP == wParam) {
		m_fDrag = false;
		m_pBack -> ResetDrag();
	...
	CPoint pt (LOWORD(lParam), HIWORD(lParam));
		...
		if (pt != m_ptLast) {
		// nur neu berechnen, wenn neue Koordinaten (#HK000214)
			m_ptLast = pt;
			return m_pBack -> SetCoords (pt, m_fDrag);
		}
	}
	return NOERROR;
}
...
CInstCoordStatics::CInstCoordStatics (void)
{
	...
	TRY {
		VERIFY(m_strIdle.LoadString (AFX_IDS_IDLEMESSAGE));
	} CATCH (CMemoryException, e) {
		ASSERT(false);
	} END_CATCH;
}
