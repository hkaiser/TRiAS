...
// $Header: /usr/local/devel/GiST/libGiST/RTree/command.cpp,v 1.1.1.1 1996/08/06 23:47:25 jmh Exp $
...
CommandDrop(const char *table)
{
    ...
    if (unlink(table))
	cout << "There is no table by that name.\n";
    else
	      ...
	      int ptr)
{
    ...
    GiST *gist = tables[i].gist;
    gist->Insert(RTentry(key, (GiSTpage)ptr));
