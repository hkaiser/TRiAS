/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imhdfwrite.c,v 1.10 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imhdfwrite.c,v 1.10 1995/06/29 00:28:04 bduggan Exp $"
...
#endif
{
	...
	Write( buffer, IM_UCHAR_TYPE, 1, 4 );
	...
	Write( &nDD,    IM_UINT16_TYPE, 2, 1 );
	offset = 0;				/* Filled in later	*/
	Write( &offset, IM_LONG_TYPE,   4, 1 );
	...
	Write( &tag,        IM_UINT_TYPE, 2, 1 );
	Write( &ref,        IM_UINT_TYPE, 2, 1 );
	Write( &dataOffset, IM_LONG_TYPE, 4, 1 );
	Write( &dataLength, IM_LONG_TYPE, 4, 1 );
...
#endif
{
	...
	offset = Tell( );
	nt[0] = 1;			/* Version 1			*/
	if ( pDim->dim_channelType == IM_UINT_TYPE)
	{
		nt[1] = IMHDFNTUINT;
	...
	offset = Tell( );
	tmpTag = IMHDFTNT;
	Write( &pDim->dim_width,       IM_INT_TYPE,    4, 1 );
	Write( &pDim->dim_height,      IM_INT_TYPE,    4, 1 );
	Write( &tmpTag,                IM_UINT16_TYPE, 2, 1 );
	Write( &tmpRef,                IM_UINT16_TYPE, 2, 1 );
	Write( &pDim->dim_pixelSize,   IM_INT_TYPE,    2, 1 );
	Write( &pDim->dim_interleave,   IM_INT_TYPE,    2, 1 );
	Write( &pDim->dim_compression, IM_UINT_TYPE,   2, 1 );
	if ( pDim->dim_compression == 0 )
	{
		...
		Write( &imHdfRef,         IM_UINT16_TYPE, 2, 1 );
...
#endif
{
	...
	if ( dim.dim_width <= 256 )
	{
		...
		Write( buffer, IM_UCHAR_TYPE, 1, 256 * 3 );
	}
	else
		Write( buffer, IM_UCHAR_TYPE, 1, n );
	free( (char *)buffer );
...
//	TagEntry       *tagEntry;	/* Tag table entry holder	*/
	...
	{
		dim.dim_channelType= IM_UINT_TYPE;
		dim.dim_pixelSize  = 1;
		clt = ImVfbQClt( vfb );
		if ( clt != IMCLTNULL )
		{
	...
	if ( (dim.dim_channelType != IM_UINT_TYPE) && (compRequest == IMCOMPRLE) )
	...
	{
		offset = Tell( );
		Write( &dim.dim_width,  IM_INT_TYPE, 2, 1 );
		Write( &dim.dim_height, IM_INT_TYPE, 2, 1 );
	...
	for ( i = 0; i < nRIG; i++ )
	{
		Write( &tagList[i], IM_UINT_TYPE, 2, 1 );
		Write( &refList[i], IM_UINT_TYPE, 2, 1 );
	}
...
#endif
{
	...
	IMCOPYPLANE( ImVfbQIndex8, IM_UCHAR_TYPE, 1 );
...
#endif
{
	...
	IMENCODEPLANE( ImVfbQIndex8, IM_UCHAR_TYPE, 1 );
...
#endif
{
	...
	IMCOPYPLANE( ImVfbQIndex16, IM_UINT32_TYPE, b );
...
#endif
{
	...
	for ( y = 0; y < height; y++ )
	{
		...
		Write( runBuffer, IM_UCHAR_TYPE, 1, width3 );
	}
	free( (char *)runBuffer );
...
#endif
{
	...
	for ( y = 0; y < height; y++ )
	{
		...
		Write( runBuffer, IM_UCHAR_TYPE, 1, width );
...
#endif
{
	...
	pPixel = ImVfbQFirst( vfb );
	IMCOPYPLANE( ImVfbQRed, IM_UCHAR_TYPE, 1 );

	pPixel = ImVfbQFirst( vfb );
	IMCOPYPLANE( ImVfbQGreen, IM_UCHAR_TYPE, 1 );

	pPixel = ImVfbQFirst( vfb );
	IMCOPYPLANE( ImVfbQBlue, IM_UCHAR_TYPE, 1 );
...
#endif
{
	...
	for ( y = 0; y < height; y++ )
	{
		...
		*rbp++   = oldIndex;
		...
		Write( runBuffer, IM_UCHAR_TYPE, 1, (rbp - runBuffer) );
		nBytes += (rbp - runBuffer);
	}
	free( (char *)runBuffer );
...
#endif
{
	...
	/* Red plane.							*/
	pPixel = ImVfbQFirst( vfb );
	IMENCODEPLANE( ImVfbQRed, IM_UCHAR_TYPE, 1 );

	/* Green plane.							*/
	pPixel = ImVfbQFirst( vfb );
	IMENCODEPLANE( ImVfbQGreen, IM_UCHAR_TYPE, 1 );

	/* Blue plane.							*/
	pPixel = ImVfbQFirst( vfb );
	IMENCODEPLANE( ImVfbQBlue, IM_UCHAR_TYPE, 1 );
