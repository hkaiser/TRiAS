...
const short DEX_QUERYOLDUNIQUEIDENTMCODE	= 486;	// Liefert MCode für ursprüngliche UniqueIdents.
...
typedef struct tagENUMLONGKEY {
	long eKey;		// @field Suchschlüssel, über den zu enumerieren ist.
...
typedef struct tagMERGEKOBJECT {
	...
	long mObj2;		// hinzuzufügendes Objekt
...
typedef struct tagDELETEKOBJECT {
	long doKObj;		// KomplexObjekt, aus dem herausgelöst werden soll
	long doObj;		// herauszulösendes Objekt
} DELETEKOBJECT;
...
typedef struct tagTARGETMERKMAL {
	DWORD dwSize;	// @field Die Größe dieser Struktur.
	LPARAM lTarget;	// @field Dieses Feld muß die Objektnummer, den
	...
	long lMCode;	// @field Enthält den Merkmalscode des zu behandelnden Merkmales.
...
typedef struct tagOBJCREATE {
	DWORD dwSize;	// @field Die Größe dieser Struktur.
	long lONr;	// @field Enthält nach erfolgreichem Ausführen der Funktion
	...
	long lIdent;	// @field Enthält den Identifikator des neu zu erzeugenden 
...
// zusätzliche Member #HK940920
	...
	DWORD m_dwData;		// evtl. Daten für SortierFunktion
} CREATEOBJEKTINFOEX;
...
typedef struct tagINVOKEOBJPROPDATA {
	...
	long lONr;	// @field Objektnummer des Objektes, für welches die 
...
typedef struct tagMODRELATION {
	DWORD dwSize;
	long lPONr;		// PrimärObjekt
	long lSONr;		// SekundärObjekt
...
typedef struct tagENUMRELATIONOBJS {
	DWORD dwSize;
	long lONr;		// Bezugsobjekt, für das Relationen geliefert werden
...
typedef struct tagCREATEDBEX {
	...
	LPCSTR m_pCFStr;	// ClipboarFormat (NULL für TRiAS.Document.2)
} CREATEDBEX;
...
typedef struct tagVIEWIDENTOBJECT {
	...
	ULONG m_lIdent;	// @field Der Identifikator, für den die
		// Funktion ausgeführt werden soll.
	long m_lONr;	// @field Die Objektnummer des Objektes, für 
