...
#elif (defined(__WATCOMC__) && defined(__386__)) || !defined(WIN16)
...
#define FLUSH_COMMITDATA	4	// #HK980323
...
_TRIAS01_ENTRY HRESULT FUNCTYPE EXPORT01 z_test_open(const char *dbfile, HPROJECT *phPr);
_TRIAS01_ENTRY HRESULT FUNCTYPE EXPORT01 z_addref_database (HPROJECT hPr);
...
_TRIAS01_ENTRY BOOL FUNCTYPE EXPORT01 GetIsCompoundDB (HPROJECT hPr, BOOL *pfIsCompound);
_TRIAS01_ENTRY BOOL FUNCTYPE CouldOpenTarget (HPROJECT hPr, BOOL *pfCouldOpen);
_TRIAS01_ENTRY BOOL FUNCTYPE IsDirty (HPROJECT hPr, BOOL *pfDirty);
_TRIAS01_ENTRY BOOL FUNCTYPE SetDirty (HPROJECT hPr, BOOL fDirty);
_TRIAS01_ENTRY BOOL FUNCTYPE IsOpened (HPROJECT hPr, BOOL *pfOpened);
_TRIAS01_ENTRY BOOL FUNCTYPE GetRelatedProject (HPROJECT hPr, HPROJECT *phPr);
_TRIAS01_ENTRY BOOL FUNCTYPE SetRelatedProject (HPROJECT hPr, HPROJECT hPrRel);
_TRIAS01_ENTRY BOOL FUNCTYPE GetTargetProject (HPROJECT hPr, HPROJECT *phPrTarget);
_TRIAS01_ENTRY HRESULT FUNCTYPE GetIdentMap (HPROJECT hPr, REFIID riid, LPVOID *ppIMap);
_TRIAS01_ENTRY HRESULT FUNCTYPE GetMCodeMap (HPROJECT hPr, REFIID riid, LPVOID *ppIMap);

_TRIAS01_ENTRY HRESULT FUNCTYPE ReReadScaleKoeffs (HPROJECT hPr);
_TRIAS01_ENTRY HRESULT FUNCTYPE GetScaleKoeffs (HPROJECT hPr, struct tagSCALEKOEFFS *pKoeffs);
_TRIAS01_ENTRY HRESULT FUNCTYPE SetScaleKoeffs (HPROJECT hPr, struct tagSCALEKOEFFS *pKoeffs);

_TRIAS01_ENTRY HRESULT FUNCTYPE ResetConnectFilter (HPROJECT hPr);
_TRIAS01_ENTRY HRESULT FUNCTYPE GetConnectFilter (HPROJECT hPr, struct tagCONNECTFILTEREX **ppFilter);
_TRIAS01_ENTRY HRESULT FUNCTYPE SetConnectFilter (HPROJECT hPr, struct tagCONNECTFILTER *pFilter);

_TRIAS01_ENTRY HRESULT FUNCTYPE GetPredefinedMCode (HPROJECT hPr, LONG *plMCode, enum tagPREDEFINEDMCODE rgWhich);
_TRIAS01_ENTRY HRESULT FUNCTYPE SetPredefinedMCode (HPROJECT hPr, LONG lMCode, enum tagPREDEFINEDMCODE rgWhich);

_TRIAS01_ENTRY HRESULT FUNCTYPE GetDataBaseName (HPROJECT hPr, char *pBuffer, int iLen);

// #HK010617
_TRIAS01_ENTRY BOOL FUNCTYPE GetIdentNoCase (HPROJECT hPr, BOOL *pfNoCase);
_TRIAS01_ENTRY BOOL FUNCTYPE SetIdentNoCase (HPROJECT hPr, BOOL fNoCase);
_TRIAS01_ENTRY BOOL FUNCTYPE GetAttribNoCase (HPROJECT hPr, BOOL *pfNoCase);
_TRIAS01_ENTRY BOOL FUNCTYPE SetAttribNoCase (HPROJECT hPr, BOOL fNoCase);

_TRIAS01_ENTRY HRESULT CreateMerkmalTarget (HPROJECT hPr, short iTTyp, long lTarget, long *pulNewTarget);
_TRIAS01_ENTRY long GetMerkmalTarget (HPROJECT hPr, void *pTarget, short iTTyp);
_TRIAS01_ENTRY HRESULT GetTextMerkmal (HPROJECT hPr, long lMCode, long lFirst, short iTyp, char **ppMText);
_TRIAS01_ENTRY HRESULT PutTextMerkmal (HPROJECT hPr, long MKode, long lTarget, short iTyp, const char *MText, BOOL fTextMCode = FALSE);
_TRIAS01_ENTRY HRESULT DeleteTextMerkmal (HPROJECT hPr, long MKode, long lTarget, short iTyp, BOOL fTextMCode = FALSE);
_TRIAS01_ENTRY HRESULT FindTextMerkmal (HPROJECT hPr, long MKode, const char *pText, int iMode, short iTyp, vector<unsigned long> &rObjs);
_TRIAS01_ENTRY HRESULT FindTextMerkmalF (HPROJECT hPr, long MKode, const char *pText, int iMode, short iTyp, HRESULT (CALLBACK *pFcn)(INT_PTR, DWORD, UINT_PTR), UINT_PTR dwData);
_TRIAS01_ENTRY HRESULT FindTextMerkmalPattern (HPROJECT hPr, long lMCode, const char *pPattern, int iMode, vector<unsigned long> &rObjs);
_TRIAS01_ENTRY HRESULT FindTextMerkmalPatternF (HPROJECT hPr, long lMCode, const char *pPattern, int iMode, HRESULT (CALLBACK *pFcn)(INT_PTR, DWORD, UINT_PTR), UINT_PTR dwData);
_TRIAS01_ENTRY HRESULT FindTextMerkmalPatternIndex (HPROJECT hPr, long lMCode, const char *pPattern, int iMode, vector<unsigned long> &rObjs);
_TRIAS01_ENTRY HRESULT FindTextMerkmalPatternIndexF (HPROJECT hPr, long lMCode, const char *pPattern, int iMode, HRESULT (CALLBACK *pFcn)(INT_PTR, DWORD, UINT_PTR), UINT_PTR dwData);
_TRIAS01_ENTRY HRESULT FindTextMerkmalPatternIndexNew (HPROJECT hPr, long lMCode, const char *pPattern, int iMode, vector<unsigned long> &rObjs);
_TRIAS01_ENTRY HRESULT FindTextMerkmalPatternIndexNewF (HPROJECT hPr, long lMCode, const char *pPattern, int iMode, HRESULT (CALLBACK *pFcn)(INT_PTR, DWORD, UINT_PTR), UINT_PTR dwData);

_TRIAS01_ENTRY HRESULT GetTextFromDBT (HPROJECT hPr, long lAdr, char *pMText, long lLen, BOOL fText);
_TRIAS01_ENTRY HRESULT PutTextToDBT (HPROJECT hPr, const char *pMText, short wf, long *madr, long zvom, BOOL fText);
_TRIAS01_ENTRY HRESULT GetTextMerkmalLen (HPROJECT hPr, long madr, long *plLen);
_TRIAS01_ENTRY HRESULT CreateLink (HPROJECT hPr, long FileNo, long *Begin, long Wert);
_TRIAS01_ENTRY HRESULT GetGeometryGuid (HPROJECT hPr, long lIdent, LPCSTR pcName, _GUID *pGuid);
_TRIAS01_ENTRY HRESULT GetGeometryGuids (HPROJECT hPr, long *pCnt, _GUID **pGuids);
#else // MSWIND
...
_TRIAS01_ENTRY int FUNCTYPE EXPORT01 CreateErrorTree();
...
_TRIAS01_ENTRY int FUNCTYPE gen_backup(struct file_entry *bf_p, struct pg_zero *pz_p, char *copyright, time_t time_val);
...
#if defined(_MSC_VER)
_TRIAS01_ENTRY int FUNCTYPE EXPORT01 qwindow(HPROJECT hPr, struct window *window, long (__cdecl *action)(struct db_addr *, ...), short iKeyIndex);
#else
...
_TRIAS01_ENTRY int FUNCTYPE EXPORT01 SetStartMode (int rgStartMode);
...
// obsolete functions
void FUNCTYPE dbinit_ (void);
long FUNCTYPE ngmini_ (HPROJECT hPr, long *, long *, long *);
long FUNCTYPE ngmc_ (long *, long *);
long FUNCTYPE lrec_ (short *lon);

_TRIAS01_ENTRY long FUNCTYPE dbcodb_ (HPROJECT hPr, long *);
_TRIAS01_ENTRY long FUNCTYPE ladr_ (HPROJECT hPr, short *lon, BOOL fLast);
