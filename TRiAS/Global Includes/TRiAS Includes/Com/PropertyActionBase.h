...
// benötigte Stringkonstante
	__declspec(selectany) const OLECHAR * g_cbPropertySequences = L"PropertySequences";		// Names des SubStreams im Projekt
...
// Funktionen, die BegleitNummern der Datenpakete schreiben/lesen
	HRESULT GetPacketNumber (IDataObject *pIDO, long *plNumber = NULL);
...
// Funktionen, die HWND zu einem IDataObject schreiben/lesen
	HRESULT GetHWND (IDataObject *pIDataObj, const FORMATETC *pcfe, HWND *phWnd = NULL);
...
// Funktionen für das Schreiben/Lesen einer ganzen PropertySequence im Projekt
	HRESULT LoadPropertySequence (BSTR bstrName, IPropertyActionSequence **ppIPropertyActionSequence, LPCOLESTR pcoleStrmName = g_cbPropertySequences);
	HRESULT SavePropertySequence (BSTR bstrName, IPropertyActionSequence *pIPropertyActionSequence, LPCOLESTR pcoleStrmName = g_cbPropertySequences);
} // namespace PropActBase
...
class ATL_NO_VTABLE XPersistPropertyBagImpl :
...
class ATL_NO_VTABLE CPropertyActionUsageImpl :
...
class ATL_NO_VTABLE CPropertyActionBase : 
...
// ***IPropertyAction3 methods
	...
	STDMETHOD(GetHeaderSubTitle) (THIS_ LPSTR pBuffer, ULONG wLen)
	{
		COM_TRY {
			if (NULL != pBuffer && wLen > 0) {
			// Namen der Property kopieren
...
// IPersistStreamInit methods
	...
	char cbBuffer[_MAX_PATH];
		...
		pcbSize -> QuadPart = sizeof(BOOL)+2*sizeof(DWORD)+sizeof(ULONG)+strlen(cbBuffer)+1;
		return S_OK;
	}
...
// diverse ZugriffsFunktionen
	IUnknown *GetUnknown() { return static_cast<IPropertyAction2 *>(this); }
...
class ATL_NO_VTABLE CPropertyActionImpl :
