...
// $Header: /usr/local/devel/GiST/libGiST/RSTree/RTentry.h,v 1.1.1.1 1996/08/06 23:47:24 jmh Exp $
...
public:
  ...
  operator < (const RTpenalty &p) const {
...
public:
  ...
  RTkey(const double xl_, const double xh_, const double yl_, const double yh_)
	{
		if (xl_ > xh_) {
			xl = xh_;
			xh = xl_;
  ...
  int disjoint(const RTkey& k) {
    return((xh < k.xl || xl > k.xh || yh < k.yl || yl > k.yh));
  }
