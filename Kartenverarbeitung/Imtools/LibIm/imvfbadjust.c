/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imvfbadjust.c,v 1.7 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imvfbadjust.c,v 1.7 1995/06/29 00:28:04 bduggan Exp $"
...
{									\
	...
	{								\
	case IMRED:							\
		pixel = (float)ImVfbQRed( dstVfb, pDst );			\
		pixel op value;						\
		...
		break;							\
	case IMGREEN:							\
		pixel = (float)ImVfbQGreen( dstVfb, pDst );			\
		pixel op value;						\
		if ( pixel > 255 ) pixel = 255;				\
		if ( pixel < 0   ) pixel = 0;				\
		...
		break;							\
	case IMBLUE:							\
		pixel = (float)ImVfbQBlue( dstVfb, pDst );			\
		pixel op value;						\
		if ( pixel > 255 ) pixel = 255;				\
		if ( pixel < 0   ) pixel = 0;				\
...
#endif
{
	...
	if ( keyStart < keyEnd )
	{
		keyLow  = (float)(keyStart);
		keyHigh = (float)(keyEnd);
	}
	else
	{
		keyHigh = (float)(keyStart);
		keyLow  = (float)(keyEnd);
	}
	if ( adjustStart < adjustEnd )
	{
		adjustLow  = (float)(adjustStart);
		adjustHigh = (float)(adjustEnd);
	}
	else
	{
		adjustHigh = (float)(adjustStart);
		adjustLow  = (float)(adjustEnd);
	}
	 ...
	 */
	if ( keyEnd != keyStart )
		factor = (float)((adjustEnd - adjustStart) / (keyEnd - keyStart));
	else
		factor = 0;
	...
	for ( i = 0; i < y; i++ )
	{
		...
		case IMRED:
			...
			break;
		    case IMVFBINDEX8:
			pixel = (float)(ImVfbQIndex8( dstVfb, pDst ));
			if ( clt != IMCLTNULL )
				pixel = (float)(ImCltQRed( ImCltQPtr( clt, (int)pixel)));
			break;
		    case IMVFBINDEX16:
			pixel = ImVfbQIndex16( dstVfb, pDst );
			if ( clt != IMCLTNULL )
			...
			break;
		    case IMVFBMONO:
			pixel = (float)(ImVfbQMono( dstVfb, pDst ) * 255);
			break;
		    }
		    break;
		case IMGREEN:
			...
			pixel = (float)(ImVfbQIndex8( dstVfb, pDst ));
			if ( clt != IMCLTNULL )
				pixel = (float)(ImCltQGreen( ImCltQPtr( clt, (int)pixel)));
			break;
		    case IMVFBINDEX16:
			pixel = ImVfbQIndex16( dstVfb, pDst );
			if ( clt != IMCLTNULL )
		...
		case IMBLUE:
			...
			pixel = (float)(ImVfbQIndex8( dstVfb, pDst ));
			if ( clt != IMCLTNULL )
				pixel = (float)(ImCltQBlue( ImCltQPtr( clt, (int)pixel)));
			break;
		    case IMVFBINDEX16:
			pixel = ImVfbQIndex16( dstVfb, pDst );
			if ( clt != IMCLTNULL )
		...
		case IMINDEX16:
		    ...
		    break;
		case IMMONO:
		    pixel = (float)(ImVfbQMono( dstVfb, pDst ));
		    break;
		case IMALPHA:
		    pixel = (float)(ImVfbQAlpha( dstVfb, pDst ));
		    break;
		case IMWPROT:
		    pixel = (float)(ImVfbQWProt( dstVfb, pDst ));
		    break;
		case IMZ:
		    pixel = (float)(ImVfbQZ( dstVfb, pDst ));
		    break;
		case IMIDATA:
		    pixel = (float)(ImVfbQIData( dstVfb, pDst ));
		    break;
		case IMFDATA:
		    pixel = ImVfbQFData( dstVfb, pDst );
		    break;
		...
		value = (float)(adjustStart + factor * (pixel - keyStart));
