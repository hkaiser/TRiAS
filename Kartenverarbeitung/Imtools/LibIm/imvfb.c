/**
 ** $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imvfb.c,v 1.20 1995/06/30 22:11:11 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imvfb.c,v 1.20 1995/06/30 22:11:11 bduggan Exp $"
...
///////////////////////////////////////////////////////////////////////////////
// use MMFiles optional
static int g_fUseMMFiles = 0;

int IMTOOLS_API ImToolsUseMMFiles()
{
	return g_fUseMMFiles;
}

int IMTOOLS_API ImToolsSetUseMMFiles (int fUseMMFiles)
{
int fUseMMFilesT = g_fUseMMFiles;

	g_fUseMMFiles = fUseMMFiles;
	return fUseMMFilesT;
}

#else
// define dummy functions
int IMTOOLS_API ImToolsUseMMFiles() { return 0; }
int IMTOOLS_API ImToolsSetUseMMFiles (int fUseMMFiles) { return 0; }
...
#endif
{
	...
	ImVfbPtr p = NULL; /* pointer to the frame buffer		*/
	...
	v -> hFile = (HANDLE)INVALID_HANDLE_VALUE;
	v -> hMMF = NULL;
	...
	if (g_fUseMMFiles) {
		...
		char cbTemp[_MAX_PATH];
			...
			GetTempPath (sizeof(cbTemp), cbTemp);
			GetTempFileName (cbTemp, "~im", 0, v -> szFile);
			v -> hFile = CreateFile (v -> szFile, GENERIC_WRITE | GENERIC_READ, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_TEMPORARY, NULL);
			if ((HANDLE)INVALID_HANDLE_VALUE != v -> hFile) {
			// Create MMF and a view to it
				v -> hMMF = CreateFileMapping (v -> hFile, NULL, PAGE_READWRITE, 0, width*height*nbytes, NULL);
				if (NULL != v -> hMMF) 
					p = (ImVfbPtr)MapViewOfFile (v -> hMMF, FILE_MAP_WRITE, 0, 0, 0);
			}
		}
...
#endif
{
	 ...
	 */
	divisor = (float)(1.0 / (src1Weight + src2Weight));
	src1Weight *= divisor;
	src2Weight *= divisor;
...
#endif
{
	...
	delta = (float)(s * ( r - 0.5 ));
	if( i <= 0.5 )
		r = (float)(2.*i*( 0.5 + delta ));
	else
		r = (float)(0.5 + delta + (2.*i-1.)*( 0.5 - delta ));

	/* green:							*/
	delta = (float)(s * ( g - 0.5 ));
	if( i <= 0.5 )
		g = (float)(2.*i*( 0.5 + delta ));
	else
		g = (float)(0.5 + delta + (2.*i-1.)*( 0.5 - delta ));

	/* blue:							*/
	delta = (float)(s * ( b - 0.5 ));
	if( i <= 0.5 )
		b = (float)(2.*i*( 0.5 + delta ));
	else
		b = (float)(0.5 + delta + (2.*i-1.)*( 0.5 - delta ));
...
#endif
{
	...
	/* compute saturation:						*/
	intens =  (float)(( fmin + fmax ) / (255.+255.));

	if( intens <= 0.5 )
		sat = diff / (fmax + fmin);
	else
		sat = (float)(diff / ( 255.+255. - fmax - fmin ));
...
#endif
{
	...
	for ( i = syt; i <= syb; i++ )
	{
		...
		pixel = (float)(fillStart + (i-syt) * yInc);
		switch ( fillField )
		{
		case IMRED:
			for ( j = sxl; j <= sxr; j++ )
