...
// Created: 03.02.2002 19:05:06 
//
// @doc
// @module BasicFunctions.inl | Declaration of the <c CBasicFunctions> class

#if !defined(_BASICFUNCTIONS_INL__09C73A0F_8E27_4A46_B13C_B405B46AD3DB__INCLUDED_)
#define _BASICFUNCTIONS_INL__09C73A0F_8E27_4A46_B13C_B405B46AD3DB__INCLUDED_
...
// Bewegungsrichtung, wenn man bei 3 gegebenen Punkten P1(x1,y1), P2(x2,y2) und 
// P3(x3,y3) vom ersten, zum zweiten und dann zum dritten geht; return-Werte sind
...
inline int 
TurnDirection (double x1, double y1, double x2, double y2, double x3, double y3)
{
double dx1 = x2 - x1;
double dy1 = y2 - y1;
double dx2 = x3 - x1;
...
// Lage des Punktes (x,y) bezüglich des geschlossenen Polygonzuges (pX, pY) der
// Länge lCnt; für bInner == true, liegt der Punkt innerhalb der eingeschlossenen 
// Fläche bzw. auf deren Kontur return-Werte sind
//   S_OK     Entscheidung konnte getroffen werden
//   E_FAIL   Polygonzug ist entartet bzw. nicht geschlossen (bInner unbestimmt)
inline HRESULT 
PunktLage (double x, double y, double* pX, double* pY, long lCnt, bool& bInner)
{
	if (!pX || !pY ||
...
long j = lCnt - 2;	// Index des letzten Polygonpunktes, von dem bekannt ist, daß
	...
	while (0 == TurnDirection (xa, ya, xe, ye, pX[j], pY[j])) {
		j--;
		if (j < 0)
			return E_FAIL;	// entarteter Polygonzug, d.h. alle Punkte liegen 
	}						// auf einer Geraden, die mit der Trägergeraden der 
							// Testlinie identsch ist
...
int iTD1, iTD2, iTD3;		// return-Werte von TurnDirection()
	...
	// Im nachfolgenden if-Zweig wird der Fall behandelt, daß der nächste Punkt 
	// P[k] nicht auf der waagerechten Testlinie liegt.
	// Liegt P[k] auf der Testlinie (else-Zweig), wird P[k] i.a. ignoriert.
	// Liegt jedoch der zu testende Punkt im Dreieck P[k-1], P[k], P[k+1], 
	// ergibt sich durch das Weglassen von P[k] eine Fehlinterpretation. 
	// Deshalb ist dieser Fall gesondert abzutesten.
		if (0 != TurnDirection (xa, ya, xe, ye, pX[k], pY[k])) {
		// Zähler lCutCnt inkrementieren, wenn echter Schnitt vorliegt, d. h. 
		// P[k] und P[j] auf verschiedenen Seiten der Testlinie liegen
			if (Intersection (pX[k], pY[k], pX[j], pY[j], xa, ya, xe, ye))
				lCutCnt++;
			j = k;
		}
		else {
		// P[k] liegt auf der Testlinie
...
// ungerade Schnittanzahl bedeutet Enthaltensein
	bInner = (lCutCnt & 1) ? true : false;
	return S_OK;
}

// 2 Strecken (x11,y11)-(x21,y21) und (x12,y12)-(x22,y22) haben wenigstens 
// einen Punkt gemeinsam; dann Rückgabe von true, sonst false
inline bool 
Intersection (double x11, double y11, double x21, double y21, double x12,
...
// Test, ob ein Punkt (x,y), der auf der Trägergeraden einer Strecke 
// (x1,y1)-(x2,y2) liegt, auch auf der Strecke selbst liegt (einschließlich der 
// beiden Endpunkt); dann ist der return-Wert "true"
// die Routine TurnDirection() wird nicht verwendet, da der Punkt (x,y) durch 
// die Rundung von double -> KoOrd "leicht" neben der Strecke liegen kann, 
// obwohl er eigentlich drauf liegen müßte
inline bool 
IsPointOnStraight (double x, double y, double x1, double y1, double x2, 
	double y2)
{
...
#endif // !defined(_BASICFUNCTIONS_INL__09C73A0F_8E27_4A46_B13C_B405B46AD3DB__INCLUDED_)
