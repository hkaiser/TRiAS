...
//	rDBO.SetPaintFlag (PF_NOTDRAWED);
	...
	if (!rDBO.LoadBitmapBars (false))	// Bitmaps für Punktobjekte laden 
		return E_OUTOFMEMORY;
...
{
#if defined(_FORCE_LEGACY_CODE)
	if (m_pDBOs -> Count() && (DBO().IsTRiASDBBased() || DBO().DB().isOpened()))	// überhaupt was geöffnet
#else
	if (m_pDBOs -> Count() > 0)	// überhaupt was geöffnet
#endif // defined(_FORCE_LEGACY_CODE)
	{
		if (!m_fIsClosing) {
...
CEierUhr Wait (this, CursorPath(g_cbCursorSave));
	...
	if (hDesc == 0) { 	// aktuelles Projekt schließen
		iIndex = m_pDBOs -> FActiveDBO();
		if (iIndex == -1) // keine aktive DBO
			iIndex = 0;	// die erste nehmen

#if defined(_FORCE_LEGACY_CODE)
		if (m_pDBOs -> Count() > 0 && (DBO().IsTRiASDBBased() || DBO().DB().isOpened()))	// überhaupt was geöffnet
#else
		if (m_pDBOs -> Count() > 0)	// überhaupt was geöffnet
#endif // defined(_FORCE_LEGACY_CODE)
		{
...
// jetzt feststellen, welches Projekt geschlossen werden soll
	...
	if (!(m_pDBOs -> Count() > iIndex &&
		  (DBO(iIndex).IsTRiASDBBased() || DBO(iIndex).DB().isOpened())))	// überhaupt was geöffnet
	{
		return (ErrCode)db_error (EC_NOOPEN, RC_CloseDB);
	}
...
// zu schließendes Projekt wiederfinden
	iIndex = m_pDBOs -> FFindItem (hDesc);
	if (iIndex == -1)
		return (ErrCode)db_error (EC_NOOPEN, RC_CloseDB);
...
bool fResult = false;
	...
	if (pDrive && pPath && pName) {
		...
		int iLevels = path.levels();
			...
			// Pfad verkürzen, lediglich den letzten Teil drinlassen
...
///////////////////////////////////////////////////////////////////////////////
// Namen für zu speicherndes Projekt abfragen
...
// einlesen sonstiger RegistryInfos
	...
	os_path path = os_string(cbBuffer);
		...
		if (strcmp (path.extension().c_str(), g_cbRiw+1)) 
			strDBName += g_cbRiw;		// einfach immer '.ris' dranhängen, wenns nicht schon da ist
...
// an dieser Stelle ist der Dateiname gültig
	return iIndex;
}
