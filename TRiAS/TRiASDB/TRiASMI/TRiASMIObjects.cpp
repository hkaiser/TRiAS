...
//
// @doc
// @module TRiASMIObjects.cpp | Implementation of the <c CTRiASMIObjects> class
...
STDMETHODIMP CTRiASMIObjects::InterfaceSupportsErrorInfo(REFIID riid)
{
...
STDMETHODIMP CTRiASMIObjects::get_DefaultType (BSTR *pbstrType)
{
	CHECKOUTPOINTER(pbstrType);

CIID Guid (CLSID_TRiASMIObject);		// DefaultType der _Elemente_ (hier: Object)!
...
STDMETHODIMP CTRiASMIObjects::SetupObjects(SETUPOBJECTSMODE rgMode)
{
HRESULT hr = E_INVALIDARG;
	...
	case SETUPOBJECTSMODE_Count:
		...
		RETURN_FAILED_HRESULT(EnsureObjectsCount());			// Objektanzahl feststellen
	...
	case SETUPOBJECTSMODE_Load:
		ERROR_EXPR_TRUE(!m_fIsInitialized, E_UNEXPECTED);
		hr =  EnsureObjectsLoaded();							// Objekte laden
	...
	case SETUPOBJECTSMODE_InitOrLoad:	// wird während der Initialisierung gerufen
		...
		WTRiASPropertyMapHandler MapHandler(m_BaseUnk);
			...
			THROW_FAILED_HRESULT(MapHandler -> AddMapEntry (
				CComBSTR(g_cbObjectsCursor), (INT_PTR)&CLSID_TRiASMICursorProperty, PROPERTYSUPPORT_NOFLAGS, vtMissing));
...
HRESULT CTRiASMIObjects::EnsureObjectsCount()
{
	...
	CMiTabLayer *pLayer = reinterpret_cast<CMiTabLayer *>(GetPropertyFrom (BaseObjs, g_cbObjectsCursor, 0L));

		_ASSERTE(NULL != pLayer);
		_ASSERTE(pLayer -> IsInitialized());
		_ASSERTE(this == pLayer -> GetObjsObject());
...
HRESULT CTRiASMIObjects::EnsureObjectsLoaded()
{
	...
	bool fUseObjectType = (BitCount(rgType) > 1) ? true : false;

		_ASSERTE(Map.IsValid());

		THROW_FAILED_HRESULT(BaseObjs -> get_Application (App.ppi()));
		for (long lObjCursor = mitab_c_next_feature_id(pLayer -> GetMiTabHandle(), -1);
			 -1 != lObjCursor;
			 lObjCursor = mitab_c_next_feature_id(pLayer -> GetMiTabHandle(), lObjCursor)) 
		{
		// zuerst versuchen, dieses Objekt in der Map wiederzufinden
		WTRiASObject Obj;
		
			if (!Map.IsValid() || S_OK != Map -> GetObject (static_cast<INT_PTR>(lObjCursor), Obj.ppu())) {
			// wenn nicht nur ein Objekttyp in der Tabelle ist, dann Objekt nach seinem Typ befragen
				if (fUseObjectType) {
				CMiTabObject Obj (mitab_c_read_feature(pLayer -> GetMiTabHandle(), lObjCursor));

					rgType = Obj.GetObjectType();
				}
				if (OBJECTTYPE_Unknown == rgType)
					continue;		// unbekannter Objekttyp (wird derzeit übergangen)
					
			// jetzt wirklich neu erzeugen und fertig initialisieren
				Obj = WTRiASObject(CLSID_TRiASMIObject);
			...
			// Typ etc. setzen
			wchar_t cbBuffer[64];
			CComBSTR bstrName (_ltow (lObjCursor, cbBuffer, 10));
			...
			// an dem erzeugten Objekt unsere Nummer (cursor) speichern
				THROW_FAILED_HRESULT(SetPropertyBy (Obj, g_cbObjectCursor, CComVariant(lObjCursor), true));
			}
...
STDMETHODIMP CTRiASMIObjects::SetupFeatures(SETUPFEATURESMODE rgMode)
{
	ERROR_EXPR_TRUE(!m_fIsInitialized, E_UNEXPECTED);
...
// unterstützte Attribute mit Objektmenge assoziieren
	if (!m_fHasFeatures || (rgMode & SETUPFEATURESMODE_Force)) {
		COM_TRY {
		WTRiASFeatures Feats (CLSID_TRiASMIFeatures);

			THROW_FAILED_HRESULT(WTRiASObjects(m_BaseUnk) -> put_Features (Feats))
			THROW_FAILED_HRESULT(Feats -> Refresh());	// ruft CTRiASMIFeatures::SetupFeatures()
			m_fHasFeatures = true;
...
// existierende Geometrien mit dieser Objektklasse assoziieren
	if (!m_fHasGeoFeatures || (rgMode & SETUPFEATURESMODE_Force)) {
		COM_TRY {
		WTRiASFeatures GeoFeats (CLSID_TRiASMIGeoFeatures);

			THROW_FAILED_HRESULT(WTRiASObjects(m_BaseUnk) -> put_GeometryFeatures (GeoFeats))
			THROW_FAILED_HRESULT(GeoFeats -> Refresh());	// ruft CTRiASMIGeoFeatures::SetupFeatures()
...
STDMETHODIMP CTRiASMIObjects::OnCreate (BSTR Name, BSTR Type, OBJECTTYPE rgType, ITRiASObject **ppIObj)
{
	...
	CIID ClsId (Type, CIID::INITCIID_InitFromProgId);
		...
		if (!ClsId)	ClsId = CLSID_TRiASMIObject;	// default ClsId
...
STDMETHODIMP CTRiASMIObjects::OnChanging(CHANGEDOBJECTS rgWhat, VARIANT vNewValue, VARIANT_BOOL *pDisAllow)
{
	...
	case CHANGEDOBJECTS_Handle:
		*pDisAllow = VARIANT_FALSE;		// immer einverstanden sein (da in MDSMap gespeichert)
		return S_OK;
...
/////////////////////////////////////////////////////////////////////////////
// Irgend was wurde modifiziert
STDMETHODIMP CTRiASMIObjects::OnChanged(CHANGEDOBJECTS rgWhat, VARIANT vOldValue)
{
	switch(rgWhat) {
	case CHANGEDOBJECTS_Name:
...
STDMETHODIMP CTRiASMIObjects::OnDelete (INT_PTR hObj)
{
#if defined(_READWRITE)
...
STDMETHODIMP CTRiASMIObjects::OnAdd (ITRiASObject *pIObj, CLASSIFYMODE rgMode)
{
...
STDMETHODIMP CTRiASMIObjects::OnRemove (ITRiASObject *pIObj)
{
#if defined(_READWRITE)
...
STDMETHODIMP CTRiASMIObjects::MapFeatureHandle (VARIANT vNameOrHandle, VARIANT *pvNewNameOrHandle)
{
...
STDMETHODIMP CTRiASMIObjects::OnFinalRelease()
{
	COM_TRY {
...
/////////////////////////////////////////////////////////////////////////////
// Helperfunktionen
HRESULT CTRiASMIObjects::OnChangedName(VARIANT vOldValue)
{
	COM_TRY {
...
HRESULT CTRiASMIObjects::OnChangedDescription(VARIANT vOldValue)
{
	COM_TRY {
...
HRESULT CTRiASMIObjects::OnChangedOKS(VARIANT vOldValue)
{
	COM_TRY {
...
HRESULT CTRiASMIObjects::OnChangedHandle(VARIANT vOldValue)
{
	COM_TRY {
	// neue Beschreibung vom Bezugsobjekt besorgen
	CComBSTR bstrOKS;
...
#if defined(_READWRITE)
HRESULT CTRiASMIObjects::OnChangedTypes(VARIANT vOldValue)
{
	return TRIASDB_E_DATABASENOTUPDATABLE;
}

HRESULT CTRiASMIObjects::OnChangedROMode(VARIANT vOldValue)
{
	return TRIASDB_E_DATABASENOTUPDATABLE;
}
#endif // defined(_READWRITE)
...
///////////////////////////////////////////////////////////////////////////////
// Koordinatensystem initialisieren und bei der Datenbank anmelden
HRESULT CTRiASMIObjects::InitCoordSystem(bool fForce)
{
	COM_TRY {
	// nur initialisieren, wenn es nicht bereits erfolgt ist, oder wenn erzwungen wird
		if (!fForce && m_guidCss.IsValid())
			return S_OK;

	// Koordinatensystemtransformation ist immer am Datenbank-Objekt abgelegt
	WTRiASObjects BaseObjs;
	WTRiASDatabase DBase;
	...
	WTRiASCSTransform CST (GetPropertyFrom (DBase, g_cbTRiASCSTransform, (IDispatch *)NULL), false);
		...
		ERROR_EXPR_FALSE(CST.IsValid(), TRIASMI_E_NO_COORDSYSTEM_GIVEN);

	// Koordinatensystem für diese Objektklasse erzeugen/aktualisieren
	CMiTabLayer *pLayer = reinterpret_cast<CMiTabLayer *>(GetPropertyFrom (BaseObjs, g_cbObjectsCursor, 0L));
	...
	CMiTabCSFactory CSService (pLayer);
	WTRiASCS CS;	

		THROW_FAILED_HRESULT(CSService.RetrieveCS(CS.ppi()));

	// CS-Guid an der Objektklasse speichern
	CComBSTR bstrGuid;

		THROW_FAILED_HRESULT(CS -> get_GUID(&bstrGuid));
		THROW_FAILED_HRESULT(SetPropertyBy (BaseObjs, g_cbCSSGuid, bstrGuid, true));
		m_guidCss = CIID(bstrGuid);

	// dieses Koordinatensystem als InputCS zum Transformator hinzufügen
		THROW_FAILED_HRESULT(CST -> AddInputCS (CS));
		THROW_FAILED_HRESULT(CS -> Recompute());		// Transformationspfad neu berechnen
...
///////////////////////////////////////////////////////////////////////////////
// ITRiASSearchObjects
HRESULT CTRiASMIObjects::ConsiderSearch (BSTR bstrCommand, SAFEARRAY *pParams)
{
...
HRESULT CTRiASMIObjects::SearchObjects (BSTR bstrCommand, ITRiASObjects *pIObjs, SAFEARRAY *pParams)
{
	if (wcscmp (bstrCommand, SEARCHCOMMAND_IntersectsWindow)) {
	// alle sonstigen Recherchen der Basisklasse überlassen
	WTRiASSearchObjects BaseObjs;
...
// Geometrierecherchen hier selbst ausführen
	...
	WTRiASFeature Feat;
		...
		_ASSERTE(ObjectIsActually (Feat, (CTRiASMIGeoFeature *)NULL));
...
HRESULT CTRiASMIObjects::GetSearchFeature (SAFEARRAY *pParams, ITRiASFeature **ppIFeat)
{
...
HRESULT CTRiASMIObjects::ConstraintObjects (ITRiASObjects *pIObjs)
{
WTRiASSearchObjects BaseObjs;
