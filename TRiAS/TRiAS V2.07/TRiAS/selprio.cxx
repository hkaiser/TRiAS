...
short doPrioritySelect (pWindow pW, CIdTree &rIDs, char *pActSight)
{
CPrioritySelDlg PSDlg (pW, rIDs);
int iResult;

	PSDlg.Show (Centre);
	if ((iResult = PSDlg.Result()) != 0) 
		rIDs.WriteBack (pActSight);
...
// Identifikatoren laden
	m_lvIdentList.InitControl (
			OTPunkt|OTLinie|OTFlaeche|OTText, 
			SELIDMODE_SINGLETYPE, NULL, CmpPriorities, 
...
LV_ITEM lviCurr, lviCurrSub;
char cbCurr[64], cbCurrSub[64];
...
LV_ITEM lviPrev, lviPrevSub;
char cbPrev[64], cbPrevSub[64];
...
// beteiligte Items vertauschen
	lviCurr.iItem = lviCurrSub.iItem = iIndexN;
	m_lvIdentList.SetItem (&lviCurrSub);
	m_lvIdentList.SetItem (&lviCurr);

	lviPrev.iItem = lviPrevSub.iItem = iIndexO;
