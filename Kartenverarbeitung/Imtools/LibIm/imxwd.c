/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imxwd.c,v 1.12 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER        "    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imxwd.c,v 1.12 1995/06/29 00:28:04 bduggan Exp $"
...
{
	/* xwd_pixmapFormat */		IM_ULONG_TYPE, 4, 1,
	/* xwd_pixmapDepth */		IM_ULONG_TYPE, 4, 1,
	/* xwd_pixmapWidth */		IM_ULONG_TYPE, 4, 1,
	/* xwd_pixmapHeight */		IM_ULONG_TYPE, 4, 1,
	/* xwd_xOffset */		IM_ULONG_TYPE, 4, 1,
	/* xwd_byteOrder */		IM_ULONG_TYPE, 4, 1,
	/* xwd_bitmapUnit */		IM_ULONG_TYPE, 4, 1,
	/* xwd_bitmapBitOrder */	IM_ULONG_TYPE, 4, 1,
	/* xwd_bitmapPad */		IM_ULONG_TYPE, 4, 1,
	/* xwd_bitsPerPixel */		IM_ULONG_TYPE, 4, 1,
	/* xwd_bytesPerLine */		IM_ULONG_TYPE, 4, 1,
	/* xwd_visual */		IM_ULONG_TYPE, 4, 1,
	/* xwd_redMask */		IM_ULONG_TYPE, 4, 1,
	/* xwd_greenMask */		IM_ULONG_TYPE, 4, 1,
	/* xwd_blueMask */		IM_ULONG_TYPE, 4, 1,
	/* xwd_bitsPerRGB */		IM_ULONG_TYPE, 4, 1,
	/* xwd_colormapEntries */	IM_ULONG_TYPE, 4, 1,
	/* xwd_nColors */		IM_ULONG_TYPE, 4, 1,
	/* xwd_windowWidth */		IM_ULONG_TYPE, 4, 1,
	/* xwd_windowHeight */		IM_ULONG_TYPE, 4, 1,
	/* xwd_windowX */		 IM_LONG_TYPE, 4, 1,
	/* xwd_windowY */		 IM_LONG_TYPE, 4, 1,
	/* xwd_windowBorderWidth */	IM_ULONG_TYPE, 4, 1,
#ifndef no_headerEnd
	/* Removed since even the Cray doesn't include it in the header.*/
	/* xwd_headerEnd */		IM_ULONG_TYPE, 4, 1,
#endif
					    0, 0, 0,
};
...
{
	/* xwd_pixel */		IM_ULONG_TYPE,  4, 1,
	/* xwd_red */		IM_USHORT_TYPE, 2, 1,
	/* xwd_green */		IM_USHORT_TYPE, 2, 1,
	/* xwd_blue */		IM_USHORT_TYPE, 2, 1,
	/* xwd_flags */		IM_CHAR_TYPE,   1, 1,
	/* xwd_pad */		IM_CHAR_TYPE,   1, 1,
				0,      0, 0,
};
...
//	ImVfbPtr     pPixel;		/* Xwdel pointer		*/
	 ...
	 */
	BinByteOrder( BINMBF );
	if ( ImBinRead( ioType, fd, fp, &headerSize, IM_ULONG_TYPE, 4, 1 ) == -1 )
		ImReturnBinError( );
	if ( ImBinRead( ioType, fd, fp, &version, IM_ULONG_TYPE, 4, 1 ) == -1 )
		ImReturnBinError( );
	...
	if ( version != IMXWDVERSION )
	{
		...
		for ( i = 0; i < sizeof( unsigned long ); i++ )
			swapBuffer[i] = (unsigned char)((version>>(i*8)) & 0xFF);
		version = 0;
		for ( i = 0; i < sizeof( unsigned long ); i++ )
			version = (version << 8) | swapBuffer[i];
		if ( version != IMXWDVERSION )
		{
			sprintf( message, "Unsupported XWD format version.\
  Only support version %d.",
				IMXWDVERSION );
			ImErrorFatal( message, -1, IMEUNSUPPORTED );
		}
		for ( i = 0; i < sizeof( unsigned long ); i++ )
			swapBuffer[i] = (unsigned char)((headerSize>>(i*8)) & 0xFF);
		headerSize = 0;
		for ( i = 0; i < sizeof( unsigned long ); i++ )
	...
	if ( i > 0 )	/* read window name */
	{
		...
		if( ImBinRead( ioType, fd, fp, xwd_windowName, IM_UCHAR_TYPE,
			1, i ) == -1 )
		{
			ImReturnBinError( );
		}
		TagTableAppend( tagTable, TagEntryAlloc( "image name",
			IM_POINTER_TYPE, &xwd_windowName ) );
	}
...
#endif
{
	...
	{
		pBuffer = buffer;
		if ( ImBinRead( ioType, fd, fp, buffer, IM_UCHAR_TYPE, 1, header->xwd_bytesPerLine ) == -1 )
		{
			free( (char *)buffer );
			ImReturnBinError( );
		}
...
#endif
{
	...
	if ( header->xwd_bitmapBitOrder == IMXWDLBF )
	{
		...
		{
			pBuffer = buffer;
			if ( ImBinRead( ioType, fd, fp, buffer, IM_UINT16_TYPE, 2,
				nBytes ) == -1 )
			{
				free( (char *)buffer );
				ImReturnBinError( );
...
#endif
{
	...
	if ( header->xwd_bitmapBitOrder == IMXWDLBF )
	{
		...
		{
			pBuffer = buffer;
			if ( ImBinRead( ioType, fd, fp, buffer, IM_UINT32_TYPE, 4,
				nBytes ) == -1 )
			{
				free( (char *)buffer );
				ImReturnBinError( );
...
#endif
{
	...
	{
		pBuffer = buffer;
		if ( ImBinRead( ioType, fd, fp, buffer, IM_UINT32_TYPE, 4, nPix)== -1)
		{
			free( (char *)buffer );
			ImReturnBinError( );
		}
...
#endif
{
	...
	BinByteOrder( BINMBF );
	if ( ImBinWrite( ioType, fd, fp, &headerSize, IM_ULONG_TYPE, 4, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
	if ( ImBinWrite( ioType, fd, fp, &version, IM_ULONG_TYPE, 4, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
	...
	if ( ImBinWrite( ioType, fd, fp, name, IM_UCHAR_TYPE, 1,
		strlen( name ) + 1 ) == -1 )
	{
		ImReturnBinError( );
	}
...
#endif
{
	...
	for ( y = 0; y < height; y++ )
	{
		 ...
		 */
		if ( ImBinWrite( ioType, fd, fp, buffer, IM_UCHAR_TYPE, 1, width)== -1 )
		{
			free( (char *)buffer );
			ImReturnBinError( );
		}
...
#endif
{
	...
	for ( y = 0; y < height; y++ )
	{
		 ...
		 *  Write out the buffer.
		 */
		if ( ImBinWrite( ioType, fd, fp, buffer, IM_UINT32_TYPE, 4, width)== -1)
		{
			free( (char *)buffer );
			ImReturnBinError( );
		}
