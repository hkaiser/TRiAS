...
CGIapi wwwServer(db, items(dispatchTable), dispatchTable);
char* defaultAddress = "localhost:6101";
socket_t::socket_domain domain = socket_t::sock_local_domain;
#else
#ifdef USE_QUEUE_MANAGER
HTTPapi wwwServer(db, items(dispatchTable), dispatchTable, true);
#else
HTTPapi wwwServer(db, items(dispatchTable), dispatchTable, false);
#endif
