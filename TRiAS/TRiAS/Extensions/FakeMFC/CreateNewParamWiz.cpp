...
void CCreateNewParamWiz::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_ADVANCEDDESC, m_ftAdvanced);
...
BEGIN_MESSAGE_MAP(CCreateNewParamWiz, CPropertyPage)
	...
	ON_BN_DOUBLECLICKED(IDC_EDITDATA, OnDoubleclickedEditdata)
	ON_BN_DOUBLECLICKED(IDC_IMPORTDATA, OnDoubleclickedImportdata)
...
// evtl. Dialog nicht zulassen
CConnectParams *pParams = GetPropSheet() -> GetConnectParams();

	if (pParams -> m_dwExtProps) {
		m_ftAdvanced.EnableWindow (TRUE);
		GetDlgItem(IDC_ADVANCED) -> EnableWindow (TRUE);
	} else {
		m_ftAdvanced.EnableWindow (FALSE);
		GetDlgItem(IDC_ADVANCED) -> EnableWindow (FALSE);
	}
}

CREATEMODE CCreateNewParamWiz::RetrieveCreateMode (void)
{
	if (m_rbImportData.GetCheck())
		GetPropSheet() -> m_rgCreateMode = CREATEMODE_ImportData;
	else
		GetPropSheet() -> m_rgCreateMode = CREATEMODE_EditData;
	
	return GetPropSheet() -> GetCreateMode();
}
...
void CCreateNewParamWiz::OnDoubleclickedEditdata() 
{
CPropertySheet *pSheet = (CPropertySheet *)GetParent();
...
void CCreateNewParamWiz::OnDoubleclickedImportdata() 
{
CPropertySheet *pSheet = (CPropertySheet *)GetParent();
