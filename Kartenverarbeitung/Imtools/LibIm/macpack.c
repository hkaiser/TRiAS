...
/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/macpack.c,v 1.10 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/macpack.c,v 1.10 1995/06/29 00:28:04 bduggan Exp $"
...
#endif
{
	...
	while ( wid > 0 ) 
	{
		...
		else if ( (cIn[0]==cIn[1]) &&  (cIn[1]==cIn[2]) ) 
		{
			...
			cnt=3;
			while ( ((unsigned int)cnt<wid) && (cnt<127) && (cIn[0]==cIn[cnt]) ) 
			{
				cnt++;
			}
		...
		else 
		{
			...
			cnt=0;
			while ( ((unsigned int)cnt<wid) && (cnt<127)
				&& ! ( (cIn[cnt+0]==cIn[cnt+1])
				    && (cIn[cnt+1]==cIn[cnt+2]) )    ) 
			{
				cOut[cnt+1] = cIn[cnt];
