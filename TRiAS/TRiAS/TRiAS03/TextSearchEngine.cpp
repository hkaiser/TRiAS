...
ULARGE_INTEGER liOwnSize;
	...
	liOwnSize.QuadPart = 7*sizeof(LONG);
	liOwnSize.QuadPart += ::GetSizeMax (m_Status.m_strFindWhat);

	for (CSearchList::iterator it = m_Status.m_lstFindWhat.begin(); 
		 it != m_Status.m_lstFindWhat.end(); ++it)
	{
		liOwnSize.QuadPart += ::GetSizeMax (*it);
	}
	liOwnSize.QuadPart += ::GetSizeMax (m_Status.m_strObjProp);
