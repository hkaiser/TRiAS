/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imhdfread.c,v 1.20 1995/06/30 22:00:40 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imhdfread.c,v 1.20 1995/06/30 22:00:40 bduggan Exp $"
...
//	imHdfDD      *pPrevDD;		/* Previous DD pointer		*/
	...
	for ( ; ; )
	{
		Read( &nDD,    IM_UINT_TYPE, 2, 1 );
		Read( &offset, IM_LONG_TYPE, 4, 1 );
		for ( i = 0; i < nDD; i++ )
		{
			Read( &tag,        IM_UINT_TYPE, 2, 1 );
			Read( &ref,        IM_UINT_TYPE, 2, 1 );
			Read( &dataOffset, IM_LONG_TYPE, 4, 1 );
			Read( &dataLength, IM_LONG_TYPE, 4, 1 );
	...
	for ( pRIG = rigList; pRIG; pRIG = pRIG->rig_next )
	{
		...
		for ( i = 0; i < nTag; i++ )
		{
			Read( &tagList[i], IM_UINT_TYPE, 2, 1 );
			Read( &refList[i], IM_UINT_TYPE, 2, 1 );
		}
		...
		for ( i = 0; i < nTag; i++ )
		{
			...
			case IMHDFTAR:	/* Aspect ratio			*/
				Seek( imHdfDDQDataOffset( pDD ) );
				Read( &pRIG->rig_aspectRatio, IM_FLOAT_TYPE, 4, 1 );
				break;

			case IMHDFTCCN:	/* Color correction		*/
				pRIG->rig_ccnGiven = TRUE;
				Seek( imHdfDDQDataOffset( pDD ) );
				Read( &pRIG->rig_gamma, IM_FLOAT_TYPE, 4, 1 );
				Read( pRIG->rig_red,    IM_FLOAT_TYPE, 4, 3 );
				Read( pRIG->rig_green,  IM_FLOAT_TYPE, 4, 3 );
				Read( pRIG->rig_blue,   IM_FLOAT_TYPE, 4, 3 );
				Read( pRIG->rig_white,  IM_FLOAT_TYPE, 4, 3 );
				break;
			...
			case IMHDFTCFM:	/* Color format			*/
			{
				...
				Seek( imHdfDDQDataOffset( pDD ) );
				Read( buffer, IM_CHAR_TYPE, 1, imHdfDDQDataLength( pDD ) );
		...
		if ( pRIG->rig_cltDD != NULL && pRIG->rig_cltDim == NULL )
		{
			...
			pRIG->rig_cltDim->dim_channelType        = IM_UCHAR_TYPE;
...
#endif
{
	...
	Read( &dim->dim_width,     IM_INT_TYPE,  4, 1 );
	Read( &dim->dim_height,    IM_INT_TYPE,  4, 1 );
	Read( &tagNT,              IM_UINT_TYPE, 2, 1 );
	Read( &refNT,              IM_UINT_TYPE, 2, 1 );
	Read( &dim->dim_pixelSize, IM_INT_TYPE,  2, 1 );
	Read( &dim->dim_interleave, IM_INT_TYPE,  2, 1 );
	Read( &tagComp,            IM_UINT_TYPE, 2, 1 );
	Read( &refComp,            IM_UINT_TYPE, 2, 1 );
	...
	case IMHDFNTINT:	/* int						*/
		dim->dim_channelType = (nt[1] == IMHDFNTUINT) ? IM_UINT_TYPE : IM_INT_TYPE ;
		switch ( nt[3] )
		{
		default:
		case IMHDFINTMBO:
	...
	case IMHDFNTUCHAR:/* unsigned char					*/
		...
		dim->dim_channelType = IM_UCHAR_TYPE;
		break;

	case IMHDFNTCHAR:	/* char						*/
		/* Ignore nt[3] = ASCII or bitwise-numeric.		*/
		dim->dim_channelType = IM_CHAR_TYPE;
		break;

	case IMHDFNTFLOAT:/* float					*/
		dim->dim_channelType = IM_FLOAT_TYPE;
	case IMHDFNTDOUBLE:/* double					*/
		dim->dim_channelType = (nt[1] == IMHDFNTFLOAT) ? IM_FLOAT_TYPE : IM_DOUBLE_TYPE ;
		switch ( nt[3] )
		{
		default:
		case IMHDFFLOATPC:
...
#endif
{
	...
	if ( pDim->dim_channelType != IM_UCHAR_TYPE || pDim->dim_channelSize != 1 ||
	...
	case 0:	/* RGB, RGB, RGB, ...				*/
		...
		Read( buffer, IM_UCHAR_TYPE, 1, n * 3 );
		n *= 3;
		for ( i = 0; i < n; i+=3 )
		{
...
#endif
{
	...
	if ( pDim->dim_channelType == IM_FLOAT_TYPE || pDim->dim_channelType == IM_DOUBLE_TYPE )
...
#endif
{
	...
	{
		Read( buffer, IM_UCHAR_TYPE, 1, pDim->dim_width );
		pBuffer = buffer;
...
#endif
{
	...
	Read( buffer, IM_UCHAR_TYPE, 1, imHdfDDQDataLength( pRIG->rig_imageDD ) );
...
#endif
{
	...
	for ( i = 0; i < pDim->dim_height; i++ )
	{
		Read( buffer, IM_UINT32_TYPE, pDim->dim_channelSize, pDim->dim_width );
		pBuffer = buffer;
		for ( j = 0; j < pDim->dim_width; j++ )
		{
			/* Truncate upper bits.				*/
...
#endif
{
	...
	for ( i = 0; i < pDim->dim_height; i++ )
	{
		Read( buffer, IM_UCHAR_TYPE, 1, pDim->dim_width * 3 );
		pBuffer = buffer;
		for ( j = 0; j < pDim->dim_width; j++ )
		{
