...
// Created: 25.05.2000 21:27:03
//
// @doc
// @module PathDialog.cpp | Implementation of the <c CPathDialog> class
...
{
HWND hEdit = ::GetDlgItem(m_hWnd, IDC_NEW_EDIT_PATH);

	if (NULL != hEdit) {
		::GetWindowText(hEdit, m_pPathDialog->m_szPathName, MAX_PATH);
		if (0 == m_pPathDialog->MakeSurePathExists (m_pPathDialog->m_szPathName)) {
...
HWND hEdit = ::GetDlgItem(m_hWnd, IDC_NEW_EDIT_PATH);
	...
	if (NULL != hEdit) {
		::GetWindowText(hEdit, m_pPathDialog->m_szPathName, MAX_PATH);
		SendMessage(BFFM_ENABLEOK, 0, _tcslen(m_pPathDialog->m_szPathName) > 0);
	} 
}
...
CPathDialog::CPathDialog(
	BOOL fOpenDlg, LPCTSTR lpszCaption, LPCTSTR lpszTitle, LPCTSTR lpszInitialPath, CWnd *pParent)
{
	m_hWnd = NULL;
...
CPathDialog* pDlg = (CPathDialog *)pData;
	...
	case BFFM_SELCHANGED:
		{
		HWND hEdit = ::GetDlgItem (hwnd, IDC_NEW_EDIT_PATH);
			...
			char szSelection[MAX_PATH];
				...
				::SetWindowText (hEdit, szSelection);
			}
		}
		break;
...
int iResult = -1;
	...
	if (SUCCEEDED(SHGetMalloc(&pMalloc))) {
		...
		if (pidl != NULL) {
			if (m_fOpenDlg)
				m_bGetSuccess = SHGetPathFromIDList(pidl, m_szPathName);
			pMalloc->Free(pidl);
		}
		if (m_bGetSuccess)
			iResult = IDOK;
...
//	VERIFY(strCap.LoadString (IDS_LONGCLASSNAME));
// KK010321
	...
	// validate path
		...
		strMsg.Format(IDS_NOFOLDER, lpPath);
		if (m_PathDialogSub.MessageBox(strMsg, strCap, MB_YESNO | MB_ICONQUESTION) != IDYES)
			return -1;
	...
	catch (int nErrCode)
	{
		...
		{
		case 1:
			strMsg.Format(IDS_INVALIDFOLDER, lpPath);
			break;

		case 2:
		default:
			strMsg.Format(IDS_COULDNOTCREATE, lpPath);
			break;
		}
		m_PathDialogSub.MessageBox(strMsg, strCap, MB_OK | MB_ICONEXCLAMATION);
	}
	return iRet;
}
...
// path must be "x:\..."
	if ((nLen < 3) || 
		( (szPath[0] < _T('A') || _T('Z') < szPath[0]) && (szPath[0] < _T('a') || _T('z') < szPath[0]) || 
