...
{
CProperty*  pProp;
CPropList::iterator it = m_PropList.begin();
...
CPropList::iterator it = m_PropList.begin();
	...
	while(it != m_PropList.end())
		delete (*it++);		
	m_PropList.erase(m_PropList.begin(), m_PropList.end());
	m_SH.cProperties = 0;
}
...
CPropList* CPropertySection::GetList( void )
{   return &m_PropList;  }
...
BOOL CPropertySection::WriteToStream( IStream* pIStream )
{
	...
	m_SH.cbSection = 0;
	m_SH.cProperties = m_PropList.size();
	LISet32( li, 0 );
	...
	LARGE_INTEGER liPropSet )
{
	...
	if (m_SH.cProperties || !m_PropList.empty())
		RemoveAll();
...
BOOL CPropertySection::SetName( DWORD dwPropID, LPCTSTR pszName )
{
	...
	TRY
	{
		...
		if (bNameExists)
			...
			m_NameDict.SetAt(strName, dwPropID);
	}
	CATCH (CException, e)
	{
...
CNameDictMap::iterator it = m_NameDict.begin();

	while (it != m_NameDict.end())
	{
...
CSectionList::iterator it = m_SectionList.begin();

	while(it != m_SectionList.end())
	{
		pSect = (*it++);
...
CSectionList::iterator it = m_SectionList.begin();
	...
	m_SectionList.erase(m_SectionList.begin(), m_SectionList.end());
	m_PH.cSections = 0;
}
...
CSectionList* CPropertySet::GetList( void )
{   return &m_SectionList;  }


HRESULT CPropertySet::WriteToStream( IStream* pIStream )
{
LPSTREAM        pIStrFIDO;
FORMATIDOFFSET  fido;
ULONG           cb;
...
// Write the Property List Header
	m_PH.cSections = m_SectionList.size();
	hr = pIStream->Write((LPVOID)&m_PH, sizeof(m_PH), &cb);
	if (sizeof(m_PH) != cb)
	{
...
CSectionList::iterator it = m_SectionList.begin();
	...
	// Using our cloned stream write the Format ID / Offset pair
		...
		hr = pIStrFIDO->Write((LPVOID)&fido, sizeof(fido), &cb);
		if (sizeof(fido) != cb)
		{
...
HRESULT CPropertySet::ReadFromStream( IStream* pIStream )
{
ULONG               cb;
FORMATIDOFFSET      fido;
ULONG               cSections;
