...
// Fenster aufmachen
CImportDlg *pIDlg = CImportDlg :: CreateInstance (this, *m_pEL, DBO());
	...
	DELETE_OBJ (pIDlg);		// hier wird die Erweiterung rausgeworfen
...
CImportDlg :: CImportDlg (pWindow pW, ExtensionList &rEL, DatenBasisObjekt &rDBO)
	    : COpenDialog (pW, RetrieveAvailImportFormats (rEL)),
	      m_rEL (rEL), m_fValid(false), m_pParent (pW), 
		  m_pCTF(rDBO.pCT()), m_pW(pW), m_rDBO(rDBO)
{
#if !defined(WIN16)
	m_lSem = -1;
#endif // WIN32
...
CImportDlg *CImportDlg :: CreateInstance (pWindow pW, ExtensionList &rEL, DatenBasisObjekt &rDBO)
{
CImportDlg *pDlg = new CImportDlg (pW, rEL, rDBO);
...
// AnfangsVerzeichnis auf das aktuelle Projektverzeichnis setzen
	...
	os_path path = os_string(m_pInitPath);

		path.extension(os_string(g_cbNil));
		path.base (os_string(g_cbNil));
		SetInitialDir ((LPCSTR)path);
	}
...
UINT uiCmd = GET_WM_COMMAND_CMD (wParam, lParam);
	...
	case IDC_COORDSYS:
		return SelectCoordSysParams (GetParent(), IDS_IMPORTFMTDLGCAP, DBO(), m_pCTF);
...
*/
			...
			string strKey;
...
#if defined(ENABLE_WIN32S_CODE)
				{
					strKey = strProgID;	// ProgID
					strKey += string(g_cbDel) + g_cbImportFormat;
					pReg = new CClassesRoot (strKey.c_str(), KEY_READ);
				}
#endif // ENABLE_WIN32S_CODE
