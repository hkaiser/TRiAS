...
STDMETHODIMP_(DWORD) CUndoRedoVisInfo :: Capabilities (signed char **pDesc)
{
	...
	ResString resUndo (ResID (IDS_UNDOVISINFO, &RF), 128);
		...
		*pDesc = (signed char *)CoTaskMemAlloc (_MAX_PATH);
		...
		char cbOKS[64];
			...
			ClassFromIdent(m_viOld.m_lIdent, cbOKS, sizeof(cbOKS));
			ultoa (m_viOld.m_lIdent, cbBuffer, DEX_GetIdBase());
			wsprintf ((char *)*pDesc, resUndo.Addr(), cbOKS);
		}
	}
