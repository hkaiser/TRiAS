// NF_PG2H.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CSetNF_PG2H, CRecordset)

CSetNF_PG2H::CSetNF_PG2H(CDatabase* pdb)
	: CRecordset(pdb)
{
	//{{AFX_FIELD_INIT(CSetNF_PG2H)
	m_NFP2_IDE = _T("");
	m_NFP2_FLN = _T("");
	m_NFP2_FLG = 0.0;
	m_NFP2_ABF = 0.0;
...
CSetNF_PG2H::~CSetNF_PG2H()
{
	if ( IsOpen())		Close();
}

CString CSetNF_PG2H::GetDefaultConnect()
{
	return _T("ODBC;DSN=GBM-TRiAS");
}

CString CSetNF_PG2H::GetDefaultSQL()
{
	return _T("[NF_PG2H]");
}

void CSetNF_PG2H::DoFieldExchange(CFieldExchange* pFX)
{
	//{{AFX_FIELD_MAP(CSetNF_PG2H)
...
/////////////////////////////////////////////////////////////////////////////
// CSetNF_PG2H diagnostics

#ifdef _DEBUG
void CSetNF_PG2H::AssertValid() const
{
	CRecordset::AssertValid();
}

void CSetNF_PG2H::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG

void CSetNF_PG2H::InitListen( const char* id )
{
	if ( !IsOpen())		Open();
	else				Requery();
}	// InitListen

void CSetNF_PG2H::InitArray( const char* id, CPG2DArray* array )
{
	if ( !IsOpen())		Open();
	else
	{
...
void CSetNF_PG2H::RewriteArray( CPG2DArray* array )
{
...
//			m_NFP2_ANS = pS -> m_NFP2_ANS;
			...
			g_pOPAngeschlGS -> SetName( pS -> m_NFP2_IDE, pS -> m_NFP2_FLN, 'V', ( test == "Y" ));
		}
...
BOOL CSetNF_PG2H::ModIDE( const char* alt, const char* neu )
{
...
void CSetNF_PG2H::DeleteIDE( const char* ide )
{
	ASSERT( ide != NULL );
	ASSERT( *ide != '\0' );
...
BOOL CSetNF_PG2H::IstAngeschlossen( const char* ide, const char* nr )
{
...
*/
	...
	{
		text = "Fehler in NF_PG2H::GetCount()";
		AfxMessageBox( text );
		goto cleanup;
	}

	request = "SELECT Count(*) FROM NF_PG2H";
