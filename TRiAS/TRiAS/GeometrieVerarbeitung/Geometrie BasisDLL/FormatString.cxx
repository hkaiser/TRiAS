...
void _GEOMETR_EXPORT __cdecl Format (string& rStr, LPCTSTR lpszFormat, ...)
{
	_ASSERTE (TIsValidString(lpszFormat, false));
...
void _GEOMETR_EXPORT FormatV (string& rStr, LPCTSTR lpszFormat, va_list argList)
{
	_ASSERTE (TIsValidString(lpszFormat, false));
...
TCHAR* pTmp = NULL;
	...
	if (NULL != pTmp)
	{
		memset (pTmp, '\0', sizeof(pTmp));		// 03.08.98 (wegen BoundsChecker)

	int iLen = _vstprintf(pTmp, lpszFormat, argListSave);

		_ASSERTE (iLen <= nMaxLen);
		rStr = pTmp;
		delete [] pTmp;
	}
