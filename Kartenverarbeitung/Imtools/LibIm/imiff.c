/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imiff.c,v 1.24 1995/06/30 22:16:18 bduggan Exp $
...
#define HEADER        "    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imiff.c,v 1.24 1995/06/30 22:16:18 bduggan Exp $"
...
#endif
{
	...
	do
	{
		if ( ImBinRead( ioType, fd, fp, buf, IM_CHAR_TYPE, 1, 1 ) == -1 )
		{
			ImReturnBinError( );
		}
...
#endif
{
	 ...
	 */
	if ( ImBinRead( ioType, fd, fp, magic, IM_UCHAR_TYPE, 1, IMIFFMAGICCNT+1)== -1 )
		ImReturnBinError( );
	...
	for( i=0; i<sizey; i++ )  
	{
		rbp   = runBuffer;
	 	...
	 	if ((ImBinRead( ioType, fd, fp, rbp, IM_UCHAR_TYPE, 1, bands * sizex)) == -1) 
		{
			free( (char *)runBuffer );
			ImReturnBinError( );
		}	
...
#endif
{
	 ...
	 */
	sprintf( buf, "ncaa\n\0" );		/* Magic		*/
	ImBinWrite( ioType, fd, fp, buf, IM_UCHAR_TYPE, 1, strlen(buf) );
	...
	if ( 1 == bands )
		for ( y = 0; y < sizey; y++ )
		{
			...
			/*
		 	 ...
		 	 */
			if ( ImBinWrite( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, 
							sizex * bands ) == -1)
			{
				free( (char *)runBuffer );
				ImReturnBinError( );
