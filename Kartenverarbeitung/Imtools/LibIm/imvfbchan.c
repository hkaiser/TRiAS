/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imvfbchan.c,v 1.6 1995/09/28 05:11:40 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imvfbchan.c,v 1.6 1995/09/28 05:11:40 bduggan Exp $"
...
#endif
{
   ...
   * (if it's not NULL)
   */
  if ((inFieldMask!=0) && !(inFields & inFieldMask))
  {
  ...
  inOK  = (inFieldMask==0);  /* Okay to copy from "none" */
  outOK = 0;                    /* Can't copy to "none"     */
  for (tempMask = (((unsigned int)1)<<31); (tempMask!=0) ; tempMask >>= 1)
  {
	if (tempMask==(unsigned int)inFieldMask)	
		inOK  = 1;
	if (tempMask==(unsigned int)outFieldMask)
		outOK = 1;
  }
  if (!inOK || !outOK)
  {
...
#endif
{
	...
	for (vfbIndex = 0; vfbIndex < numVfbs; vfbIndex++)
	{
		...
		TagTableReplace( tagTable, TagEntryQNthEntry( vfbEntry),
			TagEntryAlloc( "image vfb", IM_POINTER_TYPE, &outVfb));
		ImVfbFree (inVfb);
	}
	...
	if (fieldsToTest & IMVFBRED)
	{
		...
		{ 	/* Add a mapping to red to the flags table */
			...
			TagTableAppend(flagsTable,
				TagEntryAlloc("channel map request",IM_POINTER_TYPE, &newString));
		}
