/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imsynu.c,v 1.16 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imsynu.c,v 1.16 1995/06/29 00:28:04 bduggan Exp $"
...
#endif
{
	...
	for ( ; ; )
	{
				...
				case 'B':	/* skip bytes	*/
					for ( ; size > 0; size-=IMSYNUBUFSIZE)
						if ( ImBinRead(ioType,fd,fp,
							junk,IM_CHAR_TYPE,1,IMSYNUBUFSIZE)<0)
						{
							ImReturnBinError();
						}
					break;
		...
		{
			if ( ImBinRead( ioType, fd, fp, buffer, IM_UCHAR_TYPE, 1, n ) != n )
			{
				free( (char *)buffer );
				ImReturnBinError();
			}
...
#endif
{
	...
	for ( i=0; i < nVfbs; i++)
	{
		...
		if ( (fields & IMVFBINDEX8) != 0)
		{
			...
			if ( ImBinWrite(ioType,fd,fp,str,IM_CHAR_TYPE,1,strlen(str))
				== -1 )
			{
				ImReturnBinError();
			}
		...
		if ( ImBinWrite(ioType,fd,fp,str,IM_CHAR_TYPE,1,strlen(str)) == -1 )
		{
			ImReturnBinError();
		}
		...
		for ( y = height-1; y >= 0; y-- )
		{
			...
			if ( ImBinWrite( ioType, fd, fp, buffer, IM_UCHAR_TYPE, 1, width * 3 ) == -1 )
			{
				free( (char *)buffer );
				ImReturnBinError();
			}
...
#endif
{
	...
	do
	{
		switch ( ImBinRead( ioType, fd, fp, buf, IM_CHAR_TYPE, 1, 1 ) )
		{
