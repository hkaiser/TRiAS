...
static const TCHAR szRegSection[] = _T("Software\\uve\\TRiAS\\TipOfTheDay");
...
CString strStoredTime;
	...
	if (!IsWin32s()) {
	CCurrentUser regTip (szRegSection);
	DWORD dwVal = 0;

		if (regTip.GetDWORD (szIntStartup, dwVal))
...
struct _stat buf;
	...
	if (!IsWin32s()) {
	CCurrentUser regTip (szRegSection, true);
	...
	if (fseek(m_pStream, iFilePos, SEEK_SET) != 0)
		m_strTip.LoadString(CG_IDP_FILE_CORRUPT);
	else
...
bool bError = false;
	...
	if (bError)
		strNext.LoadString(CG_IDP_FILE_CORRUPT);
}
...
// Update the startup information stored in the INI file
	...
	DWORD dwVal = m_bStartup ? true : false;
		...
		regTip.SetDWORD (szIntStartup, dwVal);
	} else {
	CWinApp* pApp = AfxGetApp();
