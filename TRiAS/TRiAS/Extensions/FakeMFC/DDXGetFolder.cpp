...
#include "ConnectParams.h"		// CFiles
...
CString sInitialDir;
	...
	BYTE WinMajorVersion = LOBYTE(LOWORD(GetVersion()));
		...
		BROWSEINFO bi;
			...
			TCHAR sPath[_MAX_PATH];
				...
				if (SHGetPathFromIDList(pItemIDList, sPath)) {
					...
					SetWindowText(sPath);
					m_Files.insert(sPath);
					GetParent()->PostMessage(WM_DIRSELECTED);
				}
		...
		CFileDirDialog dlg(FALSE, NULL, NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_ENABLETEMPLATE, NULL, this);
			...
			if (dlg.DoModal() == IDOK)
			{
				...
				m_Files.insert(dlg.m_ofn.lpstrFile);
				GetParent()->PostMessage(WM_DIRSELECTED);
			}
		}
	} else {
	// nur so tun als ob
		...
		m_Files.insert (os_string(sInitialDir));
		GetParent()->PostMessage(WM_DIRSELECTED);
	}
}
...
BEGIN_MESSAGE_MAP(CFindTreeView, CTreeCtrl)
	//{{AFX_MSG_MAP(CFindTreeView)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CFindTreeView message handlers

HTREEITEM CFindTreeView::FindItem (HTREEITEM hItem, LPCSTR pcItem)
{
...
// anschlieﬂen alle Childs abtesten
...
HTREEITEM CFindTreeView::FindString (LPCSTR pcItem)
{
