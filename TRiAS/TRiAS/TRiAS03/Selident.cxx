...
// ListViewCtrl initialisieren/Identifikatoren laden (aber nur gewünschte Typen)
...
HRESULT hr = S_OK;
	...
	ULONG ulCount = (NULL != m_pIEnumObjs) ? m_pIEnumObjs -> Count() : 0L;
		...
		// alle Objekte der Eingabemenge durchgehen und Zugehörigkeit testen
		LONG lONr = 0;
			...
			CIdents::iterator it = Ids.find (lIdent);
				...
				CIdents::iterator it = Ids.find (DEX_GetObjIdentEx (lONr));		
					...
					if (it != Ids.end() && ((*it).second & OTypToBits((ObjTyp)iTyp))) {
						IObjs -> AddItem (lONr);	// zur Ausgabemenge hinzufügen
						Tick();
					}
				} 
				else {
