/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/impic.c,v 1.17 1995/06/29 00:28:04 bduggan Exp $ 
...
#define HEADER	"   $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/impic.c,v 1.17 1995/06/29 00:28:04 bduggan Exp $"
...
{
	{ IM_UINT_TYPE, 4, 1 },			/* Magic number			*/
	{ IM_USHORT_TYPE, 2, 1 },		/* Zero for current release	*/
	{ IM_UCHAR_TYPE, 1, IMPICLABEL },	/* Ascii description of image	*/
	{ IM_LONG_TYPE, 4, 1 },			/* Ptr to label continuation	*/
	{ IM_UCHAR_TYPE, 1, IMPICGAP },		/* Unused space in header info	*/
	{ IM_USHORT_TYPE, 2, 1 },		/* Pixel height of full image	*/
	{ IM_USHORT_TYPE, 2, 1 },		/* Pixel width of full image	*/
	{ IM_USHORT_TYPE, 2, 1 },		/* Pixel height of each tile	*/
	{ IM_USHORT_TYPE, 2, 1 },		/* Pixel width of each tile	*/
	{ IM_USHORT_TYPE, 2, 1 },		/* four bits designating RGBA	*/
	{ IM_USHORT_TYPE, 2, 1 },		/* encoding and number of bits	*/
	{ IM_USHORT_TYPE, 2, 1 },		/* optimum disk transfer chunk	*/
	{ IM_USHORT_TYPE, 2, 1 },		/* matted-to-black:0 unassoc.:1	*/
	{ IM_USHORT_TYPE, 2, 1 },		/* horizontal offset for picture*/
	{ IM_USHORT_TYPE, 2, 1 },		/* vertical offset for picture	*/
	{ IM_UCHAR_TYPE, 1, IMPICSPACE },	/* mysterious void in header	*/
	{ IM_UINT_TYPE, 4, 1 },			/* Unused int space		*/
	{ IM_UCHAR_TYPE, 1, IMPICUNUSED },	/* Unused expansion space 	*/
	{ IM_LONG_TYPE, 4, 1 },			/* Location of tile		*/
	{ IM_UINT_TYPE, 4, 1 },			/* Length of tile		*/
	{ 0, 0, 0 }
};
...
#endif
{
	...
	if ( (h->tile_ptr - IMPICHEADERSIZE) > 0 )
	{
		...
		else
		{
			...
			if (ImBinRead(ioType, fd, fp, empty, IM_UCHAR_TYPE, 
	 ...
	 */
	if (ImBinRead(ioType, fd, fp, info, IM_UCHAR_TYPE, 1, h->block)==-1)
	{
		free( (char *)info );
		ImReturnBinError();
	}
	...
	switch ( h->pic_format )
	{
		...
		for ( i=0; i<y; i++ )
		{
			...
			if (  bytecount >= h->block-2  )
			    ...
			    if (ImBinRead(ioType, fd, fp, info, IM_UCHAR_TYPE,
					  1, h->block) == -1)
			    {
				free( (char *)info );
				ImReturnBinError();
	...
	switch ( h->pic_format )
	{
		...
		for ( i=0; i<y; i++ )
		{
			/*
	 		 ...
	 		 */
			if (ImBinRead(ioType,fd,fp,info,IM_UCHAR_TYPE,1,4*x) == -1)
			{
				free( (char *)info );
				ImReturnBinError();
			}
		...
		for ( i=0; i<y; i++ )
		{
			/*
	 		 * read a scanline of pixel info to be dumped
	 		 */
			if (ImBinRead(ioType,fd,fp,info,IM_UCHAR_TYPE,1,3*x) == -1)
			{
				free( (char *)info );
				ImReturnBinError();
			}
		...
		for ( i=0; i<y; i++ )
		{
			/*
	 		 * read a scanline of pixel info to be dumped
	 		 */
			if (ImBinRead(ioType,fd,fp,info,IM_UCHAR_TYPE,1,x) == -1)
			{
				free( (char *)info );
				ImReturnBinError();
			}
...
#endif
{
	 ...
	 * Then read first block of pixel info
	 */
	if (ImBinRead(ioType, fd, fp, info, IM_UINT16_TYPE, 2, h->block/2) == -1)
	{
		free( (char *)info );
		ImReturnBinError();
	}
	...
	switch ( h->pic_format )
	{
		...
		for ( i=0; i<y; i++ )
		{
			...
			if (  bytecount >= h->block-2  )
		    	{
			    if (ImBinRead(ioType, fd, fp, info, IM_UINT16_TYPE,
					  2, h->block/2 ) == -1)
			    {
				free( (char *)info );
				ImReturnBinError();
	...
	switch ( h->pic_format )
	{
		...
		for ( i=0; i<y; i++ )
		{
			/*
	 		 * read a scanline of pixel info to be dumped
	 		 */
			if (ImBinRead(ioType,fd,fp,info,IM_UINT16_TYPE,2,4*x) == -1)
			{
				free( (char *)info );
				ImReturnBinError();
			}
		...
		for ( i=0; i<y; i++ )
		{
			/*
	 		 * read a scanline of pixel info to be dumped
	 		 */
			if (ImBinRead(ioType,fd,fp,info,IM_UINT16_TYPE,2,3*x) == -1)
			{
				free( (char *)info );
				ImReturnBinError();
			}
		...
		for ( i=0; i<y; i++ )
		{
			/*
	 		 * read a scanline of pixel info to be dumped
	 		 */
			if (ImBinRead(ioType,fd,fp,info,IM_UINT16_TYPE,2,x) == -1)
			{
				free( (char *)info );
				ImReturnBinError();
			}
...
#endif
{
	...
	if ( numWritten < IMPICWRITESTART )
	{
		...
		if(ImBinWrite(ioType,fd,fp,buf,IM_UCHAR_TYPE,1,numToWrite)== -1 )
		{
			free( buf );
			ImReturnBinError();
		}
...
#endif
{
	...
	for( i=0; i<y; i++ )
	{
		...
		if( ImBinWrite(ioType,fd,fp,bufStart,IM_UCHAR_TYPE,1,4*x) == -1 )
		{
			free( bufStart );
			ImReturnBinError();
		}
...
#endif
{
	...
	for( i=0; i<y; i++ )
	{
		...
		*(bufStart+1) = (unsigned char) ((flag << 4) + (count >> 8));
		...
		if(ImBinWrite(ioType,fd,fp,bufStart,IM_UCHAR_TYPE,1,numToWrite)== -1 )
		{
			free( bufStart );
			ImReturnBinError();
		}
...
#endif
{
	...
	for( i=0; i<y; i++ )
	{
		...
		if(ImBinWrite(ioType,fd,fp,bufStart,IM_UCHAR_TYPE,1,3*x) == -1 )
		{
			free( bufStart );
			ImReturnBinError();
		}
...
#endif
{
	...
	for( i=0; i<y; i++ )
	{
		...
		if(ImBinWrite(ioType,fd,fp,bufStart,IM_UCHAR_TYPE,1,x) == -1 )
		{
			free( bufStart );
			ImReturnBinError();
		}
