...
namespace si  {
  ...
  //
  template<>
  inline void Length::showAs( Length const newUnit, string const & tag )  {
    prepareToDisplay();
    Length::setScale( WV::dimLength, newUnit.n_, tag );
  }
  template<>
  inline void Mass::showAs( Mass const newUnit, string const & tag )  {
    prepareToDisplay();
    Mass::setScale( WV::dimMass, newUnit.n_, tag );
  }
  template<>
  inline void Time::showAs( Time const newUnit, string const & tag )  {
    prepareToDisplay();
    Time::setScale( WV::dimTime, newUnit.n_, tag );
  }
  template<>
  inline void Current::showAs( Current const newUnit, string const & tag )  {
    prepareToDisplay();
    Current::setScale( WV::dimCurrent, newUnit.n_, tag );
  }
  template<>
  inline void Temperature::showAs( Temperature const newUnit, string const & tag )  {
    prepareToDisplay();
    Temperature::setScale( WV::dimTemp, newUnit.n_, tag );
  }
  template<>
  inline void Molarity::showAs( Molarity const newUnit, string const & tag )  {
    prepareToDisplay();
    AmountOfSubstance::setScale( WV::dimSubst, newUnit.n_, tag );
  }
  template<>
  inline void Luminosity::showAs( Luminosity const newUnit, string const & tag )  {
    prepareToDisplay();
  ...
  //
  template<>
  inline bool Length::initAs( Length const newUnit, string const & tag )  {
    return  Length::initScale( WV::dimLength, newUnit.n_, tag );
  }
  template<>
  inline bool Mass::initAs( Mass const newUnit, string const & tag )  {
    return  Mass::initScale( WV::dimMass, newUnit.n_, tag );
  }
  template<>
  inline bool Time::initAs( Time const newUnit, string const & tag )  {
    return  Time::initScale( WV::dimTime, newUnit.n_, tag );
  }
  template<>
  inline bool Current::initAs( Current const newUnit, string const & tag )  {
    return  Current::initScale( WV::dimCurrent, newUnit.n_, tag );
  }
  template<>
  inline bool Temperature::initAs( Temperature const newUnit, string const & tag )  {
    return  Temperature::initScale( WV::dimTemp, newUnit.n_, tag );
  }
  template<>
  inline bool Molarity::initAs( Molarity const newUnit, string const & tag )  {
    return  AmountOfSubstance::initScale( WV::dimSubst, newUnit.n_, tag );
  }
  template<>
  inline bool Luminosity::initAs( Luminosity const newUnit, string const & tag )  {
