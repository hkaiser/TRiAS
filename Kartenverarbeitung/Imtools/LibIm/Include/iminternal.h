...
#define CCR		IMCOMPCCITTRLE			/* CCITTRLE compression			*/
#define CC3		IMCOMPCCITTFAX3			/* CCITTFAX3 compression		*/
#define CC4		IMCOMPCCITTFAX4			/* CCITTFAX4 compression		*/
...
IMTOOLS_API extern char *ImErrorProgramName;		/* Program name		    */
IMTOOLS_API extern char *ImErrorFileName;			/* File name		    */
IMTOOLS_API extern FILE *ImErrorStream;			/* Error stream		    */
IMTOOLS_API extern FILE *ImInfoStream;			/* Info stream		    */
#ifdef __STDC__
IMTOOLS_API extern int (*ImErrorHandler)(int, int, char* );	/* Error handler	    */
IMTOOLS_API extern int (*ImInfoHandler)(char*,char*,char*);	/* Info handler	    	    */
#else
...
#define ImInfoPlain(msg)						\
{									\
	char infoMessage[500];						\
	sprintf( infoMessage, "%-*s\n", IMINFOMSGWIDTH1, msg );\
	if ( ImInfoHandler )						\
		(*ImInfoHandler)( ImErrorProgramName, ImErrorFileName,	\
			infoMessage );					\
	else if ( ImInfoStream )					\
 ...
 */
#ifdef __STDC__
IMTOOLS_API extern int ImGetTransparency(TagTable* tagTable, TagTable* flagsTable, ImVfb* vfb);
IMTOOLS_API extern int ImVfbProcessMapRequests( TagTable* flagsTable, TagTable* tagTable);
IMTOOLS_API extern int imLzwReadByte(int ioType,int fd,FILE *fp,int flag,int input_code_size);
IMTOOLS_API extern int imLzwCompGif(int ioType,int fd,FILE *fp,int code_size, unsigned char *rasterdata, int size);
IMTOOLS_API extern void PackBits( unsigned char *cIn, unsigned char *cOut, unsigned int *width );
IMTOOLS_API extern void PackBits3( unsigned char *cIn, unsigned char *cOut, unsigned int * width );
IMTOOLS_API extern void UnpackBits( unsigned char *srcPtr, unsigned char *dstPtr, unsigned int *cnt );
IMTOOLS_API extern void UnpackBits3( unsigned char *srcPtr, unsigned char *dstPtr, unsigned int *cnt );
