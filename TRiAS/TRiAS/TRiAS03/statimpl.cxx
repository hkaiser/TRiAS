...
STDMETHODIMP CProgressIndicator :: Restart (LONG ulMin, LONG ulMax, DWORD dwFlags)
{
	if (NULL == m_pDlg) {
...
STDMETHODIMP_(int) CProgressIndicator :: SetPosition (LONG uiNewPos)
{
...
//#if _MSC_VER >= 1100
STDMETHODIMP CProgressIndicator::InitNew (LONG hWndParent, LONG dwReserved, LONG *phWnd)
//#else
...
STDMETHODIMP CProgressIndicator::Restart (LONG ulMinPos, LONG ulMaxPos, PIFLAGS rgFlags)
{
...
STDMETHODIMP CProgressIndicator::SetPosition (LONG ulNewPos, INT *piPercent)
{
...
STDMETHODIMP CProgressIndicator::get_Canceled (VARIANT_BOOL *pfCanceled)
{
	if (NULL == pfCanceled)
		return E_POINTER;
	*pfCanceled = WasCanceled() ? VARIANT_TRUE : VARIANT_FALSE;
	return S_OK;
}
...
STDMETHODIMP CProgressIndicator::RetrieveRange (LONG *pulMinPos, LONG *pulMaxPos)
{
	return m_pDlg -> GetRange ((ULONG *)pulMinPos, (ULONG *)pulMaxPos);
}

// #HK001017
STDMETHODIMP CProgressIndicator::Show()
{
	if (!m_pDlg -> isVisible())
		m_pDlg -> Show();
	return S_OK;
...
STDMETHODIMP CProgressIndicator::Hide()
{
	if (m_pDlg -> isVisible())
		m_pDlg -> Hide();
	return S_OK;
