/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imrgb.c,v 1.22 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imrgb.c,v 1.22 1995/06/29 00:28:04 bduggan Exp $"
...
{
	{ IM_USHORT_TYPE, 2, 1 }, 		/* rgb_imagic			*/
	{ IM_USHORT_TYPE, 2, 1 },		/* rgb_type			*/
	{ IM_USHORT_TYPE, 2, 1 },		/* rgb_dim			*/
	{ IM_USHORT_TYPE, 2, 1 },		/* rgb_width			*/
	{ IM_USHORT_TYPE, 2, 1 },		/* rgb_height			*/
	{ IM_USHORT_TYPE, 2, 1 },		/* rgb_zsize			*/
	{ IM_INT_TYPE, 4, 1 },			/* rgb_min			*/
	{ IM_INT_TYPE, 4, 1 },			/* rgb_max			*/
	{ IM_INT_TYPE, 4, 1 },			/* rgb_wastedbytes		*/
	{ IM_CHAR_TYPE, 1, 80 },		/* rgb_name			*/
	{ IM_INT_TYPE, 4, 1 },			/* rgb_colormap			*/

	{ IM_CHAR_TYPE, 1, 32 },		/* rgb_dummy			*/

	{ IM_UINT_TYPE, 4, 1 },			/* rgb_rleend			*/
	{ IM_UINT_TYPE, 4, 1 },			/* rgb_rowstart			*/
	{ IM_UINT_TYPE, 4, 1 },			/* rgb_rowsize			*/
	{ IM_CHAR_TYPE, 1, 360 },		/* rgb_pad			*/
	{ 0, 0, 0 }
};
...
#endif
{
	 ...
	 */
	BinByteOrder( BINMBF );
	if ( ImBinRead( ioType, fd, fp, &(header.rgb_imagic),IM_USHORT_TYPE,2,1) == -1 )
	{
		ImReturnBinError( );
	}
	...
	if( IMRGB_ISRLE( header.rgb_stype ) )
	{
		 ...
		 */
		if( ImBinRead( ioType, fd,fp,starttable,IM_ULONG_TYPE, 4, y * z) == -1 )
		{
			free( (char *)starttable );
			free( (char *)sizetable );
			ImReturnBinError( );
		}
		if( ImBinRead( ioType, fd, fp, sizetable,IM_LONG_TYPE, 4, y * z) == -1 )
		{
			free( (char *)starttable );
			free( (char *)sizetable );
			ImReturnBinError( );
		...
		switch( z )
		{
		  ...
		  case 3:	/* 24-bit RGB image */
			...
			if ( ImBinRead ( ioType, fd, fp, runBuffer,
				IM_UCHAR_TYPE, 1, sizetable[0*y+j]) == -1 )
			{
				free( (char *)runBuffer );
				free( (char *)starttable );
				free( (char *)sizetable );
			...
			if ( ImBinRead ( ioType, fd, fp, runBuffer,
				IM_UCHAR_TYPE, 1, sizetable[1*y+j]) == -1 )
			{
				free( (char *)runBuffer );
				free( (char *)starttable );
				free( (char *)sizetable );
			...
			if ( ImBinRead ( ioType, fd, fp, runBuffer,
				IM_UCHAR_TYPE, 1, sizetable[2*y+j]) == -1 )
			{
				free( (char *)runBuffer );
				free( (char *)starttable );
				free( (char *)sizetable );
		  ...
		  case 4:	/* 32-bit RGB image + alpha channel */
			...
			if ( ImBinRead ( ioType, fd, fp, runBuffer,
				IM_UCHAR_TYPE, 1, sizetable[3*y+j]) == -1 )
			{
				free( (char *)runBuffer );
				free( (char *)starttable );
				free( (char *)sizetable );
	...
	else /* VERBATIM format */
	{
		...
		if ( ImBinRead( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, 4*x*y)==-1)
		{
			free( (char *)runBuffer );
			ImReturnBinError( );
		}
	...
	if ( strlen( header.rgb_name ) > 0 )
	{
		...
		TagTableAppend( tagTable,
			TagEntryAlloc( "image name", IM_POINTER_TYPE, &rgbName ) );
	}
...
//	int          n;			/* Counter			*/
	 ...
	 */
	if ( ImBinWrite( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, x * y ) == -1)
	{
		free( (char *)runBuffer );
		ImReturnBinError( );
	}
...
//	int          n;			/* Counter			*/
	 ...
	 *  Write out the buffer.
	 */
	if ( ImBinWrite( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, x * y * 3 ) == -1)
	{
		free( (char *)runBuffer );
		ImReturnBinError( );
	}
...
//	int          n;			/* Counter			*/
	 ...
	 *  Write out the buffer.
	 */
	if ( ImBinWrite( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, x * y * 4 ) == -1)
	{
		free( (char *)runBuffer );
		ImReturnBinError( );
	}
...
#endif
{
	...
	if ( ImBinWrite( ioType, fd, fp, offsetTable, IM_INT_TYPE, 4, y*z ) == -1 )
	{
		free( (char *)offsetTable );
		ImReturnBinError( );
	}
	if ( ImBinWrite( ioType, fd, fp, sizeTable, IM_INT_TYPE, 4, y*z ) == -1 )
	{
		free( (char *)offsetTable );
		free( (char *)sizeTable );
		ImReturnBinError( );
	...
	if ( ioType & IMFILEIOFD )
		status = _lseek( fd, IMRGBHEADERSIZE, L_SET );
	else
