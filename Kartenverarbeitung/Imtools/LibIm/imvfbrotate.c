/**
** $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imvfbrotate.c,v 1.10 1995/06/30 22:12:11 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imvfbrotate.c,v 1.10 1995/06/30 22:12:11 bduggan Exp $ "
...
#endif
{
	...
	*/
	degree *= 2.0;
	degree -= ((int)(degree / 360.0 + .5)) * 360.0;
	...
	*/
	if ( degree > -M_PI ) {
	double dT = ImVfbQWidth(srcVfb) - tan(degree/2)*ImVfbQHeight( srcVfb );

		dw	   = (int)( (dT > 0) ? dT + 0.5 : dT - 0.5); 
		xDif   = 0;
...
#endif
{
	...
	*/
	if ( degree>-M_PI ) {
	double dT = ImVfbQHeight(srcVfb) - sin(degree)*ImVfbQWidth( srcVfb );

		dh	   = (int)(( (dT > 0) ? dT + 0.5 : dT - 0.5 )); 
...
#endif
{
	...
	*/
	...
	iHeight = ImVfbQHeight(dummyVfb);
	if (ImVfbQHeight(dstVfb) < iHeight)
		iHeight = ImVfbQHeight(dstVfb);
	
	iWidth = ImVfbQWidth(dummyVfb);
	if (ImVfbQWidth(dstVfb) < iWidth)
		iWidth = ImVfbQWidth(dstVfb);
