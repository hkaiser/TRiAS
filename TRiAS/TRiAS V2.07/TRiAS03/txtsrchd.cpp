...
ResourceFile RF (g_pDLLName);
	...
	if (m_SelRegExpr.LoadBitmaps (ResID (IDB_ARROW_DOWN, &RF), 
...
// Optionen initialisieren
	...
	int iIndex = m_cbFindWhat.AddString (m_pStatus -> m_strFindWhat.c_str());
		...
		if (iIndex >= 0) m_cbFindWhat.ChangeCurrentIndex(iIndex);
...
///////////////////////////////////////////////////////////////////////////////
// Subclasses Edit der ComboBox
CComboEditWnd::CComboEditWnd (pWindow pW, ComboBox &rCB)
	: ControlWindow (new SingleLineEdit (pW, GetDlgItem (rCB.Handle(), 1001)))
{
}

CComboEditWnd::~CComboEditWnd (void)
{
	if (GetControl()) delete ((SingleLineEdit *)GetControl());
}

void CComboEditWnd::FocusChange (FocusChangeEvt e)
{
	if (!e.gotFocus())
...
Point Pt = m_SelRegExpr.GetPosition();
...
BOOL fResult = TrackPopupMenu (m_hMenu, 0, Pt.X(), Pt.Y(), 0, hWnd, NULL);
...
WDMatchString Match;
	...
	try {
		...
		CComObject<CMatchRegExprExakt> *pMatch = NULL;

			THROW_FAILED_HRESULT(CComObject<CMatchRegExprExakt>::CreateInstance(&pMatch));
			pMatch -> SetFullMatch (!m_pStatus -> m_fPartialMatch);
			Match = pMatch;
		} 
		...
		CComObject<CMatchFuzzy> *pMatch = NULL;

			THROW_FAILED_HRESULT(CComObject<CMatchFuzzy>::CreateInstance(&pMatch));
			Match = pMatch;
		} 
		else {
		// Text vollständig wiederfinden (bzw. Textanfang)
		CComObject<CMatchPlainTextExakt> *pMatch = NULL;

			THROW_FAILED_HRESULT(CComObject<CMatchPlainTextExakt>::CreateInstance(&pMatch));
			pMatch -> SetFullMatch(!m_pStatus -> m_fPartialMatch);
			pMatch -> SetExistanceOnly(m_pStatus -> m_fExistsOnly);
			pMatch -> SetMatchBOB(m_pStatus -> m_fTestBOB);
			Match = pMatch;
		}
	...
	CComBSTR bstrMatch (m_pStatus -> m_strFindWhat.c_str());
		...
		THROW_FAILED_HRESULT(Match -> put_MatchCase (!m_pStatus -> m_fCapital));
		THROW_FAILED_HRESULT(Match -> put_NegateMatch (m_pStatus -> m_fNegate));
...
public:
	CSearch (DMatchString *pIMatch, IObjectProperty *pIObjProp) : 
			m_Match(pIMatch), m_ObjProp(pIObjProp) 
	...
	HRESULT hr = m_ObjProp -> Eval (lONr, m_pBuffer, SEARCH_BUFFERSIZE-1, NULL);
		...
		VARIANT_BOOL fNegate = VARIANT_FALSE;

			hr = m_Match -> get_NegateMatch (&fNegate);
			return fNegate ? false : true;	// gematched/nicht gematched
		}
		if (FAILED(hr)) throw hr;	// sonstiger Fehler, nicht wieder rufen
