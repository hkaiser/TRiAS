...
#include "ctfx.hxx"		// MEIN (!!!) modifizierter Header
#else
...
class _GEOMETR_ENTRY CoordTransXBase 
{
public:
	...
	virtual double CoordTransAspectRatio (ObjRechteck &) = 0;
	...
	virtual bool Scale (const double &, const double &, EPunkt &) = 0;
	virtual bool Scale (const double &, const double &, KoOrd *, KoOrd *) = 0;
	virtual bool UnScale (const EPunkt &, double *, double *) = 0;
	virtual bool UnScale (const KoOrd &, const KoOrd &, double *, double *) = 0;

	virtual bool Metric (const EPunkt &, double *, double *) = 0;	// metrische Koordinaten erzeugen
	virtual bool Metric (const KoOrd &, const KoOrd &, double *, double *) = 0;
	virtual bool Native (const double &, const double &, EPunkt &) = 0;	// aus metrischen Koordinaten konvertieren
...
// Gebrauch mit CoTaskMemFree(pData) wieder freigegeben werden !
	virtual bool GetMetricParam (const EPunkt &, LPVOID *) = 0;
...
// sondern an die folgenden Funktionen übergeben (#HK980305)
	...
	virtual bool MetricEx (const EPunkt &, double *, double *, LPVOID) = 0;		// metrische Koordinaten erzeugen
	virtual bool MetricEx (KoOrd, KoOrd, double *, double *, LPVOID) = 0;
	virtual bool NativeEx (const double &, const double &, EPunkt &, LPVOID) = 0;	// aus metrischen Koordinaten konvertieren
...
class _GEOMETR_ENTRY CCoordTransServiceObsoleteX :
...
class _GEOMETR_ENTRY CCoordTransServiceX 
#if defined(_USE_LEGACY_CODE)
	: public CoordTransXBase
#endif // _USE_LEGACY_CODE
{
