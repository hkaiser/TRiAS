...
DWORD isCustomized (long lIdent, DWORD dwData)
{
	TX_ASSERT(0L != dwData);
...
void CVisInfoPage::ButtonClick (ControlEvt e) 
{
	...
	case IDC_SETSCALE_UPPER:
		...
		int iRow = m_lvIdentList.GetLastItemFocus();
			...
			if (iRow >= 0)
				m_lvIdentList.PostEvent (WM_SHOWCOMBO, iRow, UPPER_COLUMN);
		}
		break;
	...
	case IDC_SETSCALE_LOWER:
		...
		int iRow = m_lvIdentList.GetLastItemFocus();
			...
			if (iRow >= 0)
				m_lvIdentList.PostEvent (WM_SHOWCOMBO, iRow, LOWER_COLUMN);
		}
		break;
	}
}
...
DWORD iBits = VTypToBits(m_rgVTyp) | EnumVirtualData;		// virtuelle Objektklassen berücksichtigen
	...
	m_lvIdentList -> InitControl (iBits, SELIDMODE_SINGLETYPE, 
			isCustomized, CmpEntries, reinterpret_cast<DWORD>(this));
...
// Merkmal am Ident lesen
	TX_ASSERT(-1 != lMCode);
	if (!ReadTextMerkmal (TT_Identifikator, lIdent, lMCode, cbBuffer, _MAX_PATH))
		return false;
...
// irgend ein Fehler
	...
	WriteTextMerkmal (TT_Identifikator, lIdent, lMCode, NULL);	// löschen

	return false;		// nichts gefunden
}

bool DatenBasisObjekt::SetIdentScaleData (LPCSTR pcView, long lIdent, 
...
long lMCode = -1;
	...
	if (0 == lUpper && 0 == lLower) 	// evtl. vorherige Einstellungen löschen
		return WriteTextMerkmal (TT_Identifikator, lIdent, lMCode, NULL);

// jetzt Merkmal an Ident schreiben
char cbBuffer[64];

	wsprintf (cbBuffer, "%ld,%ld", lUpper, lLower);
	return WriteTextMerkmal (TT_Identifikator, lIdent, lMCode, cbBuffer);
}
...
string strUpper (m_lvIdentList -> GetItemText (iIndex, UPPER_COLUMN));
string strLower (m_lvIdentList -> GetItemText (iIndex, LOWER_COLUMN));
...
int iPrefix = strlen(g_cbScalePrefix);
	...
	if (0 < strUpper.length())
		rlUpper = myatol (strUpper.c_str() + iPrefix);
	else
		rlUpper = 0L;

	if (0 < strLower.length())
		rlLower = myatol (strLower.c_str() + iPrefix);
	else
		rlLower = 0L;
	return true;
}
...
ErrCode DatenBasisObjekt::ChangeVisInfo (
	long lIdent, short iVTyp, VISINFO &rVI, char *pcView, short fBits, bool fForce)
{
...
ErrCode DatenBasisObjekt::ChangeVisInfo (
	CIdentifikator &rId, VisType iVTyp, VisInfo *pVI, char *pcView, short fBits)
{
...
CStringList lstItems;
	...
	int iInit = FindIndex (iUpperScale, index, colnum);
...
int CSelectIdentList::FindIndex (int iFirst, int iRow, int iCol)
{
...
ComboBox* CSelectIdentList::ShowInPlaceList
	(int nItem, int nCol, CStringList &lstItems, int nSel)
{
...
Dimension dim (rect.right-rect.left, rect.bottom-rect.top);
	...
	m_pList -> Create (nItem, nCol, lstItems, nSel);
...
ResString strLower (IDS_LOWERSCALE, 32);
	...
	GetListCtrl() -> InsertColumn (UPPER_COLUMN, strUpper, LVCFMT_LEFT, -1, UPPER_COLUMN);
	GetListCtrl() -> InsertColumn (LOWER_COLUMN, strLower, LVCFMT_LEFT, -1, LOWER_COLUMN);
