...
BOOL CGetFolderControl::UnsubclassEdit()
{
	if (!IsWindow(m_hWnd)) {
		ASSERT(FALSE);
		...
		BROWSEINFO bi;
			...
			TCHAR sPath[_MAX_PATH];
				...
				if (SHGetPathFromIDList(pItemIDList, sPath)) {
					...
					SetWindowText(sPath);
					GetParent()->PostMessage(WM_DIRSELECTED, GetDlgCtrlID());
				}
...
void CGetFolderControl::OnShowWindow(BOOL bShow, UINT nStatus)
{
...
HWND hWndCtrl = pDX->PrepareEditCtrl(nIDC);
	...
	if (rCGetFolderControl.m_hWnd == NULL)    // not subclassed yet
	{
		if (!rCGetFolderControl.SubclassEdit(hWndCtrl))
		{
			ASSERT(FALSE);      // possibly trying to subclass twice ?
