/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/impix.c,v 1.26 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER        "    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/impix.c,v 1.26 1995/06/29 00:28:04 bduggan Exp $"
...
{
	{ IM_SHORT_TYPE, 2, 1 },		/* pix_width			*/
	{ IM_SHORT_TYPE, 2, 1 },		/* pix_height			*/
	{ IM_SHORT_TYPE, 2, 1 },		/* pix_dummy1			*/
	{ IM_SHORT_TYPE, 2, 1 },		/* pix_dummy1			*/
	{ IM_SHORT_TYPE, 2, 1 },		/* pix_depth			*/
	{ 0, 0, 0 }
};
...
//	long             seekPtr;	/* File seek pointer		*/
	 ...
	 */
	if ( (nRead = ImBinRead( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, 4*x*y )) == -1 )
	{
		free( (char *)runBuffer );
		ImReturnBinError( );
	}
...
//	int             n;		/* Counter			*/
	...
	for ( y = 0; y < header.pix_height; y++ )
	{
		 ...
		 */
		if ( ImBinWrite( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, nRuns * 4 ) == -1 )
		{
			free( (char *)runBuffer );
			ImReturnBinError( );
		}
	...
	*runBuffer = 0;			/* Terminate the runs		*/
	if ( ImBinWrite( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, 4 ) == -1 )
	{
		free( (char *)runBuffer );
		ImReturnBinError( );
	}
...
//	long               seekPtr;	/* File seek pointer		*/
	...
	if ( (nRead = ImBinRead( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, 2*x*y )) == -1 )
	{
		free( (char *)runBuffer );
		ImReturnBinError( );
	}
...
//	int               n;		/* Counter			*/
	...
	for ( y = 0; y < header.pix_height; y++ )
	{
		 ...
		 *  Write out the run buffer.
		 */
		if ( ImBinWrite( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, nRuns * 2 ) == -1 )
		{
			free( (char *)runBuffer );
			ImReturnBinError( );
		}
	 ...
	 */
	*runBuffer = 0;			/* Terminate the runs		*/
	if ( ImBinWrite( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, 2 ) == -1 )
	{
		free( (char *)runBuffer );
		ImReturnBinError( );
	}
