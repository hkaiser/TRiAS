...
BEGIN_OBJECT_MAP(ObjectMap)
	...
	OBJECT_ENTRY(CLSID_ResetVisAddEntryCB, CResetVisAddEntryCB)
	OBJECT_ENTRY(CLSID_ResetHideAddEntryCB, CResetHideAddEntryCB)
END_OBJECT_MAP()
...
template<class T, class D, const GUID *piid>
STDMETHODIMP CVisInfo<T, D, piid>::get_Locatable (int *pfFlags)
{
	TEST_E_POINTER("CVisInfo<T, D, piid>::get_Locatable", pfFlags);

	*pfFlags = GetVisInfo()->IsLocatable();
	return NOERROR;
}

template<class T, class D, const GUID *piid>
STDMETHODIMP CVisInfo<T, D, piid>::put_Locatable (int fFlags)
{
	GetVisInfo()->SetLocatable (fFlags);
	Changed() |= DO_OthersChanged;
	return NOERROR;
}

template<class T, class D, const GUID *piid>
STDMETHODIMP CVisInfo<T, D, piid>::get_Visible (int *pfFlags)
{
	TEST_E_POINTER("CVisInfo<T, D, piid>::get_Visible", pfFlags);

	*pfFlags = GetVisInfo()->IsVisible();
	return NOERROR;
}

template<class T, class D, const GUID *piid>
STDMETHODIMP CVisInfo<T, D, piid>::put_Visible (int fFlags)
{
	GetVisInfo()->SetVisible (fFlags);
	Changed() |= DO_OthersChanged;
	return NOERROR;
}

template<class T, class D, const GUID *piid>
STDMETHODIMP CVisInfo<T, D, piid>::get_GeometryName (BSTR *pbstrDesc)
{
	USES_CONVERSION;

	if (NULL == pbstrDesc) return E_POINTER;
	*pbstrDesc = SysAllocString(A2OLE(GetVisInfo()->GeometryName()));

	return NOERROR;
}

template<class T, class D, const GUID *piid>
STDMETHODIMP CVisInfo<T, D, piid>::put_GeometryName (BSTR bstrDesc)
{
	USES_CONVERSION;

	GetVisInfo()->SetGeometryName(OLE2A(bstrDesc));
	Changed() |= DO_OthersChanged;
	return NOERROR;
}

template<class T, class D, const GUID *piid>
STDMETHODIMP CVisInfo<T, D, piid>::get_HasInfos (VARIANT_BOOL *pHasInfos)
{
	if (NULL == pHasInfos)
		return E_POINTER;
	*pHasInfos = VARIANT_FALSE;

	if (!!m_bstrName && !!m_bstrInfo)
		*pHasInfos = VARIANT_TRUE;

	return S_OK;
}

template<class T, class D, const GUID *piid>
STDMETHODIMP CVisInfo<T, D, piid>::get_ModuleInfo (BSTR *pbstrName)
{
	if (NULL == pbstrName)
		return E_POINTER;
	*pbstrName = NULL;
...
template<class T, class D, const GUID *piid>
STDMETHODIMP CVisInfo<T, D, piid>::put_ModuleInfo(BSTR bstrName)
{
	m_bstrName = bstrName;
	Changed() |= DO_OthersChanged;
	return S_OK;
...
template<class T, class D, const GUID *piid>
STDMETHODIMP CVisInfo<T, D, piid>::get_ExtendedInfo(BSTR *pbstrInfo)
{
	if (NULL == pbstrInfo)
		return E_POINTER;
	*pbstrInfo = NULL;

CComBSTR bstr (m_bstrInfo);
...
template<class T, class D, const GUID *piid>
STDMETHODIMP CVisInfo<T, D, piid>::put_ExtendedInfo(BSTR bstrInfo)
{
	m_bstrInfo = bstrInfo;
	Changed() |= DO_OthersChanged;
	return S_OK;
...
// für objektbezogene Visualisierung den objektbezogenen Anteil merken
	...
	HANDLEVISINFOEX *pHVIEx = (HANDLEVISINFOEX *)pHVI;
		...
		WExtendedObjectVisInfo VisEx;

			RETURN_FAILED_HRESULT((*ppIVis)->QueryInterface(VisEx.ppi()));
			RETURN_FAILED_HRESULT(VisEx -> put_ModuleInfo(CComBSTR(pHVIEx->m_pModule)));
			RETURN_FAILED_HRESULT(VisEx -> put_ExtendedInfo(CComBSTR(pHVIEx->m_pInfo)));
		}
	}
...
// #HK020906
	RETURN_FAILED_HRESULT(pIVis -> get_Visible(&iData));
	pVI -> m_fVisible = iData ? true : false;
	RETURN_FAILED_HRESULT(pIVis -> get_Locatable(&iData));
	pVI -> m_fLocatable = iData ? true : false;
...
inline 
char *CopyString (char const *pT)
{
char *pR = NULL;

	ATLTRY(pR = new char [strlen(pT)+1]);
...
HANDLEVISINFO *pHVI = NULL;
	...
	COM_TRY {
	// nachsehen, ob HANDLEVISINFOEX gebraucht wird
	WExtendedObjectVisInfo ExtVI;
	CComBool fHasInfos;

		if (SUCCEEDED(pIVisInfo -> QueryInterface(ExtVI.ppi())))
		{
			THROW_FAILED_HRESULT(ExtVI -> get_HasInfos(&fHasInfos));
		}

	// HANDLEVISINFO erzeugen
		pHVI = ctHANDLEVISINFO(VTypeFromVISINFOTYPE(rgType), fHasInfos);
	...
	int fFlags = DO_VisInfoChanged;
		...
		if (fHasInfos) {
		CComBSTR bstrName, bstrInfo;

			THROW_FAILED_HRESULT(ExtVI -> get_ModuleInfo(&bstrName));
			THROW_FAILED_HRESULT(ExtVI -> get_ExtendedInfo(&bstrInfo));

			USES_CONVERSION;

		HANDLEVISINFOEX *pHVIEx = (HANDLEVISINFOEX *)pHVI;

			pHVIEx -> m_pModule = CopyString(OLE2A(bstrName));
			pHVIEx -> m_pInfo = CopyString(OLE2A(bstrInfo));

			pHVI -> m_fFlags |= DT_WillDrawComplete;
		}
...
char cbBuffer[VISNAMESIZE+1];
	...
	RETURN_FAILED_HRESULT(LoadChunk (pStm, cbBuffer, VISNAMESIZE+1));
	pVI -> SetVisName (cbBuffer);
...
// neu ab V3.00
	RETURN_FAILED_HRESULT(LoadLong (pStm, lData));
	pVI -> SetLocatable (lData ? true : false);
	RETURN_FAILED_HRESULT(LoadLong (pStm, lData));
	pVI -> SetVisible (lData ? true : false);

	RETURN_FAILED_HRESULT(LoadChunk (pStm, cbBuffer, VISNAMESIZE+1));
	pVI -> SetGeometryName (cbBuffer);
...
STDMETHODIMP CVisInfo<T, D, piid>::Save (LPSTREAM pStm, BOOL fClearDirty, VisInfo *pVI)
{
	...
	RETURN_FAILED_HRESULT(SaveChunk (pStm, const_cast<char *>(pVI -> VisName()), VISNAMESIZE+1));
	RETURN_FAILED_HRESULT(SaveLong (pStm, (long &)pVI -> PrimColor()));

// neu ab V3.00
LONG lData = pVI -> IsLocatable();

	RETURN_FAILED_HRESULT(SaveLong (pStm, lData));
	lData = pVI -> IsVisible();
	RETURN_FAILED_HRESULT(SaveLong (pStm, lData));

char cbBuffer[VISNAMESIZE+1];

	strcpy (cbBuffer, pVI -> GeometryName());
	RETURN_FAILED_HRESULT(SaveChunk (pStm, cbBuffer, VISNAMESIZE+1));
...
STDMETHODIMP CVisInfo<T, D, piid>::GetSizeMax (ULARGE_INTEGER *pCbSize)
{
	...
	pCbSize -> QuadPart += 2*sizeof(long) + (VISNAMESIZE+1);	// neu ab V3.00
	return S_OK;
}
...
long lSignature = 0L;
	...
	if (VISINFO_SIGNATURE != lSignature)
		RETURN_HRESULT(TRIASDB_E_BADOBJECTSIGNATURE);

long lVersion = 0L;

	RETURN_FAILED_HRESULT(LoadLong (pStm, lVersion));			// VersionInfo
	if ((lVersion & ~VISINFO_VERSION_MINOR_MASK) > LASTKNOWN_VISINFO_VERSION)
		RETURN_HRESULT(TRIASDB_W_NEWERVIEWITEM);
...
{
	RETURN_FAILED_HRESULT(SaveLong (pStm, VISINFO_SIGNATURE));
	RETURN_FAILED_HRESULT(SaveLong (pStm, LASTKNOWN_VISINFO_VERSION));
...
namespace point_symbol {

	WORD *GetOffsetTab (ResID resID, DWORD *pdwSize)
	{
		_ASSERTE(NULL != pdwSize);
		*pdwSize = NULL;

	// Finden der Resource dieser Instanz
	HRSRC hRes = FindResource((HMODULE)resID.Handle(), MAKEINTRESOURCE(uint(resID)), "POINTSYMBOL");

		if (NULL == hRes) return NULL;

	// Resource in Memory laden
	HGLOBAL	hMem = LoadResource((HMODULE)resID.Handle(), hRes);

		if (NULL == hMem) return NULL;

	// lock it, get pointer to first VOID
	BYTE *lpMem = (BYTE *)LockResource(hMem);
	...
	DWORD dwSize = SizeofResource((HMODULE)resID.Handle(), hRes);
	WORD *pTable = (WORD *)new BYTE[dwSize];
	
		if (NULL != pTable) {
			memcpy (pTable, lpMem, dwSize);
			*pdwSize = dwSize;
		}
	...
	{
	private:
		typedef map<WORD, WORD, less<WORD> > CPointIds;
	...
	public:
		...
		WORD GetCorrectId (WORD wOldId)
		{
			if (!m_fIsValid && !InitTables()) 
				return wOldId;

		CPointIds::iterator it = m_PtIds.find(wOldId);

			return (it != m_PtIds.end()) ? (*it).second : wOldId;
		}
	...
	WORD *pNewTable = NULL;
		...
		DWORD dwNewSize = 0L;
			...
			pOldTable = GetOffsetTab (ResID(IDR_OFFSETTABOBSOLETE, &RF), &dwOldSize);
			pNewTable = GetOffsetTab (ResID(IDR_OFFSETTAB, &RF), &dwNewSize);

			_ASSERTE(dwOldSize == dwNewSize);
			if (NULL != pOldTable && NULL != pNewTable) {
				for (int i = 0; i < dwNewSize / sizeof(WORD); ++i) 
					m_PtIds.insert(CPointIds::value_type(pOldTable[i], pNewTable[i]));

			// selbst wenn die falschen Ids nicht eindeutig waren, man kann es ja schließlich
			// nicht allen Recht machen ...
//					_ASSERTE(m_PtIds.size() == (dwNewSize / sizeof(WORD)));	// oder gibt es etwa doppelte alte Ids?
				m_fIsValid = true;
			}
		} 
		catch(...) {
		...
		DELETE_OBJ(pOldTable);
		DELETE_OBJ(pNewTable);

		return m_fIsValid;
	}

// mapping der 'alten' (falschen) auf die 'neuen' (richtigen) PunktIDs
	CPointIdMap PointIds;
}
...
long lSignature = 0L;
	...
	RETURN_FAILED_HRESULT(LoadLong (pStm, lSignature));			// Signatur
	if (PVISINFO_SIGNATURE != lSignature)
		RETURN_HRESULT(TRIASDB_E_BADOBJECTSIGNATURE);

long lVersion = 0L;

	RETURN_FAILED_HRESULT(LoadLong (pStm, lVersion));			// VersionInfo
	if ((lVersion & ~PVISINFO_VERSION_MINOR_MASK) > LASTKNOWN_PVISINFO_VERSION)
		RETURN_HRESULT(TRIASDB_W_NEWERVIEWITEM);
...
#if !defined(_DEBUG)
// Das Problem besteht darin, daß nur die Release-Variante von TRiAS bis zur Version
// 3.17.1108 eine zermatschte PunktId-Tabelle hatte, die Punktsymbole also mit
// falschem Id gespeichert worden sind. 
// Daher wird nur in der Release-Variante der PunktId richtiggestellt, was allerdings
// vorraussetzt, das niemand eine alte, mit der Release-Variante geschriebene
// Datenbank mit einem neueren Debug-TRiAS öffnet ... (#HK000927)
	if (lVersion < PVISINFO_VERSION_101)		// ggf. gelesenes Punktsymbol korrigieren
		m_PVI.Style() = point_symbol::PointIds.GetCorrectId(m_PVI.Style());
...
{
	RETURN_FAILED_HRESULT(SaveLong (pStm, PVISINFO_SIGNATURE));
	RETURN_FAILED_HRESULT(SaveLong (pStm, LASTKNOWN_PVISINFO_VERSION));
...
{
	RETURN_FAILED_HRESULT(visinfo_t::GetSizeMax (pCbSize));
	pCbSize -> QuadPart += 3*sizeof(long) + sizeof(short) + 2*sizeof(long);
	return S_OK;
}
...
long lSignature = 0L;
	...
	RETURN_FAILED_HRESULT(LoadLong (pStm, lSignature));			// Signatur
	if (LVISINFO_SIGNATURE != lSignature)
		RETURN_HRESULT(TRIASDB_E_BADOBJECTSIGNATURE);

long lVersion = 0L;

	RETURN_FAILED_HRESULT(LoadLong (pStm, lVersion));			// VersionInfo
	if ((lVersion & ~LVISINFO_VERSION_MINOR_MASK) > LASTKNOWN_LVISINFO_VERSION)
		RETURN_HRESULT(TRIASDB_W_NEWERVIEWITEM);
...
{
	RETURN_FAILED_HRESULT(SaveLong (pStm, LVISINFO_SIGNATURE));
	RETURN_FAILED_HRESULT(SaveLong (pStm, LASTKNOWN_LVISINFO_VERSION));
...
{
	RETURN_FAILED_HRESULT(visinfo_t::GetSizeMax (pCbSize));
	pCbSize -> QuadPart += sizeof(short) + 2*sizeof(long);
	return S_OK;
}
...
long lSignature = 0L;
	...
	RETURN_FAILED_HRESULT(LoadLong (pStm, lSignature));			// Signatur
	if (FVISINFO_SIGNATURE != lSignature)
		RETURN_HRESULT(TRIASDB_E_BADOBJECTSIGNATURE);

long lVersion = 0L;

	RETURN_FAILED_HRESULT(LoadLong (pStm, lVersion));			// VersionInfo
	if ((lVersion & ~LVISINFO_VERSION_MINOR_MASK) > LASTKNOWN_FVISINFO_VERSION)
		RETURN_HRESULT(TRIASDB_W_NEWERVIEWITEM);

	RETURN_FAILED_HRESULT(visinfo_t::Load (pStm, &m_FVI));
	RETURN_FAILED_HRESULT(LoadLong (pStm, (long &)m_FVI.SecColor()));
	if (lVersion >= FVISINFO_VERSION_101) {
	long lData = 0;

		RETURN_FAILED_HRESULT(LoadLong (pStm, lData));
		m_FVI.SetAttrEx(lData);
		RETURN_FAILED_HRESULT(LoadLong (pStm, lData));
		m_FVI.SetWidth(lData);
	}
	return S_OK;
}
...
{
	RETURN_FAILED_HRESULT(SaveLong (pStm, FVISINFO_SIGNATURE));
	RETURN_FAILED_HRESULT(SaveLong (pStm, LASTKNOWN_FVISINFO_VERSION));

	RETURN_FAILED_HRESULT(visinfo_t::Save (pStm, fClearDirty, &m_FVI));
	RETURN_FAILED_HRESULT(SaveLong (pStm, (long &)m_FVI.SecColor()));
	RETURN_FAILED_HRESULT(SaveLong (pStm, (long)m_FVI.AttrEx()));
	RETURN_FAILED_HRESULT(SaveLong (pStm, (long)m_FVI.Width()));
	return S_OK;
}
...
{
	RETURN_FAILED_HRESULT(visinfo_t::GetSizeMax (pCbSize));
	pCbSize -> QuadPart += sizeof(long) + 2*sizeof(long);
	return S_OK;
}
...
long lSignature = 0L;
	...
	RETURN_FAILED_HRESULT(LoadLong (pStm, lSignature));			// Signatur
	if (TVISINFO_SIGNATURE != lSignature)
		RETURN_HRESULT(TRIASDB_E_BADOBJECTSIGNATURE);

long lVersion = 0L;

	RETURN_FAILED_HRESULT(LoadLong (pStm, lVersion));			// VersionInfo
	if ((lVersion & ~TVISINFO_VERSION_MINOR_MASK) > LASTKNOWN_TVISINFO_VERSION)
		RETURN_HRESULT(TRIASDB_W_NEWERVIEWITEM);
...
{
	RETURN_FAILED_HRESULT(SaveLong (pStm, TVISINFO_SIGNATURE));
	RETURN_FAILED_HRESULT(SaveLong (pStm, LASTKNOWN_TVISINFO_VERSION));
...
{
	RETURN_FAILED_HRESULT(visinfo_t::GetSizeMax (pCbSize));
	pCbSize -> QuadPart += 2*sizeof(long) + 3*sizeof(short) + 2*sizeof(long);
	return S_OK;
}
...
// neu ab V3.00
	MERGEVIS(DO_OthersChanged, int, Visible, *ppIVisInfo);
	MERGEVIS(DO_OthersChanged, int, Locatable, *ppIVisInfo);
	MERGEVIS(DO_OthersChanged, CComBSTR, GeometryName, *ppIVisInfo);
