...
inline double EvalSatKoeff (double dGrau, double dColor, double dMax, 
							HIGHLOW &rrgStatus)
{
//	TX_ASSERT(0.0 > dColor && dColor < dMax);		// darf nicht gesättigt sein
...
inline bool IsSaturated (double dColor, double dMax)
{
	return (0.0 == dColor || dMax == dColor);
}
...
{
//	TX_ASSERT(0 <= iRed && 255 >= iRed);
//	TX_ASSERT(0 <= iGreen && 255 >= iGreen);
//	TX_ASSERT(0 <= iBlue && 255 >= iBlue);
...
//	TX_ASSERT(rgTarget != TARGETCOLOR_UNKNOWN);
//	TX_ASSERT(rgStatus != HIGHLOW_UNKNOWN);
...
//	TX_ASSERT(false);		// not reachable code
	return 0.0;
}
...
{
//	TX_ASSERT(0 <= iHue && iHue <= 360);
//	TX_ASSERT(0 <= iSat && iSat <= 100);
//	TX_ASSERT(0 <= iBright && iBright <= 100);
