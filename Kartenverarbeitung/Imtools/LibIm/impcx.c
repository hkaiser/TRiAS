/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/impcx.c,v 1.12 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER        "    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/impcx.c,v 1.12 1995/06/29 00:28:04 bduggan Exp $"
...
{
	/* pcx_manufacturer */		IM_UCHAR_TYPE, 1, 1,
	/* pcx_version */		IM_UCHAR_TYPE, 1, 1,
	/* pcx_encoding */		IM_UCHAR_TYPE, 1, 1,
	/* pcx_bitsPerPixel */		IM_UCHAR_TYPE, 1, 1,
	/* pcx_window */		IM_USHORT_TYPE, 2, 4,
	/* pcx_horizontalRes */		IM_USHORT_TYPE, 2, 1,
	/* pcx_verticalRes */		IM_USHORT_TYPE, 2, 1,
	/* pcx_clt */			IM_UCHAR_TYPE, 1, 48,
	/* pcx_reserved */		IM_UCHAR_TYPE, 1, 1,
	/* pcx_nPlanes */		IM_UCHAR_TYPE, 1, 1,
	/* pcx_bytesPerLine */		IM_USHORT_TYPE, 2, 1,
	/* pcx_cltType */		IM_USHORT_TYPE, 2, 1,
	/* pcx_filler */		IM_UCHAR_TYPE, 1, 58,
					    0, 0, 0,
};
...
#endif
{
	...
	else if ( cltAtEnd )
	{
		...
		n = ImBinRead( ioType, fd, fp, cltBuffer, IM_UCHAR_TYPE, 1, 769 );
	...
	if ( ImBinRead( ioType, fd, fp, encoded, IM_UCHAR_TYPE, 1, 2 * n ) == -1 )
	{
		free( (char *)encoded );
		free( (char *)decoded );
		if ( clt != IMCLTNULL )
...
#endif
{
	...
	for ( y = 0; y < height; y++ )
	{
		...
		if ( ImBinWrite( ioType, fd, fp, encoded, IM_UCHAR_TYPE, 1, n ) == -1 )
		{
			free( (char *)encoded );
			free( (char *)decoded );
			ImReturnBinError( );
...
#endif
{
	...
	*pColorBuf++ = 0x0C;			/* Yes, it's here	*/
	...
	/* Write it out.						*/
	if ( ImBinWrite( ioType, fd, fp, colorBuf, IM_UCHAR_TYPE, 1, n ) == -1 )
	{
		free( (char *)colorBuf );
		ImReturnBinError( );
	}
