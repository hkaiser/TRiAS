...
// Created: 23.04.2002 22:39:33 
//
// @doc
// @module InPlaceCombo.cpp | Definition of the <c CInPlaceCombo> class
...
static 
ComboBox::ComboStyle GetComboStyle (DWORD dwStyle)
{
	...
	_ASSERTE(false);		// falscher ComboStyle
	return ComboBox::Simple;
}

CInPlaceCombo::CInPlaceCombo (pWindow pW, CvID uiID, Point pt, Dimension dim, 
		DWORD dwStyle) : 
...
bool CInPlaceCombo::Create (int iItem, int iSubItem, CStringList &rlstItems, 
...
// alle Elemente zur Box hinzufügen
	...
	if (-1 != nSel) 
		...
		_ASSERTE(ComboBox::DropDownList == GetComboStyle(m_dwStyle));
...
void CInPlaceCombo::FocusChange (FocusChangeEvt e) 
{
	...
	HWND hParent = GetParent()->Handle(API_WINDOW_HWND);
		...
		GetParent()->GetParent()->ForwardEvent(WM_NOTIFY, GetDlgCtrlID(hParent), 
			(LPARAM)&dispinfo);
