/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imsoftimage.c,v 1.4 1995/06/30 22:10:16 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imsoftimage.c,v 1.4 1995/06/30 22:10:16 bduggan Exp $"
...
{
	{ IM_UCHAR_TYPE,  1, 4 },		/* si_magic	*/
	{ IM_FLOAT_TYPE,  4, 1 },		/* si_version	*/
	{ IM_CHAR_TYPE,   1, 80},		/* si_comments	*/
	{ IM_CHAR_TYPE,   1, 4},		/* si_id	*/
	{ IM_UINT_TYPE,   2, 1},		/* si_width	*/
	{ IM_UINT_TYPE,   2, 1},		/* si_height	*/
	{ IM_FLOAT_TYPE,  4, 1},		/* si_ratio	*/
	{ IM_UINT16_TYPE, 2, 1 },		/* si_fields	*/
	{ IM_UCHAR_TYPE,  1, 2 },		/* si_pad	*/
	{ 0, 0, 0 }
};
...
{
	{ IM_UCHAR_TYPE,  1, 1 },		/* si_last                      */
	{ IM_UCHAR_TYPE,  1, 1 },		/* si_size			*/
	{ IM_UCHAR_TYPE,  1, 1 },		/* si_compression		*/
	{ IM_UCHAR_TYPE,  1, 1 },		/* si_channels			*/
	{ 0, 0, 0 }
};
...
if( ImBinRead( ioType, fd, fp, buf, IM_UCHAR_TYPE, 1, numBytes)==-1)	\
	{ 							\
		ImReturnBinError( );				\
	}
...
#define imSoftDumpVarRun(bytesPerPixel, buffer, len)	\
	...
	if ( ImBinWrite( ioType, fd, fp, imSoftBuf, IM_UCHAR_TYPE, 1, 1)==-1) \
	{                                               \
		ImReturnBinError( );                    \
	}						\
...
#define imSoftDumpConstantRun3(r,g,b,len)			\
	...
	{							\
		...
		imSoftBuf[4] = g;                               \
		imSoftBuf[5] = b;                               \
		if ( ImBinWrite( ioType, fd, fp, imSoftBuf, IM_UCHAR_TYPE, 1, 6)==-1) \
		{                                               \
			ImReturnBinError( );                    \
		}                                               \
	}                                                       \
	...
	{                                                       \
		...
		imSoftBuf[2] = g;                               \
		imSoftBuf[3] = b;                               \
		if ( ImBinWrite( ioType, fd, fp, imSoftBuf, IM_UCHAR_TYPE, 1, 4)==-1) \
		{                                               \
			ImReturnBinError( );                    \
		}                                               \
	}
...
#define imSoftDumpConstantRun1(a,len)				\
	...
	{                                                       \
		...
		imSoftBuf[0] |= 0x80;				\
		imSoftBuf[1] = a;                               \
		if ( ImBinWrite( ioType, fd, fp, imSoftBuf, IM_UCHAR_TYPE, 1, 2)==-1) \
		{                                               \
			ImReturnBinError( );                    \
		}                                               \
	}
...
//	imSoftHeaderInfo  header;	/* file header			 */
	...
	for (i = 0;i < ImVfbQHeight( vfb ) ; i++)
	{
		...
		if ( ImBinWrite( ioType, fd, fp, buffer, IM_UCHAR_TYPE, 1, 3 * ImVfbQWidth(vfb))==-1) 
		{                                               
			ImReturnBinError( );                   
		}                                             
	}
...
//	imSoftHeaderInfo  header;	/* file header			 */
	...
	for (i = 0;i < ImVfbQHeight( vfb ) ; i++)
	{
		...
		/* Write alpha */
		if ( ImBinWrite( ioType, fd, fp, alphaBuffer, IM_UCHAR_TYPE, 1,  ImVfbQWidth(vfb))==-1) 
		{                                               
			ImReturnBinError( );                   
		}
	}
