...
typedef IConnectionPointContainerImpl<class CTRiASToolBarObj> CComToolBarCPC;
#else
...
#if defined(__ATL_20_USED__)
	public CProxyITRiASBarNotifySink<CTRiASToolBarObj>,
#endif // __ATL_20_USED__
...
// Connections informieren/abfragen
	HRESULT OnRequestAddButtons (int iIndex, const TBBUTTON *pBttn, int iCnt)
		{ return Fire_OnRequestAddButtons(this, iIndex, pBttn, iCnt); }
	HRESULT OnAddedButtons (int iIndex, const TBBUTTON *pBttn, int iCnt)
		{ return Fire_OnAddedButtons(this, iIndex, pBttn, iCnt); }
	HRESULT OnRequestDeleteItem (UINT uiCtrlID, DWORD dwStyle)
...
public:
	DECLARE_AGGREGATABLE(CTRiASToolBarObj)
...
// IControlToolBar methods
	...
	STDMETHOD(CommandToIndex) (THIS_ int uiID, int *piIndex);
	STDMETHOD(DockBar) (THIS_ DOCKSTATE rgState, int iLeft, int iTop, int iRight, int iBottom);
	STDMETHOD(FloatBar) (THIS_ ALIGNMODE rgState, int iLeft, int iTop);
	STDMETHOD(get_IsFloating) (THIS_ VARIANT_BOOL *pfIsFloating);
