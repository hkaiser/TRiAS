/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/imtools/src/RCS/imgray.c,v 1.7 1995/06/29 01:05:35 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/imtools/src/RCS/imgray.c,v 1.7 1995/06/29 01:05:35 bduggan Exp $"

/**
 **  FILE
 **	imgray.c	-  Image File Format Conversion to grayscale
 **
 **  PROJECT
 **	IM		-  Image Manipulation Tools
 **
 **  DESCRIPTION
 **	imgray converts image files to grayscale.
 **
 **  PUBLIC CONTENTS
 **			d =defined constant
 **			f =function
 ...
 **	updated copyright
 **
 **	Revision 1.5  1995/06/15  19:17:04  bduggan
 **	took out some useless vars
 **
 **	Revision 1.4  1995/05/18  00:02:24  bduggan
 **	Added new read-write options routines.  Added option-specific help.
 **
 **	Revision 1.3  94/10/03  15:49:27  nadeau
 ...
 **	
 **	Revision 1.2  92/08/31  17:08:40  vle
 **	Updated copyright notice.
 **	
 **	Revision 1.1  91/10/03  13:19:54  nadeau
 **	Initial revision
 **	
 **/
...
"%command converts each input image to grayscale and saves it to an output\n\
...
Typical Invocations:\n\
    Convert an RGB Alias PIX image to a grayscale Sun rasterfile:\n\
        %command fullcolor.pix gray.ras\n\
    Convert a color Compuserve GIF image to a grayscale RGB Alias PIX image:\n\
        %command color.gif gray.pix\n\
\n\
Additional Help:\n\
...
Grayscale conversion:\n\
    Conversion from color to grayscale computes the gray value for each\n\
    pixel using the equation:\n\
\n\
        gray = 0.30 * red + 0.59 * green + 0.11 * blue\n\
\n\
    This is the same equation used by black-and-white televisions to display\n\
    color signals in black-and-white.\n\
";
...

{
	...
	ImVfb      *grayVfb;		/* New grayscale image		*/
	 ...
	 *	   grayscale.  Replace the input images with the new
	 *	   images.
	 */
	for ( i = 0; i < nInVfb; i++ )
	{
		 ...
		 *  but replace its image fields with a grayscale field.
		 */
		w = ImVfbQWidth( sourceVfb );
		h = ImVfbQHeight( sourceVfb );
		fields = ImVfbQFields( sourceVfb );
		grayVfb = ImVfbAlloc( w, h,
			(fields & ~IMVFBIMAGEMASK) | IMVFBGRAY );
		if ( grayVfb == IMVFBNULL )
		{
			ImPError( ImToolsProgram );
			fprintf( stderr, "%s: Couldn't create grayscale image for image %d of %d.\n",
		...
		if ( fields & ~IMVFBIMAGEMASK )
		{
			if ( ImVfbCopy( sourceVfb,	/* Use this VFB	*/
				0, 0,			/* Start here	*/
				w, h,			/* This much	*/
				(fields & ~IMVFBIMAGEMASK),/* This deep	*/
				grayVfb,		/* Put in VFB	*/
		...
		/*
		 *  Convert the source image to grayscale and save the
		 *  new pixels in the grayscale VFB.
		 */
		if ( ImToolsVerbose )
			fprintf( stderr, "%s: Converting image %d of %d to grayscale\n",
				ImToolsProgram,
				i + 1, nInVfb );
		ImVfbToGray( sourceVfb, grayVfb );
		...
		/*
		 *  Replace the source image with the grayscale image 
		 *  in the same data table.
		 */
		TagTableReplace( toolInTable,
			TagEntryQNthEntry( dataEntry ),
			TagEntryAlloc( "image vfb", IM_POINTER_TYPE, &grayVfb ) );
