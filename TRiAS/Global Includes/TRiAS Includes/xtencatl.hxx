		...
		if (NULL == g_pTE) 
		{
			...
			if (p != NULL)
			{
				...
				if (hRes == S_OK)
					hRes = p->QueryInterface(riid, ppv);
				if (hRes != S_OK) {
					delete p; p = NULL; g_pTE = NULL;
				}
			}
		} else 
			hRes = g_pTE->QueryInterface(riid, ppv);
		return hRes;
	}
};

#define DECLARE_NOT_AGGREGATABLE_SINGLE(x,p) public:\
	typedef CComCreator2< CComCreatorSingle< CComObject<x> >, CComFailCreator<CLASS_E_NOAGGREGATION> > _CreatorClass;
#define DECLARE_AGGREGATABLE_SINGLE(x,p) public:\
	typedef CComCreator2< CComCreatorSingle< CComObject<x> >, CComCreatorSingle< CComAggObject<x> > > _CreatorClass;
#define DECLARE_ONLY_AGGREGATABLE_SINGLE(x,p) public:\
	typedef CComCreator2< CComFailCreator<E_FAIL>, CComCreatorSingle< CComAggObject<x>> > _CreatorClass;
...
protected:	// spezifische Member einer TriasErweiterung
	...
	CWindow *	m_pMainWnd;			// HauptFenster
	CWindow *	m_pViewWnd;			// AnzeigeFenster
#else // __CV__
	CMenu *		m_pLoadedMenu;   	// gesamtes Menu
	CMenu *		m_pLoadedORMenu; 	// Menu für ObjektFenster
...
#elif defined(__WTL__)
	CWindow *MWind() { return m_pMainWnd; }
	CWindow *MVWind();
#else
...
#else
inline	CMenu *pExtMenu (void) const { return m_pLoadedMenu; }  // HauptMenu (MFC oder WTL)
