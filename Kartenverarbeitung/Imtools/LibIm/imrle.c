/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imrle.c,v 1.14 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imrle.c,v 1.14 1995/06/29 00:28:04 bduggan Exp $"
...
{
	IM_USHORT_TYPE, 2, 1,			/* rle_magic			*/
	IM_USHORT_TYPE, 2, 1,			/* rle_xpos			*/
	IM_USHORT_TYPE, 2, 1,			/* rle_ypos			*/
	IM_USHORT_TYPE, 2, 1,			/* rle_xsize			*/
	IM_USHORT_TYPE, 2, 1,			/* rle_ysize			*/
	IM_UCHAR_TYPE, 1, 1,			/* rle_flags			*/
	IM_UCHAR_TYPE, 1, 1,			/* rle_ncolors			*/
	IM_UCHAR_TYPE, 1, 1,			/* rle_pixelbits		*/
	IM_UCHAR_TYPE, 1, 1,			/* rle_ncmap			*/
	IM_UCHAR_TYPE, 1, 1,			/* rle_cmaplen			*/
	0, 0, 0
};
...
#endif
{
	...
	{
	case 0:
		if ( ImBinRead( ioType, fd, fp, bgColor, IM_UCHAR_TYPE, 1, 1 ) == -1 )
		{
			ImReturnBinError( );
		}
	...
	case 3:
		...
		if ( ImBinRead( ioType, fd, fp, bgColor, IM_UCHAR_TYPE, 1, 3 ) == -1 )
		{
			ImVfbFree( vfb );
			ImReturnBinError( );
		}
	...
	if ( header.rle_ncmap == 3 )
	{
		...
		ImMalloc( cmap, unsigned short *, sizeof( unsigned short ) * cmaplen * 3 );
		if ( ImBinRead( ioType, fd, fp, cmap, IM_USHORT_TYPE, 2, cmaplen * 3 )== -1 )
		{
			free( (char *)cmap );
			if ( vfb != IMVFBNULL )
				ImVfbFree( vfb );
	...
	if ( header.rle_flags & IMRLE_COMMENT )
	{
		...
		if ( ImBinRead( ioType, fd, fp, &len, IM_UINT_TYPE, 2, 1 )== -1 )
		{
			if ( vfb != IMVFBNULL )
				ImVfbFree( vfb );
			ImReturnBinError( );
		...
		else
		{
			...
			if ( ImBinRead( ioType, fd, fp, buffer, IM_UCHAR_TYPE, 1, len )== -1 )
			{
				free( (char *)buffer );
				if ( vfb != IMVFBNULL )
					ImVfbFree( vfb );
	...
	for ( ; ; )
	{
		switch ( (j = ImBinRead( ioType, fd, fp, buffer, IM_UCHAR_TYPE, 1, 2 )))
		{
		case -1:	/* Error				*/
			free( (char *)buffer );
			ImVfbFree( vfb );
		...
		{
			if ( ImBinRead( ioType, fd, fp, &operand, IM_UINT_TYPE, 2, 1 ) == -1 )
			{
				free( (char *)buffer );
				ImVfbFree( vfb );
				ImReturnBinError( );
		...
		case IMOPPixelData:
			...
			operand++;
			i = (operand + 1) & ~0x1;
			if ( ImBinRead( ioType, fd, fp, buffer, IM_UCHAR_TYPE, 1, i ) == -1 )
			{
				free( (char *)buffer );
				ImVfbFree( vfb );
				ImReturnBinError( );
...
{									\
	 ...
	 */								\
	buffer[1] = 0;					/* Pad	*/	\
	if ( ImBinWrite( ioType, fd, fp, buffer, IM_UCHAR_TYPE, 1, 2 ) == -1 )	\
	{								\
		free( (char *)buffer );					\
		ImReturnBinError();					\
	}								\
...
{									\
	...
	if ( ImBinWrite( ioType, fd, fp, buffer,			\
		IM_UCHAR_TYPE, 1, (litCount + 2) & ~0x1 ) == -1)		\
	{								\
		free( (char *)buffer );					\
		ImReturnBinError();					\
	}								\
...
//	ImVfbPtr  pLitPixel;		/* Literal pixel pointer	*/
	 ...
	 */
	i = 0;
	if ( ImBinWrite( ioType, fd, fp, &i, IM_INT_TYPE, 1, 1 ) == -1 )
	{
		ImReturnBinError();
	}
	...
	{
		k = cltLen + cltPadLen;
		ImMalloc( cmap, unsigned short *, sizeof( unsigned short ) * 3 * k );
		...
		if ( ImBinWrite( ioType, fd, fp, cmap, IM_USHORT_TYPE, 2, 3 * k ) == -1)
		{
			free( (char *)cmap );
			ImReturnBinError();
		}
	...
	for ( y = header.rle_ysize - 1; y >= 0; y-- )
	{
		...
		command[1] = 1;
		if ( ImBinWrite( ioType, fd, fp, command, IM_UCHAR_TYPE, 1, 2 ) == -1 )
		{
			ImReturnBinError();
		}
	}
