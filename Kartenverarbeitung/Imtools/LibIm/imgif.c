/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imgif.c,v 1.22 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imgif.c,v 1.22 1995/06/29 00:28:04 bduggan Exp $"
...
{
	{ IM_UCHAR_TYPE,  1, 6 },		/* gif_magic			*/
	{ IM_UINT16_TYPE, 2, 1 },		/* gif_width			*/
	{ IM_UINT16_TYPE, 2, 1 },		/* gif_height			*/
	{ IM_UCHAR_TYPE,  1, 1 },		/* gif_flags			*/
	{ IM_UCHAR_TYPE,  1, 1 },		/* gif_background		*/ 
	{ IM_UCHAR_TYPE,  1, 1 },		/* gif_aspect			*/
	{ 0, 0, 0 }
};
...
{
	{ IM_UINT16_TYPE, 2, 1 },		/* im_left        		*/
	{ IM_UINT16_TYPE, 2, 1 },		/* im_top         		*/
	{ IM_UINT16_TYPE, 2, 1 },		/* im_width       		*/
	{ IM_UINT16_TYPE, 2, 1 },		/* im_height      		*/
	{ IM_UCHAR_TYPE,  1, 1 },		/* im_flags	  		*/ 
	{ 0, 0, 0 }
};
...
{
	{ IM_UCHAR_TYPE, 1, 1},			/* blocksize                       */
	{ IM_UCHAR_TYPE, 1, 1},                 /* flags                           */
	{ IM_UINT16_TYPE, 2, 1},                /* delay                           */
	{ IM_UCHAR_TYPE, 1, 1},                 /* transparent_color               */
	{ IM_UCHAR_TYPE, 1, 1},                 /* terminator                      */
	{ 0, 0, 0 }
};
...
#endif
{
	...
	ImMalloc( cltBuffer, unsigned char *, sizeof( IM_UCHAR_TYPE ) * nClt * 3 );


	/* Read in Red, Green, and Blues.                        	*/
        if ( ImBinRead( ioType, fd, fp, cltBuffer, IM_UCHAR_TYPE, 1, nClt * 3 ) == -1 )
        {
                free( (char *)cltBuffer );
                ImReturnBinError( );
        }
...
#endif
{
	 ...
	 */
	if ( ImBinRead( ioType, fd, fp, &c, IM_UCHAR_TYPE, 1, 1 ) == -1 )
	{
		ImReturnBinError();
	}
	...
	if (transparency==1)
	{
		...
		if (block.flags & IMGIFTRANSPARENCY)
		{
			...
			sprintf(tmp,"index=%d",(int)block.transparent_color);
				...
				TagEntryAlloc( "transparency value", IM_POINTER_TYPE, &tmp));
		}
	...
	else
	{
		...
		for ( ; ; )
		{
			 ...
			 *	Skip over block
			 */
			if ( ImBinRead( ioType, fd, fp, buf, IM_UCHAR_TYPE, 1, c  ) == -1 )
			{
				ImReturnBinError();
			}
		}
	...
	while(!end)
	{
		...
		case IMGIFSEPARATOR:	/* Image separator character 	  */
			   ...
			   && ( lClt != IMCLTNULL) )
		   {
		   	ImVfbSClt( vfb, lClt );
		        TagTableAppend( tagTable,
				TagEntryAlloc( "image clt", IM_POINTER_TYPE, &lClt));
		   }
		   else if (gClt != IMCLTNULL)
		   {
			ImVfbSClt( vfb, gClt );
		        TagTableAppend( tagTable,
				TagEntryAlloc( "image clt", IM_POINTER_TYPE, &gClt));
		   }
		   TagTableAppend( tagTable,
...
#endif
{
	...
	if ( ImBinWrite( ioType, fd, fp, cltBuffer, IM_UCHAR_TYPE, 1, newClt * 3 ) == -1)
        {
...
#endif
{
	...
	c = IMGIFSEPARATOR;
	if ( ImBinWrite( ioType, fd, fp, &c, IM_UCHAR_TYPE, 1, 1 ) == -1 )
	{
                ImReturnBinError( );  
	}
...
#endif
{
	...
	blockId = IMGIFEXTENSION;
        if ( ImBinWrite( ioType, fd, fp, &blockId, IM_UCHAR_TYPE, 1, 1 ) == -1 )
        {
                ImReturnBinError( );
        }
