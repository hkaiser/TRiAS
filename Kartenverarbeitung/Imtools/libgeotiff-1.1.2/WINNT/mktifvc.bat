...
nmake -f Maketif.vc clean

set DLLOPTS=
set DLLDEFS=
set DLLLINK=

nmake -f Maketif.vc tif_fax3sm.c
...
nmake %MAKEOPTS% -f Maketif.vc
if "%1"=="" goto done

:console
set DLLOPTS=$(cvarsdll)
set DLLDEFS=-DCONSOLE_BUILD
set DLLLINK=$(dlllflags) -DEF:libctif.def
if "%1"=="ALL" nmake -f Maketif.vc clean_objs
nmake %MAKEOPTS% -f Maketif.vc libctif.dll
if "%1"=="CON" goto done
...
set DLLOPTS=$(cvarsdll)
set DLLDEFS=
set DLLLINK=$(dlllflags) -DEF:libgtif.def
if "%1"=="ALL" nmake -f Maketif.vc clean_objs
nmake %MAKEOPTS% -f Maketif.vc libgtif.dll
if "%1"=="GUI" goto done
