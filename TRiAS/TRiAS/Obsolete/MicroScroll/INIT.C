 ...
 * INIT.C
 *
 * Initialization for MicroScroll32 control called from DLLMain entry point. 
 * Used once or very infrequently.
 *
 ...
 *  nMsg            UINT message, either WM_NCCREATE or WM_CREATE.
 *  pMS             PMUSCROLL pointer to existing MUSCROLL structure.
 *
 * Return Value:
 *  LRESULT         0L if memory could not be allocated on WM_NCCREATE.
 *                  1L in all other cases.
 */

LRESULT PASCAL LMicroScrollCreate(HWND hWnd, UINT nMsg, PMUSCROLL pMS,
                               LPCREATESTRUCT lpCreate)
{
    PMUSCROLL      pMuscrollData;
    BOOL           fTextRange;
    INT            iMin, iMax, iPos;
    ...
    if (WM_NCCREATE==nMsg)
    {
        ...
        pMuscrollData = (PMUSCROLL)LocalAlloc(LPTR, CBMUSCROLL);
        // Want this set to NULL if malloc fails
        SetWindowLong(hWnd, GWL_MUSCRL32DATA, (LONG)pMuscrollData);
    ...
    if (WM_CREATE==nMsg)
    {
        ...
        pMS->hWndAssociate = GetParent( hWnd );
...
// Richtung umkehren (#HK950119)
	if (pMS->dwStyle & MSS_INVERTRANGE)
         ...
         * FTextParse failed.
         */
        pMS->nMin=(fTextRange ? iMin : IDEFAULTMIN);
        pMS->nMax=(fTextRange ? iMax : IDEFAULTMAX);
        pMS->nPos=(fTextRange ? iPos : IDEFAULTPOS);
...
BOOL PASCAL FTextParse(LPSTR psz, LPINT piMin, LPINT piMax, LPINT piPos)
{
     ...
     * on NTranslateUpToChar to modify psz to point to the character
     * after the delimeter which is why we pass &psz.
     */
    iMin=NTranslateUpToChar(&psz, ',');
 ...
 *
 *  NOTE!! This implementation assumes no negative values!
 *
...
INT PASCAL NTranslateUpToChar(LPSTR FAR *ppsz, char chDelimeter)
{
    INT    nRet=0;
    char    ch;
    LPSTR   psz;
