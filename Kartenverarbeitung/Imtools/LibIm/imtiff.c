/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imtiff.c,v 1.41 1995/10/03 20:42:37 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imtiff.c,v 1.41 1995/10/03 20:42:37 bduggan Exp $"
...
#include "tiff.h"
#include "tiffio.h"
#if defined(_USE_LIBGEOTIFF)
#include "libxtiff/xtiffiop.h"
#endif // defined(_USE_LIBGEOTIFF)
#endif
...
static ImFileFormatReadMap imTiffReadMap[ ] =
{
        ...
        { IN,1,1,       CCR,    IMVFBMONO,      0 },
        { IN,1,1,       CC3,    IMVFBMONO,      0 },
        { IN,1,1,       CC4,    IMVFBMONO,      0 },
#else
        { IN,1,1,       0,      IMVFBMONO,      0 },
        { IN,1,4,       0,      IMVFBINDEX8,    0 },
        { IN,1,8,       0,      IMVFBINDEX8,    0 },
...
static ImFileFormatWriteMap imTiffWriteMap[ ] =
{
        ...
        { IMVFBMONO,    0,      IN,1,1,         CCR,     imTiffWrite },
        { IMVFBMONO,    0,      IN,1,1,         CC3,     imTiffWrite },
        { IMVFBMONO,    0,      IN,1,1,         CC4,     imTiffWrite },

        { IMVFBMONO,    C,      IN,1,1,         C|LZW,   imTiffWrite },
        { IMVFBMONO,    C,      IN,1,1,         C|PB,    imTiffWrite },
        { IMVFBMONO,    C,      IN,1,1,         C,       imTiffWrite },
        { IMVFBMONO,    0,      IN,1,1,         C|CCR,   imTiffWrite },
        { IMVFBMONO,    0,      IN,1,1,         C|CC3,   imTiffWrite },
        { IMVFBMONO,    0,      IN,1,1,         C|CC4,   imTiffWrite },

        { IMVFBMONO,    0,      IN,1,1,         PI|LZW,  imTiffWrite },
        { IMVFBMONO,    0,      IN,1,1,         PI|PB,   imTiffWrite },
        { IMVFBMONO,    0,      IN,1,1,         PI,      imTiffWrite },
        { IMVFBMONO,    0,      IN,1,1,         PI|CCR,  imTiffWrite },
        { IMVFBMONO,    0,      IN,1,1,         PI|CC3,  imTiffWrite },
        { IMVFBMONO,    0,      IN,1,1,         PI|CC4,  imTiffWrite },
 
        { IMVFBMONO,    C,      IN,1,1,         PI|C|LZW,imTiffWrite },
        { IMVFBMONO,    C,      IN,1,1,         PI|C|PB, imTiffWrite },
        { IMVFBMONO,    C,      IN,1,1,         PI|C,    imTiffWrite },
        { IMVFBMONO,    0,      IN,1,1,         PI|C|CCR, imTiffWrite },
        { IMVFBMONO,    0,      IN,1,1,         PI|C|CC3, imTiffWrite },
        { IMVFBMONO,    0,      IN,1,1,         PI|C|CC4, imTiffWrite },
...
{										\
	case IMVFBGREY :  /* same as INDEX8 */					\
		...
		break;								\
	case IMVFBMONO  :							\
		value = (uint16) ImVfbQMono  (vfb, vfbptr);			\
		break;								\
	case IMVFBRED  :							\
	...
	while ( nextDir )
	{
		...
		if ( clt != IMCLTNULL )
		{
			...
			TagTableAppend( tagTable, 
				TagEntryAlloc( "image clt", IM_POINTER_TYPE, &clt ));
		}
		...
		switch( imTiffInfo.t_compression )
		{
			...
			case IM_COMPRESSION_CCITTRLE:
				sprintf( message, "CCITT Run Length Encoded (Huffman RLE)" );
				break;
			...
			case IM_COMPRESSION_NEXT:
				sprintf( message, "Run Length Encoded (NeXT 2-bit RLE)" );
				break;
			case IM_COMPRESSION_CCITTRLEW:
				sprintf( message, "Run Length Encoded W (RLE with word alignment)" );
				break;
			case IM_COMPRESSION_PACKBITS:
				sprintf( message, "Apple Macintosh Packbits");
				break;
			case IM_COMPRESSION_THUNDERSCAN:
				sprintf( message, "Run Length Encoded (Thunderscan RLE)" );
				break;
			case IM_COMPRESSION_PICIO:
				sprintf( message, "Run Length Encoded (Pixar RLE)" );
				break;
			case IM_COMPRESSION_SGIRLE:
				sprintf( message, "Run Length Encoded (Silicon Graphics RLE)" );
				break;
		}
...
#endif
{
	...
	else
	{
		...
		case IMTIFFSHORT:
			ImMalloc( sp, unsigned short *, d->tdir_count * sizeof(unsigned short));
...
#endif
{
	...
	if ( imTDI->t_compression != IM_COMPRESSION_NONE )
		ImMalloc( ucstrip, unsigned char *, ((size+3)/4)*4 );
...
*/
	...
	else if ( compRequest == IMCOMPCCITTRLE )
	{
		imTiffWriteDir[IMTIFF_COMPRESSION_INDEX].tdir_offset = 
							COMPRESSION_CCITTRLE << 16;
		compress = IM_COMPRESSION_RLE;
	}
	else if ( compRequest == IMCOMPCCITTFAX3 )
	{
		imTiffWriteDir[IMTIFF_COMPRESSION_INDEX].tdir_offset = 
							COMPRESSION_CCITTFAX3 << 16;
		compress = IM_COMPRESSION_CCITTFAX3;
	}
	else if ( compRequest == IMCOMPCCITTFAX4 )
	{
		imTiffWriteDir[IMTIFF_COMPRESSION_INDEX].tdir_offset = 
							COMPRESSION_CCITTFAX4 << 16;
		compress = IM_COMPRESSION_CCITTFAX4;
	}
	else
	{
		imTiffWriteDir[IMTIFF_COMPRESSION_INDEX].tdir_offset = 
	...
	if ( clt != IMCLTNULL && cltRequest == IMCLTYES )
	{
		...
		ImMalloc( cbuffer, unsigned short*, sizeof(unsigned short) * n );
	 ...
	 */
	ImCalloc( stripbytecounts,  unsigned int *, sizeof(unsigned int), y );  
	...
	ImCalloc( stripoffsets,  unsigned int *,  sizeof(unsigned int), y );
...
*/
	...
	switch( compress )
	{
		...
		case IM_COMPRESSION_PACKBITS:
			...
			break;
		case IM_COMPRESSION_CCITTRLE:
			ImInfo("Compression Type", "CCITT Run Length Encoded (Huffman RLE)" );
			break;
		case IM_COMPRESSION_CCITTFAX3:
			ImInfo("Compression Type", "CCITT Group 3 FAX" );
			break;
		case IM_COMPRESSION_CCITTFAX4:
			ImInfo("Compression Type", "CCITT Group 4 FAX" );
			break;
		case IM_COMPRESSION_NONE:
...
#if defined(_USE_LIBGEOTIFF)
static void
XTIFFPrintGeoTIFFInfo (TIFF* tif, long flags)
{
	...
	if (TIFFFieldSet(tif, FIELD_GEOTIEPOINTS))
	{
		...
		if (num > 0) {
			ImInfo ("Geo Tiepoints", "");
			message[0] = '\0';
			for (i = 0; i < num; i += 6)
			{
			   sprintf(message, "\t(%f %f %f)", xd->xd_geotiepoints[i], xd->xd_geotiepoints[i+1], xd->xd_geotiepoints[i+2]);
			   sprintf(cbBuffer, "->(%f %f %f)", xd->xd_geotiepoints[i+3], xd->xd_geotiepoints[i+4], xd->xd_geotiepoints[i+5]);
			   strcat (message, cbBuffer);
			   ImInfoPlain (message);
			}
		}
	}
	...
	if (TIFFFieldSet(tif, FIELD_GEOPIXELSCALE))
	{
		...
		if (num > 0) {
			ImInfo ("Geo Pixel Scale", "");
			strcpy (message, "\t");
			for (j = 0; j < num; j++) {
				sprintf (cbBuffer, " %f", xd->xd_geopixelscale[j]);
				strcat (message, cbBuffer);
			}
			ImInfoPlain (message);
		}
	...
	if (TIFFFieldSet(tif, FIELD_INTERGRAPH_MATRIX))
	{
		...
		if (num > 0) {
			ImInfo ("Intergraph Transformation Matrix", "");
			for (i = 0; num > 3; num -= 4)
			{
				message[0] = '\0';
				for (j = 0; j < 4; j++) {
					sprintf(cbBuffer, " %8.2f", xd->xd_geomatrix[i++]);
					strcat (message, cbBuffer);
				}
				ImInfoPlain (message);
			}
	...
	if (TIFFFieldSet(tif, FIELD_GEOTRANSMATRIX))
	{
		...
		if (num > 0) {
			ImInfo ("Geo Transformation Matrix", "");
			for (i = 0; i < num; i += 4) {
				message[0] = '\0';
				for (j = 0; j < 4; j++) {
					sprintf(cbBuffer, " %8.2f", xd->xd_geomatrix[i++]);
	...
	if (TIFFFieldSet(tif, FIELD_GEOKEYDIRECTORY))
	{
		...
		if (num > 0) {
			...
			{
				ImInfo ("GeoKey Directory", "");
				for (i = 0; i < num; i += 4)
				{
					message[0] = '\0';
					for (j = 0; j < 4; j++) {
						sprintf(cbBuffer, "  %8hu", xd->xd_geokeydirectory[i+j]);
						strcat (message, cbBuffer);
					}
					ImInfoPlain (message);
				}
			} else
				ImInfo("GeoKey Directory", "(present)");
		}
	}
	...
	if (TIFFFieldSet(tif, FIELD_GEODOUBLEPARAMS))
	{
		...
		if (num > 0) {
			...
			{
				ImInfo ("GeoKey Double Params", "");
				message[0] = '\0';
				for (i = 0; i < num; i++) {
					sprintf (cbBuffer, "  %8.2f", xd->xd_geodoubleparams[i]);
					strcat (message, cbBuffer);
				}
				ImInfoPlain (message);
			} else
				ImInfo("GeoKey Double Params", "(present)");
		}
	}

	if (TIFFFieldSet(tif, FIELD_GEOASCIIPARAMS))
	{
		ImInfo ("GeoKey ASCII Parameters", xd->xd_geoasciiparams);
	}
}
#endif // defined(_USE_LIBGEOTIFF)
...
#if defined(_USE_LIBGEOTIFF)
 tif = XTIFFFdOpen( fd, filename ? filename : "libtiff", "r"); 
#else
 tif = TIFFFdOpen( fd, filename ? filename : "libtiff", "r"); 
#endif // defined(_USE_LIBGEOTIFF)
 if (tif == IMTIFFNULL)
 {
...
//	 vfb = ImVfbAlloc( xSize, ySize, fieldMask );
 ...
 /* Discern the bit and byte order for ImInfo message */
	 TIFFGetFieldDefaulted(tif, TIFFTAG_FILLORDER, &byteOrder);
	 if (byteOrder==FILLORDER_MSB2LSB)
		ImInfo( "Bit Order", "Most Significant Bit First")
	 else
		ImInfo( "Bit Order", "Least Significant Bit First")

// Byte order bestimmen
	{ union { int32 i; char c[4]; } u; u.i = 1; fBigEndian = (u.c[0] == 0); }

	if (TIFFIsByteSwapped(tif) ? !fBigEndian : fBigEndian)
		ImInfo( "Byte Order", "Most Significant Byte First")
	else
		ImInfo( "Byte Order", "Least Significant Byte First")
...
#if defined(_USE_LIBGEOTIFF)
// print out GeoTIFF tags (if available)
	XTIFFPrintGeoTIFFInfo (tif, TIFFPRINT_GEOKEYPARAMS|TIFFPRINT_GEOKEYDIRECTORY);
#endif // defined(_USE_LIBGEOTIFF)
	 ...
	 if (photometric == PHOTOMETRIC_PALETTE)
	 {
		...
		if (imTiffReadClt(tif, &cltBuf) != 0) 
		{
			...
			TagTableAppend( tagTable,
				TagEntryAlloc( "image clt", IM_POINTER_TYPE, &cltBuf ) );
		} 
...
#if defined(_USE_LIBGEOTIFF)
 XTIFFCloseDisconnect( tif );
#else
 TIFFCloseDisconnect( tif );
#endif // defined(_USE_LIBGEOTIFF)
...
#endif
{
 ...
 uint32 row;		/* Loop indices                               */
 uint32 xSize, ySize;			/* Image size (x, y)	                      */
 uint16 numchans;               /* number of channels                         */
 uint16 chandepth; 	 	/* number of bits per channel                 */
 ...
 samplesPerScanline = xSize; // scanlineSize * 8 / chandepth;
  ...
  */
 // #HK000705
	isccitt_compression = (compression == COMPRESSION_CCITTRLE || compression == COMPRESSION_CCITTFAX3 ||
						   compression == COMPRESSION_CCITTRLEW || compression==COMPRESSION_CCITTFAX4) ? 1 : 0;
	if (isccitt_compression)
	{
 ...
 switch (chandepth)
 {
 	case 16 : mask = (unsigned short)(mask | (1<<0));  
		  mask = (unsigned short)(mask | (1<<1));
		  mask = (unsigned short)(mask | (1<<2));
		  mask = (unsigned short)(mask | (1<<3));
		  mask = (unsigned short)(mask | (1<<4));
		  mask = (unsigned short)(mask | (1<<5));
		  mask = (unsigned short)(mask | (1<<6));
		  mask = (unsigned short)(mask | (1<<7));   /* fall through */
	case  8 : mask = (unsigned short)(mask | (1<<8));
		  mask = (unsigned short)(mask | (1<<9));
		  mask = (unsigned short)(mask | (1<<10));
		  mask = (unsigned short)(mask | (1<<11));   /* fall through */
	case 4  : mask = (unsigned short)(mask | (1<<12));
		  mask = (unsigned short)(mask | (1<<13));   /* fall through */
	case 2  : mask = (unsigned short)(mask | (1<<14));   /* fall through */
	case 1  : mask = (unsigned short)(mask | (1<<15));
		  break;
	default :
 	...
 	case 1 : 
		 ...
		 */
		 ...
		 if (chandepth==16)
			 ...
			 chanList[0] = IMVFBINDEX8;	// IMVFBMONO;	// ADDED: #HK000328
		 break;
	case 3 : 
		 chanList[0] = IMVFBRED;
 ...
 ImMalloc(linebuf, unsigned char *, ((scanlineSize+3)/4)*4);
 ...
 for (row=0; row< ySize ; row++)
 {
	...
	if (TIFFReadScanline(tif, linebuf, row, 0) < 0)
		break;
	if (byteOrder==BINLBF)
		imTiffSwapBytes(linebuf, scanlineSize/*samplesPerScanline*/);
	...
	while (sampleNum < samplesPerScanline)
	{
		...
		/* Is 0 black or white? */
		if ((isccitt_compression && (photometric == PHOTOMETRIC_MINISBLACK)) ||		// #HK000705
			(!isccitt_compression && photometric == PHOTOMETRIC_MINISWHITE))
		{
		...
		switch (chanList[chanIndex])
		{
			case IMVFBINDEX8 : /* same as IMVFBGREY */
				...
				break;
			case IMVFBMONO  :	// ADDED: #HK000328
				ImVfbSMono  (vfb, vfbptr, IM_SCALE_0_255(currentValue,chandepth) );
				break;
			case IMVFBRED  :
...
static int LowBytesAreNull(uint16 *pmap, int nClt)
{
int i = 0;

	for (i = 0; i < nClt; ++i) {
		if (0 != (unsigned char)(pmap[i]))
			return 0;
	}
	return 1;
}
...
// ggf. Sonderfall für LEADTOOLS abtesten
	if (256 == nClt && LowBytesAreNull(redmap, 256) && 
		LowBytesAreNull(greenmap, 256) && LowBytesAreNull(bluemap, 256))
	{
		for (i=0;i < nClt; i++)
		{
			ImCltSRed   (cltptr, (redmap[i] >> 8)   );
			ImCltSGreen (cltptr, (greenmap[i] >> 8) );
			ImCltSBlue  (cltptr, (bluemap[i] >> 8)  );
...
// int pixelsInThisTile;   	/* Number of pixels in this tile              */
 ...
 samplesPerTile = tileWidth; // tileSize * 8 / chandepth;
 byteOrder      = BinQMachine()->bin_byteOrder;
 ...
 for (y = 0; y < imageLength ; y+=tileLength)
 {
	...
	for (x = 0; x < imageWidth; x += tileWidth)
	{
		...
		// #HK000705
			if ((isccitt_compression && (photometric == PHOTOMETRIC_MINISBLACK)) ||
				(!isccitt_compression && photometric == PHOTOMETRIC_MINISWHITE))
			{
				currentValue = ~currentValue;
...
#endif
{
 ...
 int iMonoThreshold = IMDEFMONOTHRESH;
...
#if defined(_USE_LIBGEOTIFF)
 tif = XTIFFFdOpen(fd, filename ? filename : "libtiff", "w"); 
#else
 tif = TIFFFdOpen(fd, filename ? filename : "libtiff", "w"); 
#endif // defined(_USE_LIBGEOTIFF)
 if (!tif)
 {
 ...
 entry = TagTableQDirect( flagsTable, "image mono threshold", 0 );
 if ( entry != TAGENTRYNULL )
	 TagEntryQValue( entry, &iMonoThreshold );

 entry = TagTableQDirect( flagsTable, "force bitorder swapping", 0 );
 if ( entry != TAGENTRYNULL ) 
	 TagEntryQValue( entry, &iSwapBitOrder );
 if (iSwapBitOrder)
	ImInfo ("Bit order", "ATTENTION: swapping bitorder!");
 ...
 for (curImage = 0; curImage < numImages; curImage++)
 {
	 ...
	 /* Retrieve this vfb from the table */
...
//		  TIFFSetField(tif, TIFFTAG_FILLORDER,FILLORDER_MSB2LSB);   
	 ...
	 switch (pMap->map_outAttributes & IMCOMPMASK)
	 {
	  ...
	  case IMCOMPLZW :
		...
		break;
	  case IMCOMPCCITTRLE :
		ImInfo ("Compression Type", "CCITT modified Huffman RLE");
		TIFFSetField(tif, TIFFTAG_COMPRESSION, COMPRESSION_CCITTRLE);
//		TIFFSetField(tif,TIFFTAG_PHOTOMETRIC, (iMonoThreshold < 0) ? PHOTOMETRIC_MINISWHITE : PHOTOMETRIC_MINISBLACK);
		break;
	  case IMCOMPCCITTFAX3 :
		ImInfo ("Compression Type", "CCITT Group 3 fax encoding");
		TIFFSetField(tif, TIFFTAG_COMPRESSION, COMPRESSION_CCITTFAX3);
//		TIFFSetField(tif,TIFFTAG_PHOTOMETRIC, (iMonoThreshold < 0) ? PHOTOMETRIC_MINISWHITE : PHOTOMETRIC_MINISBLACK);
		break;
	  case IMCOMPCCITTFAX4 :
		ImInfo ("Compression Type", "CCITT Group 4 fax encoding");
...
#endif
{
 ...
 /* Switch the photometric if it is a greyscale image */
	if (photometric == PHOTOMETRIC_MINISBLACK)
		...
		photometric = PHOTOMETRIC_MINISBLACK;
	TIFFSetField(tif, TIFFTAG_PHOTOMETRIC, photometric);		// #HK000704
 }
 ...
 switch (numchans)
 {
 	case 1 : 
		 if (chandepth==16)
			 ...
			 chanList[0] = IMVFBINDEX8;
// ADDED: BEGIN: #HK000327
		 else
			 chanList[0] = IMVFBMONO;
// ADDED: END: #HK000327
		 break;
	case 3 : 
		 chanList[0] = IMVFBRED;
...
// evtl. auch gleich noch bits kippen
	if (iSwapBitOrder) {
		for (i = 0; i < linesize; i++) 
			linebuf[i] = swaptable[linebuf[i]];
	}
}
