...
// Created: 27.10.2000 20:58:05 
//
// @doc
// @module DtoACTF.cpp | Definition of the DtoACTF function
...
// We don't want to display the exponent if it is zero
	...
	// The float can be shown without scientific notation
		wsprintf(lpszString, lpszFormat, (long)dblInt, ulTempFraction);
...
char *pT = lpszString;
	...
	if (iCnt > 0) 
		memmove (lpszString, &lpszString[iCnt], strlen(lpszString)-iCnt+1);	// NullByte mitkopieren
