/**
 **
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/impict.c,v 1.28 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/impict.c,v 1.28 1995/06/29 00:28:04 bduggan Exp $"
 ...
 **	changed a IM_SHORT_TYPE to IM_USHORT_TYPE to maintian type compatibility.
...
{
	IM_SHORT_TYPE,2,1, IM_SHORT_TYPE,2,1, IM_SHORT_TYPE,2,1, IM_SHORT_TYPE,2,1,
	IM_SHORT_TYPE,2,1,
	IM_SHORT_TYPE,2,1,
	IM_LONG_TYPE,4,1,
	IMFIXED,4,1,
 ...
 */
#ifdef __STDC__
static void ImSetPixel( short int x, short int y, struct RGBColor color )
#else
static void ImSetPixel( x, y, color )
	short int x, y;
...
#endif
{
	...
	while ( count > 0 )
	{
		...
		result = ImBinRead( g_ioType, g_fd, g_fp, g_buf, IM_CHAR_TYPE, 1, skip );
...
PO_BkPat( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, g_gs.bkPat, IM_UCHAR_TYPE, 1, uc ) == -1 )
	{
		ImReturnBinError( );
	}
...
PO_TxFont( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &g_gs.txFont, IM_SHORT_TYPE, 2, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
...
{
	int result = 1;
	if ( ImBinRead( g_ioType, g_fd, g_fp, &g_gs.txFace, IM_UCHAR_TYPE, 1, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
...
PO_TxMode( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &g_gs.txMode, IM_SHORT_TYPE, 2, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
...
PO_PnMode( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &g_gs.pnMode, IM_SHORT_TYPE, 2, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
...
PO_PnPat( )
{
	...
	uc = (unsigned char) result;
	if ( ImBinRead( g_ioType, g_fd, g_fp, g_gs.pnPat, IM_UCHAR_TYPE, 1, uc ) == -1 )
	{
		ImReturnBinError( );
	}
...
PO_FillPat( )
{
	...
	uc = (unsigned char) result;
	if ( ImBinRead( g_ioType, g_fd, g_fp, g_gs.fillPat, IM_UCHAR_TYPE, 1, uc )== -1)
	{
		ImReturnBinError( );
	}
...
PO_TxSize( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &g_gs.txSize, IM_SHORT_TYPE, 2, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
...
{
	int result = 4;
	if ( ImBinRead( g_ioType, g_fd, g_fp, &g_gs.fgColor, IM_LONG_TYPE, 4, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
...
{
	int result = 4;
	if ( ImBinRead( g_ioType, g_fd, g_fp, &g_gs.bkColor, IM_LONG_TYPE, 4, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
...
PO_Version( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &g_gs.version, IM_UCHAR_TYPE, 1, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
...
PO_BkPixPat( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &ushort1, IM_SHORT_TYPE, 2, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
	if ( ushort1 == 1 )
...
PO_FillPixPat( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &i, IM_INT_TYPE, 2, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
	if ( i == 1 )
...
PO_PnLocHFrac( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &g_gs.pnLocHFrac, IM_SHORT_TYPE, 2, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
...
PO_ChExtra( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &g_gs.chExtra, IM_SHORT_TYPE, 2, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
...
PO_ShortLine( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &char1, IM_CHAR_TYPE, 1, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
	if ( ImBinRead( g_ioType, g_fd, g_fp, &char2, IM_CHAR_TYPE, 1, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
...
PO_reserved4( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &ushort1, IM_USHORT_TYPE, 2, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
	if ( ushort1 > 0 )
...
//	char msg[ IMSTR_LEN ];
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &uchar1, IM_UCHAR_TYPE, 1, 1 )== -1)
	{
		ImReturnBinError( );
	}
	result += uchar1;
...
PO_DHText( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &uchar2, IM_UCHAR_TYPE, 1, 1 )== -1)
	{
		ImReturnBinError( );
	}
	result += uchar2;
...
PO_DHDVText( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &uchar3, IM_UCHAR_TYPE, 1, 1 )== -1)
	{
		ImReturnBinError( );
	}
	result += uchar3;
...
PO_reserved8( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &short1, IM_SHORT_TYPE, 2, 1 )== -1)
	{
		ImReturnBinError( );
	}
	result += short1;
...
BitMap( )
{
	...
	result = 2;
	/* read rowBytes */
	if ( ImBinRead( g_ioType, g_fd, g_fp, &rowBytes, IM_USHORT_TYPE, 2, 1 )== -1)
	{
		ImReturnBinError( );
	}
...
fprintf( stderr, "DEBUG PackBitsRect ; dst = {%d,%d,%d,%d}\n", bMap.dstRect.top, bMap.dstRect.left, bMap.dstRect.bottom, bMap.dstRect.right );
	...
	for ( lin=0; lin<sLines; lin++ )
	{
		...
		{
			result += 1;
			if ( ImBinRead(g_ioType, g_fd, g_fp, &byteCount, IM_UINT_TYPE, BCLen, 1)== -1)
			{
				ImReturnBinError( );
			}
#ifdef DEBUG
if ( lin==0 || lin==sLines-1 ) {
   ...
   if ( lin==0 )  fprintf( stderr, "...etc...\n" );
...
#endif
			result += byteCount;
			if ( ImBinRead( g_ioType, g_fd, g_fp, pixBuffer, IM_UCHAR_TYPE, 1, byteCount )== -1)
			{
				ImReturnBinError( );
			}
		...
		{	/* g_Opcode == PO_BitsRect */
			byteCount = rowBytes;
			result += byteCount;
			if ( ImBinRead( g_ioType, g_fd, g_fp, runBuffer, IM_UCHAR_TYPE, 1, byteCount )== -1)
			{
				ImReturnBinError( );
			}
#ifdef DEBUG
...
if ( i==0 || i==cTab.ctSize ) {
   ...
   if ( i==0 )  fprintf( stderr, "...etc...\n" );
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &g_gs.pnMode, IM_USHORT_TYPE, 2, 1 )== -1)
	{
		ImReturnBinError( );
	}
...
PO_LongComment( )
{
	...
	if ( ImBinRead( g_ioType, g_fd, g_fp, &ushort2, IM_USHORT_TYPE, 2, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
	result += ushort2;
...
fprintf( stderr, "DEBUG: read pictSize\n" );
	 ...
	 */
	if ( ImBinRead( ioType, fd, fp, &pictSize, IM_USHORT_TYPE, 2, 1 )== -1)
	{
		ImReturnBinError( );
	}
#ifdef DEBUG
...
fprintf( stderr, "DEBUG: read version\n" );
	 ...
	 */
	if ( ImBinRead( ioType, fd, fp, &g_pictVers, IM_SHORT_TYPE, 2, 1 )== -1)
	{
		ImReturnBinError( );
	}
	if ( g_pictVers == IMPICT_V1 )
...
fprintf( stderr, "DEBUG: fill the vfb with background color\n" );
	...
	for ( i=0; i<x; i++ )
	{
		for ( j=0; j<y; j++ )
		{
			ImSetPixel( i, j, g_gs.rgbBkColor );
		}
	}
...
#endif
	g_Opcode = 0;
	if ( ImBinRead( ioType, fd, fp, &g_Opcode, IM_USHORT_TYPE, OCLen, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
...
fprintf( stderr, "DEBUG: OPCode just read = 0x%0x\n", g_Opcode );
			...
			{
				NData += 2;
				if ( ImBinRead( ioType, fd, fp, &ushort1, IM_USHORT_TYPE, 2, 1 )== -1)
				{
					ImReturnBinError( );
				}
				if ( ushort1 > 0 )
			...
			{
				NData += 4;
				if ( ImBinRead( ioType, fd, fp, &ulong1, IM_ULONG_TYPE, 4, 1 )== -1)
				{
					ImReturnBinError( );
				}
				if ( ulong1 > 0 )
		...
		if ( g_pictVers == IMPICT_V2 )
		{
			...
			{
				NData += 1;
				if ( ImBinRead( ioType, fd, fp, &i, IM_INT_TYPE, 1, 1 ) == -1 )
				{
					ImReturnBinError( );
				}
			}
	 ...
	 */
	TagTableAppend( tagTable, TagEntryAlloc( "image clt", IM_POINTER_TYPE, &clt ) );

	/*
	 *  Add the VFB to the tagTable.
	 */
	TagTableAppend( tagTable, TagEntryAlloc( "image vfb", IM_POINTER_TYPE, &g_vfb ) );
...
fprintf( stderr, "DEBUG: write 512 byte header\n" );
	...
	if ( ImBinWrite( ioType, fd, fp, g_buf, IM_UCHAR_TYPE, 1, 512 )== -1)
	{
		ImReturnBinError( );
	}
...
fprintf( stderr, "DEBUG: write pictSize\n" );
	 ...
	 */
	pictSize = 0x0000;
	if ( ImBinWrite( ioType, fd, fp, &pictSize, IM_USHORT_TYPE, 2, 1 )== -1)
	{
		ImReturnBinError( );
	}
...
fprintf( stderr, "DEBUG: write version\n" );
	 ...
	 */
	ushort1 = 0x0011;
	if ( ImBinWrite( ioType, fd, fp, &ushort1, IM_USHORT_TYPE, 2, 1 )== -1)
	{
		ImReturnBinError( );
	}
	ushort1 = 0x02FF;
...
fprintf( stderr, "DEBUG: write the traditional start-em-off opcodes\n" );
	...
	ushort1 = 0x00A0;
	if ( ImBinWrite( ioType, fd, fp, &ushort1, IM_USHORT_TYPE, OCLen, 1 )== -1)
	{
		ImReturnBinError( );
	}
	...
	if ( ImBinWrite( ioType, fd, fp, &rowBytes, IM_USHORT_TYPE, 2, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
	...
	for ( sLine=0; sLine<sLines ; sLine++ )
	{
		...
		if ( ImBinWrite( ioType, fd, fp, &byteCount, IM_UINT_TYPE,
			BCLen, 1 ) == -1 )
		{
			ImReturnBinError( );
		}
		if ( ImBinWrite( ioType, fd, fp, rbp, IM_UCHAR_TYPE,
			1, byteCount ) == -1 )
		{
			ImReturnBinError( );
		}
	...
	{
		g_buf[0] = '\0';
		if ( ImBinWrite( ioType, fd, fp, g_buf, IM_UCHAR_TYPE, 1, 1)== -1)
		{
			ImReturnBinError( );
		}
	}
...
#endif
{
	...
	if ( ImBinWrite( ioType, fd, fp, &ulong1, IM_ULONG_TYPE, 4, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
