...
//	BOOL OF_BI_PunktGueltig( short, short );
	...
	BOOL HatObjekte( int );
	long		m_lEDBSSatzNummer;
	long		m_lNumParserCalls;
...
private:
	...
	CObArray	m_Linien;
	CObArray	m_Objekte;
	CObArray	m_Infos;				// Besondere Informationen
...
public:			CAtkisSatz();
	...
	int		AnzahlLinien()		{ return m_Linien.GetSize(); }
	int		AnzahlObjekte()		{ return m_Objekte.GetSize(); }
	int		AnzahlBesondereInformationen()	{ return m_Infos.GetSize(); }
	...
	CPunktFolge *OF_BI_Points( short, short );
