...
//
bool IrisWind :: ShowEditHeader (void) 
{
bool fResult = false;
	...
	CEierUhr Wait (this);
	CDocumentProperties DocProps (this); 

	// eigene Page hinzufügen
	CEditHeader EHPage;

		DocProps.AddPage (&EHPage);		// eigene Page ist die erste
		LoadPages (DocProps, g_cbDocKey, g_cbDocSubKey);			// zusätzliche Pages aus Registry laden
...
///////////////////////////////////////////////////////////////////////////////
// PropertySheet --------------------------------------------------------------
CDocumentProperties::CDocumentProperties (pWindow pWnd)
...
HRESULT LoadPages (CPropertySheetExt &rPage, LPCSTR pcKey, LPCSTR pcSubKey)
{
...
return rPage.LoadExtPagesFromRegistry (HKEY_CLASSES_ROOT, pcKey, pcSubKey, IDO, NULL);
}
...
bool fRet = DEX_EnumHeaderEntries(ENK);
...
HTREEITEM hItem = m_tvKeys.InsertItem (pcKey, iImage, iImage,
				(dwFlags & HEADER_SYSTEM) ? m_hSystem : m_hNormal, 
				TVI_LAST);
...
CEntryList::iterator it = m_Entries.find (hItem);
	...
	int iImage = fRO ? IMG_STRING_RO : IMG_STRING;
...
void DeleteEntryInDB (os_string &rstr)
{
GEODBHDRENTRY HE;
...
// zu modifizierende Einträge in DB abgleichen
	count_if (m_Entries.begin(), m_Entries.end(), IsDirty, iCnt);
	if (iCnt > 0) {
...
HTREEITEM hItem = m_tvKeys.GetSelectedItem();
	...
	case IDC_READONLY:
		AdjustRO (hItem, m_cbRO.isChecked());
		break;
...
HTREEITEM hItem = m_tvKeys.GetSelectedItem();
	...
	case IDM_TOGGLERO:
		AdjustRO (hItem, !RetrieveRO (hItem));
		break;
