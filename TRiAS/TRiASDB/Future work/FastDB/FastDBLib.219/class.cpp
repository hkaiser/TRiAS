...
{
    dbTableDescriptor* clone = new dbTableDescriptor(name,
						     DETACHED_TABLE,
						     appSize,
...
// #HK010426: BEGIN
// remove this object from the dbTableDescriptorChain, correct dbTableDescriptor::chain
dbTableDescriptor *tdprev = NULL;

	for (dbTableDescriptor *pp = chain; pp != NULL; pp = pp -> next) {
		if (this == chain)
			chain = next;
		if (this == pp) {
			if (NULL != tdprev)
