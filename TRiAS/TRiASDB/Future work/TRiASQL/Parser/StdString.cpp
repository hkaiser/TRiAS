...
// REMARKS:
//		This file implements the CString class which was declared in
...
#if !defined(_MFC_VER)
	HINSTANCE CString::hInstResource = ::GetModuleHandle(NULL);
#endif
...
//           
// DESCRIPTION:
//		This particular overload of the CString constructor takes either a real
//		string or a resource ID which has been converted with the MAKEINTRESOURCE()
//		macro
//
// PARAMETERS: 
//		pT	- a NULL-terminated raw string with which the CString object should be
//			  initialized or a resource ID converted with MAKEINTRESOURCE (or NULL)
//
// RETURN VALUE: none -- it's a constructor
// ---------------------------------------------------------------------------------------
CString::CString(PCTSTR pT) : STRBASE(szTNull)	// constructor for either a literal string or a resource ID
{ 
	if ( pT == NULL )
		;
...
// ---------------------------------------------------------------------
//bool CString::Wildstrcmp (PCTSTR c_s, PCTSTR c_mask)
//{
...
void CString::FormatV(PCTSTR szFormat, va_list argList)
{
...
{
	CString::iterator iter = std::find_if(begin(), end(), NotSpace());
	CString strNew(iter, end());
...
//           
// DESCRIPTION:
//		This function converts the CString to all uppercase characters using ctype
//
// PARAMETERS: 
// RETURN VALUE: 
//		a reference to this object (*this) -- allows chaining together of
...
// RETURN VALUE: none
// ---------------------------------------------------------------------------------------
void CString::CopyString(PCTSTR p_szSource, PTSTR p_szDest, int p_nMaxChars)
{
...
void CString::CopyString(PCOSTR p_szSource, POSTR p_szDest, int p_nMaxChars)
{
#ifdef _UNICODE
...
void CString::CopyString(PCSTR p_szSource, PWSTR p_szDest, int p_nMaxChars)
{
	USES_CONVERSION;
...
void CString::CopyString(PCWSTR p_szSource, PSTR p_szDest, int p_nMaxChars)
{
	USES_CONVERSION;
...
// Special, TEMPORARY operators that allow us to serialize CStrings to CArchives.
#ifdef _MFC_VER
...
// -----------------------------------------------------------------------------
// FUNCTION: CString::StreamSave
//		HRESULT StreamSave(IStream* pStream) const;
//
// DESCRIPTION:
//		This function write the length and contents of the CString object
...
// -----------------------------------------------------------------------------
HRESULT CString::StreamSave(IStream* pStream) const
{
	USES_CONVERSION;
	HRESULT hr = E_FAIL;
	ASSERT(pStream != NULL);
...
//	ULONG nChars = wcslen(pWide);
	...
	if ( FAILED(hr=pStream->Write(&nChars, sizeof(ULONG), NULL)) )
		TRACE(_T("CString::StreamSave -- Unable to write length to IStream due to error 0x%X\n"), hr);
	else if ( nChars < 0 )
		;	// (zero length means nothing to write)
	else if ( FAILED(hr=pStream->Write(pStr, nChars*sizeof(char), NULL)) )
		TRACE(_T("CString::StreamSave -- Unable to write string to IStream due to error 0x%X\n"), hr);

	return hr;
}

// -----------------------------------------------------------------------------
// FUNCTION: CString::StreamLoad
//		HRESULT StreamLoad(IStream* pStream);
//
// DESCRIPTION:
//		This function reads in a CString object from an IStream
//
// PARAMETERS:
...
// -----------------------------------------------------------------------------
HRESULT CString::StreamLoad(IStream* pStream)
{
	// All CStrings are serialized as char strings
	ASSERT(pStream != NULL);
	...
	{
		TRACE(_T("CString::StreamLoad -- Unable to read length from IStream due to error 0x%X\n"), hr);
	}
	else if ( nChars > 0 )
	{
...
//		PWSTR pWide = (PWSTR)_alloca(nByteLen+sizeof(wchar_t));		// add an extra wchar_t for terminating NULL
		...
		if ( FAILED(hr=pStream->Read(pStr, nByteLen, NULL)) )
			TRACE(_T("CString::StreamLoad -- Unable to read string from IStream due to error 0x%X\n"), hr);
		pStr[nChars] = '\0';
		*this = CString(pStr);
	}
	else
...
// PARAMETERS: none
// RETURN VALUE: 
//		length in bytes required to write the CString
// -----------------------------------------------------------------------------
ULONG CString::StreamSize() const
{
	USES_CONVERSION;
...
//		corresponding system string is returned in the form of a CString.
//
// PARAMETERS: 
...
//
// RETURN VALUE: 
//		a CString equivalent of the error code.  Currently, this function
//		only returns either English of the system default language strings.  
// -----------------------------------------------------------------------------
#define MAX_FMT_TRIES	5
#define FMT_BLOCK_SIZE	256
CString WUSysMessage(DWORD p_dwError, DWORD p_dwLangId)
{
	TCHAR szBuf[512];
...
//		CString WUFormat(UINT nId, ...);
//		CString WUFormat(PCTSTR szFormat, ...);
//
// REMARKS:
//		This function allows the caller for format and return a CString object with a single line
//		of code.  Frequently you want to print out a formatted string but don't care about it once
//		you are done with it.  You end up having to create temporary CString objects and then
