...
//
// @doc
// @module TRiASMICursorProperty.cpp | Implementation of the <c CTRiASMICursorProperty> class
...
// 'ObjectsCursor' oder 'FeatureCursor' eines Objektes (CTRiASMIObject, 
// CTRiASMIObjects bzw. CTRiASMIFeature) die dafür sorgt, daß in 
...
// (CTRiASMIObject) zuordnen zu können.
...
// bzw. 'FeatureMap') an der zugehörigen Datenbank (CTRiASMIDatabase) abgelegt. 
//
// Für andere Datenquellen ist es u.U. notwendig diese Tabellen z.B. in Bezug 
// zur Objektklasse (CTRiASMIObjects) oder evtl. zum zugehörigen Objekt 
// (CTRiASMIObject) zu speichern. Die entsprechende Modifikation ist dann von
// Hand auszuführen.
// 
// Die Verwendung einer GenObjectMap (s. TRiASMIDatabase.h: BEGIN_PROPERTYSUPPORT_MAP)
...
STDMETHODIMP CTRiASMICursorProperty::PutType (BSTR Name, PROPERTY_TYPE newVla)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
	_ASSERTE(NULL != Name);
	_ASSERTE(!wcscmp (Name, m_bstrName));		// muß voreingestellter Name sein
...
STDMETHODIMP CTRiASMICursorProperty::Refresh(BSTR bstrName)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
	return E_NOTIMPL;
}
...
///////////////////////////////////////////////////////////////////////////////
// IObjectWithSite
STDMETHODIMP CTRiASMICursorProperty::SetSite (IUnknown *pISite)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
	if (NULL != pISite)
		m_Obj = pISite;
...
STDMETHODIMP CTRiASMICursorProperty::GetSite (REFIID riid, void **ppvSite)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
	if (!m_Obj) 
...
// IObjectWithName
STDMETHODIMP CTRiASMICursorProperty::put_Name (BSTR bstrName)
{
	m_bstrName = bstrName;
	return S_OK;
}

STDMETHODIMP CTRiASMICursorProperty::get_Name (BSTR *pbstrName)
{
	if (NULL == pbstrName)
		return E_POINTER;
...
HRESULT CTRiASMICursorProperty::GetCursorMap (ITRiASObjectHandleMap **ppIMap)
{
	if (!m_Obj.IsValid())
		return TRIASDB_E_PROPERTY_NOT_INITIALIZED;

	COM_TRY {
	// geht davon aus, daß die Map am CTRiASMIDatabase/CTRiASMIObjects -Objekt als Property abgelegt ist
	WDispatch Parent;
...
HRESULT CTRiASMICursorProperty::SetCursor()
{
	COM_TRY {
...
HRESULT CTRiASMICursorProperty::ResetCursor()
{
	if (!m_Obj)
