...
BEGIN_MESSAGE_MAP(CZoomComboBoxSec, SECComboBtn)
...
// ComboBox initialisieren, Fonts setzen etc.
	if(!SECComboBtn::CreateWnd (pParentWnd, dwStyle|WS_TABSTOP|WS_DISABLED, rect, nID)) 
		return FALSE;
...
{
	SECComboBtn::OnWindowPosChanging(lpwndpos);
...
// MaßstabsComboBox füllen
	...
	CString str;
		...
		TRY { 
			str.LoadString(IDS_MASSTAB250+i);
		} CATCH_ALL(e) {
			continue;
		} END_CATCH_ALL
		AddString (str);
