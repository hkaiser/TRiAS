/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imrla.c,v 1.23 1995/06/30 22:09:41 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imrla.c,v 1.23 1995/06/30 22:09:41 bduggan Exp $"
...
{
	{ IM_SHORT_TYPE, 2, 1 },		/* Window boundaries		*/
	{ IM_SHORT_TYPE, 2, 1 },
	{ IM_SHORT_TYPE, 2, 1 },
	{ IM_SHORT_TYPE, 2, 1 },
	{ IM_SHORT_TYPE, 2, 1 },		/* Active window boundaries	*/
	{ IM_SHORT_TYPE, 2, 1 },
	{ IM_SHORT_TYPE, 2, 1 },
	{ IM_SHORT_TYPE, 2, 1 },
	{ IM_SHORT_TYPE, 2, 1 },		/* Frame			*/
	{ IM_SHORT_TYPE, 2, 1 },		/* Storage type 		*/
	{ IM_SHORT_TYPE, 2, 1 },		/* Number of image channels	*/
	{ IM_SHORT_TYPE, 2, 1 },		/* Number of matte channels	*/
	{ IM_SHORT_TYPE, 2, 1 },		/* Number of aux channels	*/
	{ IM_SHORT_TYPE, 2, 1 },		/* Aux channel type mask	*/
	{ IM_CHAR_TYPE, 1, IMRLANGAMMA },	/* Image storage gamma		*/
	{ IM_CHAR_TYPE, 1, IMRLANPRI },		/* Image red primary chromaticity */
	{ IM_CHAR_TYPE, 1, IMRLANPRI },		/* Image grn primary chromaticity */
	{ IM_CHAR_TYPE, 1, IMRLANPRI },		/* Image blu primary chromaticity */
	{ IM_CHAR_TYPE, 1, IMRLANPRI },		/* Image whi primary chromaticity */
	{ IM_INT_TYPE, 4, 1 },			/* Job number			*/
	{ IM_CHAR_TYPE, 1, IMRLANNAME },	/* Original file name 		*/
	{ IM_CHAR_TYPE, 1, IMRLANNAME },	/* File description 		*/
	{ IM_CHAR_TYPE, 1, IMRLANPROG },	/* Creating program name	*/
	{ IM_CHAR_TYPE, 1, IMRLANSTR },		/* Creating machine name	*/
	{ IM_CHAR_TYPE, 1, IMRLANSTR },		/* Creating user name	 	*/
	{ IM_CHAR_TYPE, 1, IMRLANDATE },	/* Creation date 	 	*/
	{ IM_CHAR_TYPE, 1, IMRLANASPECT },	/* Image aspect type  	 	*/
	{ IM_CHAR_TYPE, 1, IMRLANAR },		/* Image aspect ratio  	 	*/
	{ IM_CHAR_TYPE, 1, IMRLANSTR },		/* Image color space type 	*/
	{ IM_SHORT_TYPE, 2, 1 },		/* Rendered on fields flag	*/
	{ IM_SHORT_TYPE, 2, 1 },		/* Post filtered interleaved flag*/
	{ IM_INT_TYPE, 4, 1 },			/* unused                 	*/
	{ IM_INT_TYPE, 4, 1 },			/* unused                 	*/
	{ IM_INT_TYPE, 4, 1 },			/* unused                 	*/
	{ IM_INT_TYPE, 4, 1 },			/* unused                 	*/
	{ IM_SHORT_TYPE, 2, 1 },		/* unused                 	*/
	{ IM_SHORT_TYPE, 2, 1 },		/* unused                 	*/
	{ IM_SHORT_TYPE, 2, 1 },		/* unused                 	*/
	{ IM_SHORT_TYPE, 2, 1 },		/* unused                 	*/
	{ IM_CHAR_TYPE, 1, IMRLANSPACE },	/* Unused expansion space 	*/
	{ 0, 0, 0 }
};
...
//	int	     status;		/* Status returned from seek	*/
	...
	if ( ImBinRead( ioType, fd, fp, offsetTable, IM_INT_TYPE, 4, y ) == -1 )
	{
	...
	for( i=0; i<y; i++ )
	{
		...
		for ( j = 0; j < dep; j++ )
		{
			if (ImBinRead(ioType, fd, fp, &len, IM_SHORT_TYPE, 2, 1 ) == -1)
			{
			...
			if (ImBinRead(ioType, fd, fp, buf, IM_UCHAR_TYPE, 1, len )== -1)
			{
				imRlaFrBuf( redBuf, grnBuf, bluBuf, alpBuf,
					buf, offsetTable );
				ImReturnBinError( );
...
//     	strncpy( header.user, name, IMRLANSTR );
     	...
     	strncpy( header.user, "imtools", IMRLANSTR );
...
//	gethostname( header.machine, IMRLANSTR );
	strcpy (header.machine, "imtoolshost");
	...
	if ( ImBinWrite( ioType, fd, fp, offsetTable, IM_INT_TYPE, 4, y ) == -1 )
	{
		free( (char *)offsetTable );
		ImReturnBinError( );
	}
...
fprintf(stderr,"\n");
			...
			if (ImBinWrite(ioType, fd, fp, &len, IM_SHORT_TYPE, 2, 1 )== -1)
			{
				imRlaFrBuf( redBuf, grnBuf, bluBuf, alpBuf, buf,
					offsetTable );
				ImReturnBinError( );
			}
			if (ImBinWrite(ioType, fd, fp, buf, IM_UCHAR_TYPE, 1, len)== -1)
			{
				imRlaFrBuf( redBuf, grnBuf, bluBuf, alpBuf, buf,
					offsetTable );
				ImReturnBinError( );
