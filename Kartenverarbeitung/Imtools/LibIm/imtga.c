/**
 **     $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imtga.c,v 1.17 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER  "    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imtga.c,v 1.17 1995/06/29 00:28:04 bduggan Exp $"
...
{
	{ IM_UINT_TYPE, 1, 1   },	/* tga_numc_ident		*/
	{ IM_USHORT_TYPE, 1, 1 },       /* tga_cmtype 			*/
	{ IM_UINT_TYPE, 1, 1   },	/* tga_stype 			*/
	{ IM_UINT_TYPE, 2, 1   },	/* tga_cmorigin			*/
	{ IM_UINT_TYPE, 2, 1   },	/* tga_cmlen			*/
	{ IM_UINT_TYPE, 1, 1   },	/* tga_cmsize			*/
	{ IM_UINT_TYPE, 2, 1   },	/* tga_imxorg			*/
	{ IM_UINT_TYPE, 2, 1   },	/* tga_imyorg			*/
	{ IM_UINT_TYPE, 2, 1   },	/* tga_imwid			*/
	{ IM_UINT_TYPE, 2, 1   },	/* tga_imlen			*/
	{ IM_UINT_TYPE, 1, 1   },	/* tga_spix			*/
	{ IM_UINT_TYPE, 1, 1   },       /* tga_imdesc			*/
	{ 0, 0, 0}
};
...
//	ImVfbPtr pPixel;		/* VFB pixel pointer		*/
	...
	if (header.tga_numc_ident)
	{
	   ...
	   ImBinRead (ioType, fd, fp, TgaName, IM_UCHAR_TYPE,1, header.tga_numc_ident);
	...
	/* there is however a variable number of bytes per entry */
	   switch (m)
	   {
		case 16:		/* ARRRRRGG GGGBBBBB */
		   ...
		   if (ImBinRead (ioType,fd,fp,buffer, IM_UCHAR_TYPE, 1, n*2)== -1)
		   {
			free ( (char *)buffer);
			ImReturnBinError();
		   }
		...
		case 24:		/* BBBBBBBB GGGGGGGG RRRRRRRR */
		   ...
		   if (ImBinRead (ioType,fd,fp,buffer, IM_UCHAR_TYPE, 1, n*3)== -1)
		   {
			free ( (char *)buffer);
			ImReturnBinError();
		   }
		...
		case 32:		/* BBBBBBBB GGGGGGGG RRRRRRRR AAAAAAAA*/
		   ...
		   if (ImBinRead (ioType,fd,fp,buffer, IM_UCHAR_TYPE, 1, n*4)== -1)
		   {
			free ( (char *)buffer);
			ImReturnBinError();
		   }
	...
	if (header.tga_numc_ident)
	{
		TagTableAppend (tagTable,
			TagEntryAlloc("image name",IM_POINTER_TYPE,&TgaName));
	}

	/* Attach vfb to tag table */
	TagTableAppend (tagTable, TagEntryAlloc ("image vfb", IM_POINTER_TYPE, &vfb));
	return(1);
}
...
#endif
{
	...
	unsigned char *pBuffer;	/* buffer pointer		*/
		...
		if (q==8)   /* one byte per index */
		{
		   ...
		   for (y =0; y<h; y++)
		   {
		      if (ImBinRead (ioType,fd,fp,buffer, IM_UCHAR_TYPE, 1,w) == -1)
		      {
			free ( (char *)buffer);
			ImReturnBinError();
		      }
		...
		else	    /*two bytes per index */
		{
		   ...
		   for (y =0; y<h; y++)
		   {
		      if (ImBinRead (ioType,fd,fp,bufferl,IM_UINT16_TYPE,2,w) == -1)
		      {
...
#endif
{
	...
	unsigned char *pBuffer;	/* buffer pointer	*/
		  switch (q)
		  {
		   ...
		   for (y =0; y<h; y++)
		   {
		      if (ImBinRead (ioType,fd,fp,buffer,IM_UCHAR_TYPE,1,2*w) == -1)
		      {
			free ( (char *)buffer);
			ImReturnBinError();
		      }
		   ...
		   for (y =0; y<h; y++)
		   {
		      if (ImBinRead (ioType,fd,fp,buffer,IM_UCHAR_TYPE,1,3*w) == -1)
		      {
			free ( (char *)buffer);
			ImReturnBinError();
		      }
		    ...
		    for (y =0; y<h; y++)
		    {
		      if (ImBinRead (ioType,fd,fp,buffer,IM_UCHAR_TYPE,1,4*w) == -1)
		      {
			free ( (char *)buffer);
			ImReturnBinError();
		      }
...
#endif
{
	...
	switch (q)
	{
	  case 8:		/* one byte per pixel	*/
	   ...
	   while (total>0) {
		if (ImBinRead (ioType,fd,fp,buffer,IM_UCHAR_TYPE,1,1) == -1)
		{
		   free ( (char *)buffer);
		   ImReturnBinError();
		}
		...
		pBuffer = buffer;
		   ...
		   {
		        if (ImBinRead (ioType,fd,fp,buffer,IM_UCHAR_TYPE,1,count) == -1)
		        {
		           free ( (char *)buffer);
		           ImReturnBinError();
		        }
	  ...
	  case 16:		/* two bytes per pixel	*/
	   ...
	   while (total>0 ) {
		if (ImBinRead (ioType,fd,fp,bufferl,IM_UINT16_TYPE,2,1) == -1)
		{
		   free ( (sdsc_uint16 *)bufferl);
		   ImReturnBinError();
		}
		...
		pBufferl = bufferl;
		   ...
		   leadByte = (unsigned char)(*pBufferl++);
		   rle = (leadByte&0x80);	/* rle = first bit   */
		   count = (leadByte&0x7F)+1;  /*count = last 7 bytes +1 */
		   ...
		   {
		        if (ImBinRead (ioType,fd,fp,bufferl,IM_UINT16_TYPE,2,count) == -1)
		        {
		           free ( (sdsc_uint16 *)bufferl);
		           ImReturnBinError();
		        }
	...
	ImMalloc (buffer, unsigned char *, 260);
	   ...
	   while (total>0)  {
		...
		pBuffer = buffer;
		   ...
		   if (rle)	/* run length encoded packet */
		   {
		        if (ImBinRead (ioType,fd,fp,buffer,IM_UCHAR_TYPE,1,2) == -1)
		        {
		           free ( (char *)buffer);
		           ImReturnBinError();
		        }
		   ...
		   {
		        if (ImBinRead (ioType,fd,fp,buffer,IM_UCHAR_TYPE,1,count*2) == -1)
		        {
		           free ( (char *)buffer);
		           ImReturnBinError();
		        }
	  ...
	  case 24:		/*three bits per pixel	*/
	   ...
	   while (total >0 ) {
		...
		pBuffer = buffer;
		   ...
		   if (rle)	/* run length encoded packet */
		   {
		        if (ImBinRead (ioType,fd,fp,buffer,IM_UCHAR_TYPE,1,3) == -1)
		        {
		           free ( (char *)buffer);
		           ImReturnBinError();
		        }
		   ...
		   {
		        if (ImBinRead (ioType,fd,fp,buffer,IM_UCHAR_TYPE,1,count*3) == -1)
		        {
		           free ( (char *)buffer);
		           ImReturnBinError();
		        }
	  ...
	  case 32:		/* four bits per pixel	*/
	   ...
	   while (total > 0)  {
		...
		pBuffer = buffer;
		   ...
		   if (rle)	/* run length encoded packet */
		   {
		        if (ImBinRead (ioType,fd,fp,buffer,IM_UCHAR_TYPE,1,4) == -1)
		        {
		           free ( (char *)buffer);
		           ImReturnBinError();
		        }
		   ...
		   {
		        if (ImBinRead (ioType,fd,fp,buffer,IM_UCHAR_TYPE,1,count*4) == -1)
		        {
		           free ( (char *)buffer);
		           ImReturnBinError();
		        }
...
#define imTgaDumpVarRun(bytesPerPixel, buffer, len)	\
	...
	imTgaRLEBuf[0] &= 0x7f;                         \
	if ( ImBinWrite( ioType, fd, fp, imTgaRLEBuf, IM_UCHAR_TYPE, 1, 1)==-1) \
	{                                               \
		ImReturnBinError( );                    \
	}						\
	if ( ImBinWrite( ioType, fd, fp, buffer, IM_UCHAR_TYPE, 1, bytesPerPixel * (len+1))==-1) \
	{                                               \
		ImReturnBinError( );                    \
	}
...
#define imTgaDumpConstantRun1(index,len)		\
	...
	imTgaRLEBuf[0] |= 0x80;				\
	imTgaRLEBuf[1] = index;                         \
	if ( ImBinWrite( ioType, fd, fp, imTgaRLEBuf, IM_UCHAR_TYPE, 1, 2)==-1) \
	{                                                           \
		ImReturnBinError( );                                \
	}
...
#define imTgaDumpConstantRun2(index16,len)		\
	...
	if ( ImBinWrite( ioType, fd, fp, imTgaRLEBuf, IM_UCHAR_TYPE, 1, 3)==-1) \
	{                                                           \
		ImReturnBinError( );                                \
	}
...
#define imTgaDumpConstantRun3(b,g,r,len)		\
	...
	imTgaRLEBuf[2] = g;                             \
	imTgaRLEBuf[3] = r;                             \
	if ( ImBinWrite( ioType, fd, fp, imTgaRLEBuf, IM_UCHAR_TYPE, 1, 4)==-1) \
	{                                                           \
		ImReturnBinError( );                                \
	}
...
#define imTgaDumpConstantRun4(b,g,r,a,len)		\
	...
	imTgaRLEBuf[3] = r;                             \
	imTgaRLEBuf[4] = a;                             \
	if ( ImBinWrite( ioType, fd, fp, imTgaRLEBuf, IM_UCHAR_TYPE, 1, 5)==-1) \
	{                                                           \
		ImReturnBinError( );                                \
	}
...
//	char	    *tmpString;		/* Tmp string holder		*/
	...
	for (j=0; j<y; j++)
	{
		...
		if (ImBinWrite (ioType, fd, fp, buffer, IM_UCHAR_TYPE,1,pBuffer - buffer) ==-1)
		{
			free ( (char *)buffer);
			ImReturnBinError( );
		}
...
//	TagEntry    *tagEntry;		/* Tmp table entry		*/
	...
	for (j=0; j<y; j++)
	{
		...
		if (ImBinWrite (ioType, fd, fp, bufferl, IM_UCHAR_TYPE, 1, x*2 ) ==-1)
		{
			free ( bufferl);
			ImReturnBinError( );
		}
...
//	char	    *tmpString;		/* Tmp string holder		*/
	...
	unsigned char	    byte1,byte3,byte4;/* Bytes to write from rle packet */
	...
	/* see if you should rle something	*/
		...
		*pBuffer++ = byte3;
		*pBuffer++ = byte4;
		if (ImBinWrite (ioType,fd,fp,bufferRle,IM_UCHAR_TYPE,1,3)==-1)
		{
		   free ( (char *) bufferRle);
		   ImReturnBinError( );
		}
		   ...
		   pBuffer = bufferCount;
		   *pBuffer = byte1;
		   if (ImBinWrite (ioType,fd,fp,bufferCount,IM_UCHAR_TYPE,1,1)==-1)
		   {
		      free ( (char *) bufferCount);
		      ImReturnBinError( );
		   }
		   if (ImBinWrite (ioType,fd,fp,bufferUn,IM_UCHAR_TYPE,1,2*(count+1))==-1)
		   {
		      free ( (char *) bufferUn);
		      ImReturnBinError( );
		   }
...
//	ImVfbPtr     pptr;		/* Pixel pointer		*/
	...
	if (header.tga_numc_ident)  /* if there is a name to write */
	  ...
	  ImBinWrite (ioType, fd, fp, tmpString, IM_UCHAR_TYPE,1,header.tga_numc_ident);
