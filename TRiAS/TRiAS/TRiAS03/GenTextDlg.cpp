...
#if !defined(_TRIAS_HAS_RELATIONS)
	m_cbDependend.Disable();
#endif // !defined(_TRIAS_HAS_RELATIONS)
...
// die folgenden beiden erst ab TRiASV4.x aktiviert
	m_cbRotate.Hide();
...
// Optionen initialisieren
	...
	int iIndex = -1;
		...
		if (m_pStatus -> m_strDirectText.size() > 0)
			iIndex = m_cbDirectText.AddString (m_pStatus -> m_strDirectText.c_str());
		if (iIndex >= 0) 
			m_cbDirectText.ChangeCurrentIndex(iIndex);
...
// einem evtl. nachfolgenden Objekt TRiASEx.ModifyIdent.1 mitteilen,
// ob Datenquellen mit angezeigt werden sollen
...
WDataObject CtxDO;
	...
	FORMATETC c_feProgID = 	{
		...
		wsprintf(cbBuffer, "ShowConnections=%d", m_pStatus -> m_fDependend ? 0 : 1);
		SetVariantData(CComVariant(cbBuffer), CtxDO, &c_feProgID);
	}
}
...
namespace {
	...
	public:
		CCreateTextBase (CGenTextDlg::CDlgStatus &rStatus, HPROJECT hPr, /*JG*/CPropertyActionExtUsage* pPAUse = NULL) : 
			m_rStatus(rStatus), m_hPr(hPr)
		{
			m_pPAUse = pPAUse;
		}
...
// Text über festen Text (evtl. dynamisch) erzeugen
	...
	public:
		CCreateTextDirect (CGenTextDlg::CDlgStatus &rStatus, os_string str, CPropertyActionExtUsage* pPAUse, HPROJECT hPr)
			: CCreateTextBase(rStatus, hPr, pPAUse), m_strText(str)
		{
		}
...
// Text über Objekteigenschaft (statisch) erzeugen
	...
	public:
		CCreateTextFromObjProp (IObjectProperty *pIObjProp, CGenTextDlg::CDlgStatus &rStatus, 
				CPropertyActionExtUsage* pPAUse, HPROJECT hPr) :
			CCreateTextDirect(rStatus, os_string(g_cbNil), pPAUse, hPr), m_ObjProp(pIObjProp)
		{
		}
...
HRESULT CGenTextDlg::GetEnumLONG (HPROJECT hPr, IEnum<LONG> **ppIEnumObjs)
{
	...
	*ppIEnumObjs = NULL;
	...
	ULONG counter = m_Objects.Count();
		...
		WObjectProperty ObjProp;
			...
			// dynamische Textobjekte erzeugen
				...
				// nur für existierende OProps erzeugen
					transform(InEnumLONG(&m_Objects), InEnumLONG(), iter_out_if, CCreateTextFromObjProp(ObjProp, *m_pStatus, this/*JG*/, hPr));
				} 
				else {
				// alle Objekte (nicht nur für existierende OProps) erzeugen
					transform(InEnumLONG(&m_Objects), InEnumLONG(), iter_out_if, CCreateTextDirect(*m_pStatus, m_pStatus -> m_strObjProp, this/*JG*/, hPr));
				}
			} 
			else {
		...
		// für alle Objekte einen Text erzeugen
			transform(InEnumLONG(&m_Objects), InEnumLONG(), iter_out_if, CCreateTextDirect(*m_pStatus, m_pStatus -> m_strDirectText, this/*JG*/, hPr));
		}
...
HPROJECT hPr = m_hPr;
	...
	if (m_rStatus.m_fDependend)
		hPr = DEX_GetObjectProject(lONr);	// abhängige Textobjekte beim Bezugsobjekt erzeugen

	if (fDynamic && DEX_GetDBVersionEx(hPr) <= VERSION0500000A)
		fDynamic = false;
...
char cbBuffer[64];
long lUIMCode = DEX_GetUniqueIdentMCodeEx (DEX_GetObjectProject(TG.lONr));
...
HRESULT fResult = S_OK;
	...
	if (NULL != plIdent) {
		if (HACTCONNECTION == m_hPr)
			*plIdent = OG.lIdent;	// Objektklasse des Bezugsobjektes verwenden, wenn nichts anderes gegeben
		else
			*plIdent = 0L;			// ansonsten im m_hPr unbekannte Objektklasse verwenden
	}
	return fResult;
}
...
{
HPROJECT hPrP = DEX_GetObjectProject(lONr);
HPROJECT hPrT = DEX_GetObjectProject(lTxtONr);

	if (hPrP != hPrT || !DEX_GetTRiASDataSourceEx(hPrP))
		return S_FALSE;		// nicht für dieses Objekt (andere oder keine TRiAS Datenquelle)
...
{
	if (NULL != plIdent)
		*plIdent = DEX_GetObjIdentEx(lTxtONr);
...
// PI weiterschalten
	...
	HRESULT hr = m_ObjProp -> Eval (lONr, cbBuffer, sizeof(cbBuffer), NULL);
		...
		if (m_rStatus.m_fDynamic) {
			...
			if (DEX_GetDBVersionEx(DEX_GetObjectProject(lONr)) < VERSION0500000A) {
			// in alten Datenbanken leider noch nicht implementiert
