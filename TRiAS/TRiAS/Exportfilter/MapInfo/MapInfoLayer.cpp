...
// Created: 23.01.2001 12:47:45 
//
// @doc
// @module MapInfoLayer.cpp | Definition of the <c CMapInfoLayer> class
...
HRESULT CMapInfoLayer::FieldExists (LPCSTR pcName, DBFFieldType rgType, int *piField)
{
...
bool CMapInfoLayer::IsUniqueName (os_string &rStr)
{
...
os_string CMapInfoLayer::GetUniqueFieldName (LPCSTR pcName)
{
...
HRESULT CMapInfoLayer::AddField(LPCSTR pcName, DBFFieldType rgTyp, int iLen, int iDecimal, int *piField)
{
	if (NULL == piField)
		return E_POINTER;
	*piField = -1;
...
HRESULT CMapInfoLayer::ExportData (
	GSTRUCT *pGS, MFELD *pMF, LPCSTR pcUIdent, CMapInfoLayerAttributes *pMap)
{
...
// Geometrie erzeugen
	...
	// Objekt erzeugen
	CMapInfoObject Obj(SHPCreateObject(m_nShapeType, -1, iKCnt, Cnts.begin(), NULL,
	...
	// Objekt erzeugen
	CMapInfoObject Obj(SHPCreateSimpleObject(m_nShapeType, pGS -> GSize, pGS -> x, pGS -> y, NULL));
...
// Attribute sicherstellen (bei TRiAS-Datenquellen jedesmal)
	...
	// sämtliche Attribute dieses Layers durchgehen und ggf. erzeugen
		for (CMapInfoLayerAttributes::iterator it = pMap -> begin(); it != pMap -> end(); ++it) {
		// Feld in Datei erzeugen
		int iField = -1;
		CMapInfoAttribute &rAttr = (*it).second;
	...
	// Textlabel für Textobjekte
		...
		pair<CMapInfoLayerAttributes::iterator, bool> p = 
			pMap -> insert (CMapInfoLayerAttributes::value_type (-1, CMapInfoAttribute(g_cbLabelText, 'a', _MAX_PATH)));
...
// Attributwerte schreiben
	...
	// nur, wenn mindestens ein Attribut ausgegeben werden soll
		for (MFELD *pMFT = pMF; 0 != pMFT -> MCode; ++pMFT) {
			if (NULL != pMap) {
			CMapInfoLayerAttributes::iterator it = pMap -> find (pMFT -> MCode);
...
OBJECTTYPE CMapInfoLayer::GetType() 
{
...
HRESULT CMapInfoLayer::CloseAndDeleteFiles()
{
