...
/////////////////////////////////////////////////////////////////////////////
// Ersatzobjekte für CMapToPtr etc.
typedef map<os_string, DWORD, less<os_string> > CNameDictMap;

class CNameDict : public CNameDictMap {
public:
		CNameDict(int nBlockSize = 10) {}
		~CNameDict() {}

	int GetCount() const { return CNameDictMap::size(); }
	void SetAt(CString &strKey, DWORD newValue)
		{ 
		os_string str = (LPCSTR)strKey;

			CNameDictMap::insert(str, newValue); 
		}
	BOOL Lookup (CString &strKey, DWORD &rValue)
		{
		os_string str = (LPCSTR)strKey;
		CNameDictMap::iterator it = CNameDictMap::find(str);

			if (it == CNameDictMap::end()) return false;
			rValue = (*it).second;
			return true;
		}
};

typedef list<class CProperty *> CPropList;
typedef list<class CPropertySection *> CSectionList;
...
// Attributes
	...
	DWORD   GetCount( void ) ;
	CSectionList* GetList( void ) ;

// Operations
	HRESULT WriteToStream( IStream* pIStream ) ;
	HRESULT ReadFromStream( IStream* pIStream ) ;

// Implementation
private:
	PROPHEADER      m_PH ;
	CSectionList	m_SectionList ;
