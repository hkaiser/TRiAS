/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/immpnt.c,v 1.17 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER  "    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/immpnt.c,v 1.17 1995/06/29 00:28:04 bduggan Exp $"
...
#endif
{
	 ...
	 */
	BinByteOrder( BINMBF );
	if ( ImBinRead( ioType, fd, fp, packed, IM_UCHAR_TYPE, 1, IMMAC_HEADER_SIZE ) == -1 )
	{
		ImReturnBinError();
	}
	 ...
	 */
	if ( (byteCount = ImBinRead( ioType, fd, fp, packed, IM_UCHAR_TYPE, 1,  IMMAC_MAX_UNPACKED_SIZE )) == (unsigned long)-1 )
	{
		ImReturnBinError();
	}
...
//	int		mono_threshold;
	...
	if ( ImBinWrite( ioType, fd, fp, buf, IM_UCHAR_TYPE, 1, IMMAC_HEADER_SIZE ) == -1 )
	{
		ImReturnBinError();
	}
	...
	for ( sLine=0; sLine<vfbHeight; sLine++ )
	{
		...
		if ( ImBinWrite( ioType, fd, fp, packed, IM_UCHAR_TYPE, 1, count )== -1)
		{
			ImReturnBinError();
		}
	}
