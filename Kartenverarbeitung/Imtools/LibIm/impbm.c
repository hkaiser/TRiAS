/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/impbm.c,v 1.15 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/impbm.c,v 1.15 1995/06/29 00:28:04 bduggan Exp $"
 ...
 **
 **  DESCRIPTION
 **	impbm.c contains routines to _read and write the various PBM suite
 **	image files for the image manipulation library.  Raster data _read
 ...
 **
 **	readAsciiInt	f  _read in an ASCII string integer
 **
 **  HISTORY
 **	$Log: impbm.c,v $
 ...
 **	made _read/write routines static
 **
 ...
 *
 *  All 8 formats share the same _read call.  Some share the same write calls.
 */
#ifdef __STDC__
...
/*
 *  FUNCTION
 *	imPbmRead	-  _read a PBM file
 *
 *  DESCRIPTION
 *	The header is _read in, followed by the image pixels in either
 *	ASCII or raw format.
 */
...
#endif
{
	...
	if ( ImBinRead( ioType, fd, fp, magic, IM_UCHAR_TYPE, 1, 2 ) == -1 )
		ImReturnBinError( );
	...
	/*
	 *  Except for PBM and RPBM formats, _read in the maximum channel
	 *  value 'maxIntensity'.
	 *
	...
	if ( magic[1] != IMPBM_MAGIC2 && magic[1] != IMRPBM_MAGIC2 )
	{
		if ( readAsciiInt( ioType, fd, fp, &maxIntensity ) == -1 )
			return ( -1 );	/* Error already handled	*/
		colorScale = (float)(255.0 / (float) maxIntensity);
	}
	...
	case IMRPPM_MAGIC2:	/* Raw Color				*/
		...
		for (y = 0; y < height; y++)
		{
			if ( ImBinRead( ioType, fd, fp, buffer, IM_UCHAR_TYPE, 1, width * 3 ) == -1 )
			{
				free( (char *)buffer );
				ImReturnBinError( );
			}
	...
	case IMRPGM_MAGIC2:	/* Raw grayscale			*/
		...
		for (y = 0; y < height; y++)
		{
			if ( ImBinRead( ioType, fd, fp, buffer, IM_UCHAR_TYPE, 1, width ) == -1 )
			{
				free( (char *)buffer );
				ImReturnBinError( );
			}
	...
	case IMRPBM_MAGIC2:	/* Raw bitmap (mono)			*/
		...
		{
			bitShift = -1;
			if ( ImBinRead( ioType, fd, fp, buffer, IM_UCHAR_TYPE, 1, (width + 7) / 8 ) == -1 )
			{
				free( (char *)buffer );
				ImReturnBinError( );
			}
 ...
 *	readAsciiInt	-  _read in an ASCII string integer
 *
 *  DESCRIPTION
 *	White space and comments are skipped up to the first digit of a number.
 *	The number is _read in and returned.  If a _read problem occurs, a -1
...
imPbmWrite1( pMap, ioType, fd, fp, flagsTable, tagTable )
	...
	TagTable *tagTable;		/* Tag list to _read from	*/
#endif
{
