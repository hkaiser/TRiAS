/**
 **     $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imbmp.c,v 1.14 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER  "    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imbmp.c,v 1.14 1995/06/29 00:28:04 bduggan Exp $"
...
4- and 8-bit RLE compressed images.",			/* Read support	*/
	"1-, 4-, and 8-bit color index, and 24-bit RGB color, and\n\
4- and 8-bit RLE compressed images.",			/* Write support*/
	imBmpMagic,					/* Magic #'s	*/
...
{
	{ IM_UINT32_TYPE, 4, 1 },		/* bmp_size			*/
	{ IM_UINT16_TYPE, 2, 1 },		/* bmp_reserved1		*/
	{ IM_UINT16_TYPE, 2, 1 },		/* bmp_reserved2		*/
	{ IM_UINT32_TYPE, 4, 1 },		/* bmp_offsetbits		*/
	{ IM_UINT32_TYPE, 4, 1 },		/* bmp_bisize			*/
	{ IM_UINT32_TYPE, 4, 1 },		/* bmp_biwidth			*/
	{ IM_UINT32_TYPE, 4, 1 },		/* bmp_biheight			*/
	{ IM_UINT16_TYPE, 2, 1 },		/* bmp_biplanes			*/
	{ IM_UINT16_TYPE, 2, 1 },		/* bmp_bibitcount		*/
	{ IM_UINT32_TYPE, 4, 1 },		/* bmp_bicompress		*/
	{ IM_UINT32_TYPE, 4, 1 },		/* bmp_bisizeimage		*/
	{ IM_UINT32_TYPE, 4, 1 },		/* bmp_bixpm			*/
	{ IM_UINT32_TYPE, 4, 1 },		/* bmp_biypm			*/
	{ IM_UINT32_TYPE, 4, 1 },		/* bmp_biclrused		*/
	{ IM_UINT32_TYPE, 4, 1 },		/* bmp_biclrim			*/
	{ 0, 0, 0 }
};
...
{
	{ IM_UCHAR_TYPE, 1, 1 },		/* blue				*/
	{ IM_UCHAR_TYPE, 1, 1 },		/* green			*/
	{ IM_UCHAR_TYPE, 1, 1 },		/* red				*/
	{ IM_UCHAR_TYPE, 1, 1 },		/* reserved			*/
	{ 0, 0, 0 }
};
...
{									\
	if( ImBinRead( ioType,fd,fp,buf,IM_UCHAR_TYPE,1,IMBLKSIZE )==-1)		\
	{								\
		ImReturnBinError( );					\
	}								\
	bufin = 0;							\
...
#endif
{
	 ...
	 */
	BinByteOrder( BINLBF );
	if( ImBinRead( ioType, fd, fp, &bmp_type, IM_UINT16_TYPE, 2, 1) == -1 )
	{
		ImReturnBinError( );
	}
	...
	if( header.bmp_bibitcount != 24)
	{
		...
		rbufptr = rbuf;
		if( ImBinRead( ioType, fd, fp, rbuf, IM_UCHAR_TYPE, 1,
	...
	if( skipsize > 0 )
	{
		...
		else
		{
			...
			if( ImBinRead( ioType, fd, fp, sbuf, IM_UCHAR_TYPE, 1,
				skipsize ) == -1 )
			{
				free( (char*) sbuf );
				ImReturnBinError( );
	...
	if( header.bmp_bicompress == IMBMPRGB )	/* no compression */
	{
		...
		case 1:
			...
			for( i = 0; (unsigned int)i < y ; ++i )
			{
			    if( ImBinRead( ioType, fd, fp,
				 rbuf, IM_UCHAR_TYPE, 1, scanlinesize ) == -1)
			    {
				ImReturnBinError( );
			    }
			    rbufptr = rbuf;
			    
			    for( j = 0; (unsigned int)j < (x/8); ++j )
			    {
				 mask = 128;
				 for( k = 0; (unsigned int)k < 8; ++k )
				 {
...
#endif
{
	...
	for( i = 0; (unsigned int)i < y; ++i )
	{
		...
		if( ImBinWrite( ioType,fd,fp,wbuf,IM_UCHAR_TYPE,1,scanlinesize ) == -1 )
		{
			ImReturnBinError( );
		}
...
//	imBmpRGBQuad   cltentry;	/* Colortable RGBQuad entry	*/
	...
	wbufptr = wbuf;
	for( i = 0; (unsigned int)i < ncolors; ++i )
	{
	...
	if ( ImBinWrite( ioType, fd, fp, wbuf, IM_UCHAR_TYPE, 1,
		ncolors*IMBMPRGBQUADSIZE ) == -1 )
	{
		free( wbuf );		// #HK000327
		ImReturnBinError( );
...
{									\
	...
	if( bufin > (IMBLKSIZE-1) )					\
	{								\
		if( ImBinWrite( ioType,fd,fp,buf,IM_UCHAR_TYPE,1,IMBLKSIZE )==-1)	\
		{							\
			ImReturnBinError( );				\
		}							\
		bufin = 0;						\
...
#endif
{
	...
	if ( ImBinWrite( ioType, fd, fp, cbuf, IM_UCHAR_TYPE, 1,
		ncolors*IMBMPRGBQUADSIZE ) == -1 )
	{
		free( cbuf );		// #HK000327
		ImReturnBinError( );
	...
	while( (unsigned int)ycount < y )
	{
		 ...
		 */
		if( (pixel != ImVfbQIndex8( vfb, vfbptr ))
			&& ((unsigned int)xcount < (x-2) )
			&& ((unsigned int)ycount < y) )
		{
			run = 0;
			nowrite = TRUE;
			...
			while( pixel != ImVfbQIndex8( vfb, vfbptr ) )
			{
				...
				if( (unsigned int)xcount == x-1 )
				{
					 ...
					 */
					count = 0;
					for( i = 0; (unsigned int)i < run+1; i += 2)
					{
	...
	buf[ bufin ] = 1;
	++bufin;
	if( ImBinWrite( ioType,fd,fp,buf,IM_UCHAR_TYPE,1,bufin )==-1)
	{
		ImReturnBinError( );
	}
	...
	header.bmp_size = IMBMPHEADERSIZE + ( ncolors * IMBMPRGBQUADSIZE )
		+ bytecount;
	if( ImBinWrite( ioType, fd, fp, &header.bmp_size, IM_UINT32_TYPE, 4, 1) == -1 )
	{
		ImReturnBinError( );
	}
