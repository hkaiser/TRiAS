...
static inline int to_work_silently()
{
	...
	if( !was_here)
	{
		...
		char* d_silence_env = getenv( "DLIB_SILENCE");
		if( d_silence_env)
			silently = 1;
		was_here = 1;
	}
...
{
	char* d_cfg_env = getenv( "DLIB_CONFIG");
	if( d_cfg_env)
		return( d_cfg_env);
	return( "dlib.cfg");
}
...
void D_UpdateMaskInfos()
{
	...
	{
		if( !to_work_silently())
			*get_d_stream() << "D_Lib: failed to open "
				"file " << d_cfg_fname << "\n" << flush;
		return;
	}
	if( !to_work_silently())
		*get_d_stream() << "D_Lib: reading configuration "
			"from " << d_cfg_fname << "...\n" << flush;
	char buf[1024];
	int weight = INT_MIN;
	while( fgets( buf, sizeof( buf) - 1, fp))
	{
		...
		if( D_Support_c::SetFileNameStrip( buf) ||
			...
			continue;
		if( !to_work_silently())
			*get_d_stream() << "D_Lib: configuration "
		...
		const char* mask, int level, int flag)
{
	...
	*get_d_stream() << "D_Lib: " << what << " for " << flush;
	switch( scope)
	{
		...
		int level, const char* format)
{
	...
	*get_d_stream() << "D_Lib: prefix for " << flush;
	switch( scope)
	{
		case D_Support_c::D_FILE_SCOPE:
			*get_d_stream() << "files" << flush;
		...
		unsigned int level)
{
	...
	if( !found)
	{
		if( !to_work_silently())
		{
			*get_d_stream() << "D_Lib: could not find "
				"flag for: file [" << fname << "]" <<
				flush;
			if( class_name)
				*get_d_stream() << ", class [" <<
		...
		const char* kw, const char* param)
{
	...
	{
		if( !to_work_silently())
			*get_d_stream() << "D_Lib: illegal format of "
				"the " << kw << " directive: [" <<
				param << "] (level " << "[" << level <<
				"])\n" << flush;
		return( INT_MIN);
		...
		char*& val)
{
	...
	if( new_param == 0)
	{
		// no exceptions, etc. - we are in the debug lib!
		if( !to_work_silently())
			*get_d_stream() << "D_Lib: operator new "
				"failed\n" << flush;
		return( 0);
	}
...
void D_Support_c::SetFileNameStrip( unsigned int new_lvl)
{
	...
	enter_critical_section();
	if( !to_work_silently())
		*get_d_stream() << "D_Lib: file name strip level " <<
			"changed to " << new_lvl << "\n" << flush;
...
void D_Support_c::SetRefreshInterval( const time_t interval)
{
	...
	enter_critical_section();
	if( !to_work_silently())
		*get_d_stream() << "D_Lib: refresh interval " <<
			"changed to " << interval << "\n" << flush;
...
static void set_d_stream_for( ostream& str, char* name)
{
	if( *get_d_stream() == str)	// already was
		return;
	if( !to_work_silently())
		*get_d_stream() << "D_Lib: output is switched to std" <<
			name << "\n" << flush;
	close_ofstream();
	d_stream = &str;
}
...
// #HK991020
	...
	if( !to_work_silently())
		*get_d_stream() << "D_Lib: trial to switch output "
...
static int set_d_file( const char* fn, int reporting, int new_file)
{
	...
	*fname = fn;
	...
	{
		if( !to_work_silently())
			*get_d_stream() << "D_Lib: output is switched "
				"to file " << fn_to_open << "\n" <<
				flush;
	}
	close_ofstream();
