...
bool HTTPapi::serve(WWWconnection& con)
{
	...
	do { 
		...
		if (rc < 5) {
		    con.append("HTTP/1.1 200 OK\r\n"); // connection closed due to timeout expiration
		    break;
		}
		size += rc;
	    }
	...
	if (*p != '\r' || *(p+1) != '\n') { 
	    con.append("HTTP/1.1 400 Bad Request\r\n");
	    break;
	}    
	p += 2;
	int length = INT_MAX;
	char* lenptr = strstr(buf, "Content-Length: ");
	...
	if (strncmp(buf, "GET ", 4) == 0) { 
		...
		*file_end = '\0';
		...
		if (f == NULL) { 
		    con.append("HTTP/1.1 404 Not found\r\n");
		    break;
		}
		fseek(f, 0, SEEK_END);
		...
		if (dst == NULL) { 
		    con.reset();
		    con.append("HTTP/1.1 413 Request Entity Too Large\r\n");
		    break;
		}
		if (fread(dst + pos, 1, file_size, f) != file_size) { 
		    con.reset();
		    con.append("HTTP/1.1 500 Internal server error\r\n");
		    break;
		}
		fclose(f);
	    ...
	    char* body = p;
	  ScanNextPart:
	    int n = length < buf + size - body
		? length : buf + size - body;
	    while (--n >= 0 && *p != '\r' && *p != '\n') 
	    { 
		p += 1;
	    }
			    ...
			    char* host, bool& result)
{
	...
	*end = ch;
	...
	con.append("HTTP/1.1 406 Not acceptable\r\nConnection: close\r\n\r\n");
	con.sock->write(con.reply_buf, con.reply_buf_used);
	result = true;
	*end = ch;
	return false;
...
void QueueManager::start()
{
	...
	freeList = con->next;
	mutex.unlock();
	if (!server->connect(*con) || server == NULL) { 
	    return;
	}
	mutex.lock();
	con->next = waitList;
