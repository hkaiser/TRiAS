...
// MetaData Dialogs
	...
	{ ID_METADATA_MODRCODES,
		{	CAuxUIOwner::DisableAlways, CAuxUIOwner::EnableOnWriteAble, 
			NULL, 
			CAuxUIOwner::CtrlIdIsStringID, NULL, 
			CAuxUIOwner::OnMetaCmdModRCode, 
		}, 
	},
	{ ID_METADATA_DELRCODES,
		{	CAuxUIOwner::DisableAlways, CAuxUIOwner::EnableOnWriteAble, 
			NULL, 
			CAuxUIOwner::CtrlIdIsStringID, NULL, 
			CAuxUIOwner::OnMetaCmdDelRCode, 
		}, 
	},
//
// Endekennung ////////////////////////////////////////////////////////////////
...
HRESULT CAuxUIOwner::OnMetaCmdModRCode (ITRiASBar *pIBar, UINT uiID, int nCode)
{
	if (nCode != 0 /*CN_COMMAND*/)
		return S_FALSE;			// nur WM_COMMAND's bearbeiten

	TX_ASSERT(NULL != m_pXtsn);
	return m_pXtsn -> DefineRelation() ? NOERROR : E_FAIL;
}

HRESULT CAuxUIOwner::OnMetaCmdDelRCode (ITRiASBar *pIBar, UINT uiID, int nCode)
{
	if (nCode != 0 /*CN_COMMAND*/)
		return S_FALSE;			// nur WM_COMMAND's bearbeiten

	TX_ASSERT(NULL != m_pXtsn);
	return m_pXtsn -> DeleteRelation() ? NOERROR : E_FAIL;
}
