...
#define MAX_BTREE_HEIGHT 50 // Can't have trees higher than this
...
: _comparator (cmp)
{
    ...
    if (space) {
        ...
        _nodeSpace = new CL_HeapBTreeNodeSpace (_order, cmp);
        _ownNodeSpace = TRUE;
    }
}
...
CL_VoidPtr CL_GenericBTree::Find (CL_VoidPtr item) const
{
    ...
    CL_BTreeNodeHandle curHandle = _nodeSpace->RootHandle ();
    CL_GenericBTreeNode* current;
    do {
        current = _nodeSpace->BorrowNode (curHandle);
        found = current->Search (item, pos);
    if (found || current->_isLeaf) break;
        curHandle = current->_subtree [pos+1];
        _nodeSpace->ReturnNode (current);
    } while (1);
    if (found)
...
{
    bool        ans;
    CL_GenericBTreeNode* aNode,  *root;
