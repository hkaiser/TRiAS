...
#if defined(_USE_IMAGE_HELP)
#include <Atl/ImageHelpDebug.h>
#include <Atl/ImageHelpDebug.cpp>
#else
#include <Atl/PE_Debug.cpp>			// reqiured only in one module
#if defined(_USE_BSCKIT)
#include <bsc.h>
#pragma comment(lib, "msbsc50.lib")		// link to msbsc50.dll
#endif // _USE_BSCKIT
#endif // _USE_IMAGE_HELP
	...
	public:
		...
		CRegKey cfgkey;
			...
			if (ERROR_SUCCESS == cfgkey.Open(HKEY_CURRENT_USER, TEXT("Software")) &&
				ERROR_SUCCESS == cfgkey.Open(cfgkey, REG_COMPANY_KEY) &&
				ERROR_SUCCESS == cfgkey.Open(cfgkey, REG_PRODUCT_KEY) &&
...
CStackProbeList *pProbe = NULL;
	...
	// uninteresting for the user
		...
		do {
		// der folgende Fall tritt manchmal auf (???)
			if (0 == parentEBP)
				break;

		// stack weiter hoch gehen
...
{
#if defined(_USE_BSCKIT) && !defined(_USE_IMAGE_HELP)
Bsc *pBsc = NULL;
...
#else
// Variante mit ImageHlp.dll
// Leaks auflisten
static CPEDebug PE_Debug;		// workhorse
CString strModule, strFile, strFunc;
int iLineNo = 0;

	if (!PE_Debug.Initialize())
	...
	CStackProbeList *pList = (*it).second;
		...
		BYTE *pCaller = (*lit);
			...
			strFile = g_cbUnknown;
			strFunc = g_cbUnknown;
			strModule = g_cbUnknown;

		// DebugInfo suchen (und hoffentlich) finden
			PE_Debug.DumpDebugInfo (pCaller, strModule, strFile, strFunc, &iLineNo);

			ATLTRACE("%s(%ld): ", (LPCSTR)strFile, iLineNo);

		os_path path = os_string(strModule);

			ATLTRACE("%s: ", path.filename().c_str());
			ATLTRACE("%s\r\n", demangle(strModule, strFunc));
		}
