...
CTRiASToolBar *pTBZombie = m_pToolBar;
	...
	DELETE_OBJ(m_pToolBar);				// jetzt Toolbar verabschieden
...
{
	ASSERT(NULL != m_pToolBar);
	return m_pToolBar -> IsFloating() ? S_OK : S_FALSE;
}
...
bool fOldVisible = (dwStyle & WS_VISIBLE) ? true : false;
	...
	if (fOldVisible ^ fNewVisible) {
		if (fNewVisible)
			...
			dwStyle &= ~WS_VISIBLE;
		hr = m_pToolBar -> SetBarStyleEx (NULL, dwStyle, dwStyleEx, true);
	}
...
STDMETHODIMP CTRiASToolBarObj::CommandToIndex (int uiID, int *piIndex)
{
	AFX_MANAGE_STATE(AfxGetAppModuleState())
	return CommandToIndex ((UINT)uiID, piIndex);
}

STDMETHODIMP CTRiASToolBarObj::DockBar (DOCKSTATE rgState, int iLeft, int iTop, int iRight, int iBottom)
{
	AFX_MANAGE_STATE(AfxGetAppModuleState())

CRect rc(iLeft, iTop, iRight, iBottom);	// ltrb
HRESULT hr = E_INVALIDARG;

	switch (rgState) {
	case DOCKSTATE_DOCKEDTOP:
		hr = m_pToolBar -> DockBar (AFX_IDW_DOCKBAR_TOP, rc);
		break;

	case DOCKSTATE_DOCKEDLEFT:
		hr = m_pToolBar -> DockBar (AFX_IDW_DOCKBAR_LEFT, rc);
		break;

	case DOCKSTATE_DOCKEDRIGHT:
		hr = m_pToolBar -> DockBar (AFX_IDW_DOCKBAR_RIGHT, rc);
		break;

	case DOCKSTATE_DOCKEDBOTTOM:
		hr = m_pToolBar -> DockBar (AFX_IDW_DOCKBAR_BOTTOM, rc);
		break;

	case DOCKSTATE_FLOATING:
		hr = m_pToolBar -> FloatBar (CPoint(iLeft, iTop), CBRS_ALIGN_LEFT);
		break;
...
STDMETHODIMP CTRiASToolBarObj::FloatBar (ALIGNMODE rgState, int iLeft, int iTop)
{
	AFX_MANAGE_STATE(AfxGetAppModuleState())

CPoint pt (iLeft, iTop);
HRESULT hr = E_INVALIDARG;

	switch (rgState) {
	case ALIGNMODE_HORIZONTAL:
		hr = m_pToolBar -> FloatBar (pt, CBRS_ALIGN_LEFT);
		break;

	case ALIGNMODE_VERTICAL:
		hr = m_pToolBar -> FloatBar (pt, CBRS_ALIGN_TOP);
		break;
...
STDMETHODIMP CTRiASToolBarObj::get_IsFloating (VARIANT_BOOL *pfIsFloating)
{
	AFX_MANAGE_STATE(AfxGetAppModuleState())
	if (NULL == pfIsFloating)
		return E_POINTER;

CComBool IsFloating(m_pToolBar -> IsFloating());

	*pfIsFloating = IsFloating;
	return S_OK;
}
