...
{
	DELETE_OBJ(m_pSummPage);
	DELETE_OBJ(m_pStatPage);
}
...
HPROPSHEETPAGE hPSP = NULL;
	...
	if (NULL != m_pSummPage) {		// SummaryPage einhängen
		hPSP = CreatePropertySheetPage ((PROPSHEETPAGE *)&m_pSummPage->m_psp);
	...
	if (NULL != m_pStatPage) {		// StatistikPage einhängen
		hPSP = CreatePropertySheetPage ((PROPSHEETPAGE *)&m_pStatPage->m_psp);
		if (NULL == hPSP) return ResultFromScode (E_OUTOFMEMORY);
...
// alles ok, Page(s) anlegen
	...
	CSummInfo *pSI = TxGetExtension() -> GetSummInfo();
