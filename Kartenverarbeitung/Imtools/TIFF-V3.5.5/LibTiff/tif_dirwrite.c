	...
	for (fi = 0, nfi = tif->tif_nfields; nfi > 0; nfi--, fi++) {
		...
		case FIELD_STRIPOFFSETS:
			...
			if (!TIFFWriteLongArray(tif, TIFF_LONG, tag, dir,
			    (uint32)  TIFFNumberOfStrips(tif)/*td->td_nstrips*/, td->td_stripoffset))		// CHANGED: #HK000403
				goto bad;
			break;
		case FIELD_STRIPBYTECOUNTS:
			...
			if (tag != fip->field_tag)
				continue;
			if (!TIFFWriteLongArray(tif, TIFF_LONG, tag, dir,
			    (uint32)  TIFFNumberOfStrips(tif)/*td->td_nstrips*/, td->td_stripbytecount))	// CHANGED: #HK000403
				goto bad;
			break;
