...
CRegKey regDD;		// testen, ob Drag'n Drop erlaubt sein soll
...
// anlegen und registrieren des DropTargets
	m_pDropTgt = NULL;
	if (!IsEnabledDragDrop()) return NOERROR;
...
bool ObjektInfo::FPasteFromData (IDataObject *pIDataSource)
{
...
// diese Objekte zu unserem Fenster hinzufügen
...
int maxIndex = GetCount()-1;
	...
	for (int i = maxIndex; i >= 0; i--) {		// von hinten, da gelöscht wird
		...
		long lONr = (long)m_pKurzInfo -> GetItem (i);	// Eintrag holen
			...
			TX_ASSERT(lONr > 0);
			HandleDeleteObject (lONr, false);			// aus Liste entfernen
		}
	}
...
int iNewIndex = iIndex;
	...
	// liefert S_FALSE, wenn Objekt schon drin ist
		if (S_OK == RetrieveObjInfo (lONr, iIndex, &iNewIndex))
			iIndex = iNewIndex+1;
	}
...
WDropSource DropSrc;
	...
	WEnumLONG EnumObj;
		...
		if (!CreateObjEnumerator (EnumObj.ppi(), ENUMOBJECTTYPE_ACTIVATED, 
								  (DWORD)(void *)hWnd()))
		{
