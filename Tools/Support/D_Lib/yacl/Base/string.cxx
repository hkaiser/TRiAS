...
#   pragma instantiate CL_Binding0<CL_String>
#   pragma instantiate CL_Binding0<CL_Substring>
#   pragma instantiate CL_Comparator<CL_String>
#   pragma instantiate CL_Basics<CL_String>
#endif


#if defined(__BORLANDC__) && !defined(__OS2__) && !defined(__MS_WIN32__)
...
CL_String::CL_String (const char* strg)
{
    if (strg != NULL) {
        long n = strlen (strg);
        long askSize = n + DEFAULT_START_SIZE + 1;
        if (!_Init (askSize))
            return;
        strcpy (_string, strg);
        _size = n;
...
bool CL_String::ConvertToLong (long& val) const
{
    short i = 0, sign = 1;
    long value = 0;
...
bool CL_String::Delete (long pos, long len)
{
    if (!PrepareToChange())
        return FALSE;
    if (pos < 0 || pos >= _size || len <= 0)
        return FALSE;
    long nchars = minl (len, _size - pos);
    memmove (_string + pos, _string + pos + nchars,
             _size - (pos + nchars) + 1);
    // Account for the null byte -----^--
    _size -= nchars;
    if (_size >= 8 && _size < _capacity / 4) {
        // Shrink it so that the disparity between size and capacity is not
        // too much
        long newCap   = _size + DEFAULT_START_SIZE;
        char* newData = NEW_OP char [newCap];
        if (newData) {
            strcpy (newData, _string);
            _capacity = newCap;
            delete [] _string;
            _string = newData;
        }
...
bool CL_String::_DoInsert (const char* p, long pos)
{
    ...
    if (n + _size >= _capacity) {
        ...
        char* new_data = NEW_OP char[new_cap];
        if (!new_data)
            return FALSE;
        _capacity = new_cap;
        ...
        if (pos < _size - 1) {
            memmove (_string+pos+n+1, _string+pos+1, _size-pos);
                       ...
                       bool left_justified)
{
    ...
    if (num_chars <= _size)
        return FALSE;
    char* pad_strg = NEW_OP char [_capacity = num_chars+DEFAULT_START_SIZE+1];
    if (!pad_strg)
        return FALSE;
...
bool CL_String::Replace (long pos, char c)
{
    if (!_string || pos < 0 || pos >= _size || !PrepareToChange ())
        return FALSE;
    _string [pos] = c;
    Notify ();
    return TRUE;
...
long CL_String::Replace (const char* s1, const char* s2, long pos, long n)
{
    if (!PrepareToChange())
        return 0;
    long len1, len2;
    ...
    char* new_array = NEW_OP char [new_cap];
    if (!new_array)
        return 0;
    char* in = _string+pos;
    char* out = new_array;
...
bool CL_String::Replace (long pos, long len, const char* strg)
{
    if (pos < 0 || pos > _size || len < 0 || !strg || !PrepareToChange())
        return FALSE;
    len = minl (len, _size - pos);
    long strgSize = strlen (strg);
    long new_size = _size + strgSize - len;
    if (new_size == _size) { // No change in client size
        strncpy (_string + pos, strg, len);
    }
    else if (new_size < _capacity) {
        // Enough room for modification
        memmove (_string + pos + strgSize,
                 _string + pos + len,
                 _size - pos - len);
        memcpy (_string + pos, strg, strgSize);
        _size += strgSize - len;
    }
    else {
        // Not enough room: have to reallocate space
        long new_cap = new_size + 1 + DEFAULT_START_SIZE;
        char* p = NEW_OP char [new_cap];
        if (!p)
            return FALSE; // No memory
        strncpy (p, _string, pos);
        strncpy (p + pos, strg, strgSize);
        strncpy (p + pos + strgSize, _string + pos + len, _size - pos - len);
        _size += strgSize - len;
        _capacity = new_cap;
        delete [] _string;
        _string = p;
...
{
    return Replace (s1, s2, pos, _size);
}
...
CL_StringSequence CL_String::Split (char c) const
{
    CL_StringSequence seq;
    CL_String s;

    if (!_string)
        return seq; // No memory
    for (long i = 0; i < _size; i++) {
        if (_string[i] == c) {
            seq.Add (s);
            s = "";
...
void CL_String::operator= (long value)
{
    ...
    if (_capacity < DEFAULT_START_SIZE) {
        delete [] _string;
        _string = NEW_OP char [_capacity = DEFAULT_START_SIZE];
        if (!_string) {
...
bool CL_String::AssignWithFormat (const char* fmt, ...)
{
    ...
    va_start (args, fmt);
    vsprintf ((char*) msg.AsPtr(), fmt,  args);
    if (msg[1023] != '\0') {
        CL_Error::Warning ("CL_String::AssignWithFormat: arg too large, "
...
CL_String CL_String::operator- (const char* s) const
{
    long ndx = Index (s);
    if (ndx >= 0)
...
{
    return s.Write ((long) ClassId())  &&
        s.Write ((long) (_size+1)) && s.Write ((uchar*) _string, _size+1);
}
...
bool CL_String::ReadStream (istream& stream)
{
    if (!PrepareToChange() || !stream.good())
        return FALSE;
    long currentPos = stream.tellg();
    stream.seekg (0L, ios::end);
    long streamEnd = stream.tellg ();
    stream.seekg (currentPos);
    long size = streamEnd - currentPos;
#if defined(__MS_DOS__) || defined(__MS_WINDOWS__)
    if (size >= 65000L)
        return FALSE;
#endif
    char* p = NEW_OP char [size + 1];
    if (!p)
        return FALSE;
    if (_string)
        delete _string;
...
typedef CL_Binding0<CL_Substring> Bind;
...
: CL_String (s), _client (s), _len (length)
{
    ...
    if (pos >= 0 && pos < s.Size()) {
        _size = maxl (0, minl (length, s.Size() - pos));
        if (_size)
...
bool CL_Substring::_Modified ()
{
    return _client.Replace (_pos, _len, _string);
}
...
template class CL_Binding0<CL_String>;
template class CL_Binding0<CL_Substring>;
...
#pragma define(CL_Binding0<CL_String>)
#pragma define(CL_Binding0<CL_Substring>)
