...
namespace si  {
  ...
  protected:
    ...
    static string  tag_[nBaseDims];   // default unit denotations
  ...
  public:
    ...
    //
    static
    void setScale( int dim, Val__<T> unit, string const & tag )  {
      (tag_[dim] = " ") += tag;  per_[dim] = unit.pure();
    }
    static
    bool initScale( int dim, Val__<T> unit, string const & tag )  {
      if ( per_[dim] == Rep(0) )
  ...
  //
  template< class T, OutLook O >
  string  Val__<T,O>::tag_[nBaseDims] =  {
    ""              // dimPure
  , "?m"            // dimLength
  , "?kg"           // dimMass
  , "?s"            // dimTime
  ...
  protected:
    ...
    static string  tag_;         // default denotation

    // ----- Assign initial symbol for display:
    //
    static inline
    bool  initAs( MyType const newUnit, string const & tag )  {
      if ( per_ == Rep(0) )
  ...
  public:
    ...
    static inline
    void  showAs( MyType const newUnit, string const & tag )  {
      prepareToDisplay();
    ...
    //
    friend
    inline ostream &  operator<<( ostream & os, MyType self )  {
  ...
  Dim<T,O,si7frac(q,2)> const  sqrt( Dim<T,O,si7(q)> self )  {
    return  Dim<T,O,si7opL(q,/,2)>( sqrt(self.pure()) );
  }
  ...
  struct Nth  {
    ...
    Dim<T,O,si7(N*q)> const  pow ( Dim<T,O,si7(q)> self )  {
      return  Dim<T,O,si7(N*q)>( ::pow( self.pure(), N ) );
    }
    ...
    Dim<T,O,si7frac(q,N)> const  root( Dim<T,O,si7(q)> self )  {
      return  Dim<T,O,si7opL(q,/,N)>( ::pow( self.pure(), T(1) / T(N) ) );
    }
  ...
  public:
    ...
    //
    friend inline
    ostream &  operator<<( ostream & os, MyType self )  {
      return  os << self.emit( MyType::per_, MyType::tag_, si7(self.d) ) ;
    }
  ...
  template< class T, OutLook O >
  string                           Dim<T,O,0,0,0,0,0,0,0>::tag_ = "???";
