/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imx.c,v 1.15 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER        "    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imx.c,v 1.15 1995/06/29 00:28:04 bduggan Exp $"
...
#endif
{
	 ...
	 */
	BinByteOrder( BINMBF );
	if ( ImBinRead( ioType, fd, fp, &width, IM_INT_TYPE, 4, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
	if ( ImBinRead( ioType, fd, fp, &height, IM_INT_TYPE, 4, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
	...
	{
		pBuffer = buffer;
		if ( ImBinRead( ioType, fd, fp, buffer, IM_UINT32_TYPE, 4, width )== -1)
		{
			free( (char *)buffer );
			ImVfbFree( vfb );
			ImReturnBinError( );
...
#endif
{
	...
	BinByteOrder( BINMBF );
	if ( ImBinWrite( ioType, fd, fp, &width, IM_INT_TYPE, 4, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
	if ( ImBinWrite( ioType, fd, fp, &height, IM_INT_TYPE, 4, 1 ) == -1 )
	{
		ImReturnBinError( );
	}
