...
DECLARE_INTERFACE_(ICDAORecordset, IDispatch)
	...
	STDMETHOD(GetJetIDs)		(unsigned long *pSesid, unsigned long *pDbid, unsigned long *pTableID) PURE;//For Access forms recordsets 3/98 jcm
	STDMETHOD(GetForwardOnlyFlag)(THIS_ BOOL *f);//For Access forms recordsets 6/98 jcm
	STDMETHOD(GetWorkspace)		(DAOWorkspace **) PURE;//For Access forms recordsets 11/98 jcm
	};
