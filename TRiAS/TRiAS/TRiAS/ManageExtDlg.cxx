...
//	HKCR\TRiAS.Application.4\TRiASEx\PropertySheetHandlers\Customize
//
bool ExtDirisWindow::doLoadExtensionCust () 
{ 
	return doLoadExtension ("TRiASEx.TRiASToolbarCustomizePage.1");		// Customize page oben
}
...
//bool ExtDirisWindow::doSaveOpt () 
//{ 
//	if (doLoadExtension (NULL, 1)) {		// Options page oben
//		if (g_pT) {
//			CancelStrobe (g_pT);
//			DELETE_OBJ(g_pT);
//		}
...
bool ExtDirisWindow::doLoadExtension (LPCSTR pcProgID, int iStartPage) 
{
...
CSaveOptionsDlg OptionsDlg (this);

	CustDlg.AddPage (&CustExtPage);	// eigenen Pages sind die ersten
	CustDlg.AddPage (&OptionsDlg);
...
int iPage = iStartPage;
	...
	LoadPages (CustDlg, g_cbAppKey, strSubKey.c_str(), Prj, pcProgID, &iPage);			// zusätzliche Pages aus Registry laden
...
// AnfangsSeite setzen
	if (-1 != iPage) 
		CustDlg.SetCurSel ((-1 == iStartPage) ? iPage+2 : iStartPage);	// die erste wird immer geladen (lokal)

// ggf. ProjektStartupDialog anzeigen (wenn kein Projekt geöffnet ist und der Dialog wieder 
// angezeigt werden soll)
CCurrentUser regStartDlg (g_cbRegOpenMRU);
DWORD dwData = 0;	// Defaultwert 0 (Dialog anzeigen)

	regStartDlg.GetDWORD(g_cbHideOpenWizard, dwData);	// den Wert _vorher_ speichern
...
// Dialogfenster anzeigen
	...
	if (IDCANCEL != CustDlg.Result()) {
		if ((NULL == DEX_GetActiveProjectHandle()) && (0 != dwData)) {
		DWORD dwNewData = 0;

			if (regStartDlg.GetDWORD(g_cbHideOpenWizard, dwNewData) && 0 == dwNewData)
				PostEvent (__DexCmd, DEX_SHOWSTARTUPDIALOG);	// bei nächster Gelegenheit anzeigen
		}
		return true;
	}
	return false;
...
///////////////////////////////////////////////////////////////////////////////
// Spezielle Helper für ToolbarCustomize Dialog
...
inline HWND GetParentOwner (HWND hWnd)
{
...
{
	TX_ASSERT(::IsWindow(hWndParent));
	TX_ASSERT(::IsWindow(hWndChild));
...
void NotifyFloatingWindows (Window *pWnd, DWORD dwFlags)
{
	if (dwFlags & (FS_DEACTIVATE|FS_ACTIVATE)) // update parent window activation state
		pWnd -> ForwardEvent (WM_NCACTIVATE, FALSE);

// then update the state of all floating windows owned by the parent
HWND hWnd = ::GetWindow(::GetDesktopWindow(), GW_CHILD);

	while (hWnd != NULL) {
		if (IsDescendant(pWnd -> Handle(API_WINDOW_HWND), hWnd))
...
void CManageExtDlg::Activate (Event e)
{
	CPropertySheetExt::Activate (e);

ExtDirisWindow* pFrameWnd = (ExtDirisWindow *)GetParent();

	TX_ASSERT(NULL != pFrameWnd);
	NotifyFloatingWindows (pFrameWnd, (LOWORD(e.WParam()) == WA_INACTIVE) ? FS_DEACTIVATE : FS_ACTIVATE);
}

LRESULT CManageExtDlg::Dispatch (Event e)
{
	if (WM_FLOATSTATUS == e.Msg()) 
		return OnFloatStatus (e.WParam());

	return CPropertySheetExt::Dispatch (e);
}

LRESULT CManageExtDlg::OnFloatStatus (WPARAM wParam)
{
	if ((wParam & (FS_ACTIVATE|FS_DEACTIVATE)))
		ForwardEvent (WM_NCACTIVATE, (wParam & FS_ACTIVATE) != 0);

// We want floating toolbars to remain active while we are active.
	return 1;
}
...
// Variablen initialisieren
	...
	if (fForceBttns || g_fSystemAdm) {
		try {
		WCatInformation CatInfo (CLSID_CatSimpleCache);
		GUID guidImpl[1]; 
...
CManageExtPage::~CManageExtPage (void)
{
	...
	DELETE_OBJ (m_pExtName);
	DELETE_OBJ (m_pDLLName);
	DELETE_OBJ (m_pBuffer);
	DELETE_OBJ (m_pExtList);
}
...
// 2. projektbezogene Erweiterungen (nur bei geöffnetem Projekt :-)
...
string strItem (m_pExtList -> GetItemText(hItem));
	...
	m_pExtName = StrDupF (strItem.c_str(), RC_LoadListSel);	// Bezeichner holen
...
bool iResult = DEX_ExternalUnLoadExtensionEx (LE);
	...
	HRESULT hr = UnGetRegObject (pDLL, false, fGlobal, RegExt.ppi());
		...
		//  evtl. projektbezogen entfernen
			...
			HPROJECT hPr = m_pWnd->DBO().DBDesc();
...
//
	hr = RegExt -> RegisterTRiASExtension (bstrTarget, CLEARED(&bstrName), CLEARED(&bstrID), &rgFlags);
	if (SUCCEEDED(hr)) {
		USES_CONVERSION;
...
char cbBuffer[_MAX_PATH];
	...
	CClassesRoot regFilter(KEY_READ, str.c_str());
...
CRegistry regExt (hKey, strExtReg.c_str(), false, KEY_READ);
char cbBuffer[_MAX_PATH];
