...
static const char* rcsid = "@(#)$RCSfile: dh_proc.c,v $ $Revision: 1.1.2.3 $ (DOWNHILL) $Date: 1997/06/10 01:18:09 $";
#endif
	...
	if (file_Handle != NULL)
	{
        ...
        if (exec_Env != NULL) {
           ...
           for (envp = exec_Env; *envp != NULL; envp++) {
               length += strlen(*envp) + 1;
           } 
           if ((exec_Envp = _alloca(length)) != NULL) { 
...
/* Run a program in the background ========================================= */
static HANDLE
downhill_Process_Forkexec2(char* exec_Command,
			  HANDLE file_Handle[],
			  DWORD exec_Flags,
			  char* exec_Env[])
{
...
pid_t Downhill_Process_Forkexec2(char* exec_Command,
				...
				DWORD exec_Flags)
{
	HANDLE exec_Handle = downhill_Process_Forkexec2(exec_Command,
						       file_Handle,
						       exec_Flags,
						       NULL);
       ...
       int* exec_Return,int file_Return)
{
	...
	/* Run the process */
	exec_Handle = downhill_Process_Forkexec2(exec_Command,
						dump_Handle,
						0,
						NULL);
	CloseHandle(dump_Handle[1]);
