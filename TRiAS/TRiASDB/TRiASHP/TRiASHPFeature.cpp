...
//
// @doc
// @module TRiASHPFeature.cpp | Implementation of the <c CTRiASHPFeature> class
...
STDMETHODIMP CTRiASHPFeature::InterfaceSupportsErrorInfo(REFIID riid)
{
...
STDMETHODIMP CTRiASHPFeature::get_DefaultType (BSTR *pbstrType)
{
...
STDMETHODIMP CTRiASHPFeature::SetupFeature (SETUPFEATUREMODE rgMode)
{
...
STDMETHODIMP CTRiASHPFeature::OnChanging(CHANGEDFEATURE rgWhat, VARIANT vNewValue, VARIANT_BOOL *pDisAllow)
{
...
STDMETHODIMP CTRiASHPFeature::OnChanged(CHANGEDFEATURE rgWhat, VARIANT vOldValue)
{
	switch (rgWhat) {
	case CHANGEDFEATURE_Name:
...
HRESULT CTRiASHPFeature::OnChangedName(VARIANT vOldValue)
{
	COM_TRY {
	...
	CShapeField *pField = reinterpret_cast<CShapeField *>(GetPropertyFrom (BaseFeat, g_cbFeatureCursor, 0L));
...
HRESULT CTRiASHPFeature::OnChangedDescription(VARIANT vOldValue)
{
	COM_TRY {
...
#if defined(_READWRITE)
HRESULT CTRiASHPFeature::OnChangedHandle(VARIANT vOldValue)
{
	return S_OK;
}

HRESULT CTRiASHPFeature::OnChangedType(VARIANT vOldValue)
{
...
HRESULT CTRiASHPFeature::OnChangedROMode(VARIANT vOldValue)
{
...
STDMETHODIMP CTRiASHPFeature::Eval(ITRiASObject *pIObj, VARIANT *pVal)
{
HRESULT hr = TRIASDB_E_NOSUCHFEATURE;
	...
	CComBSTR bstrVal;
		...
		hr = pField -> Eval (lObjCursor-1, &bstrVal);
		if (SUCCEEDED(hr)) {
...
STDMETHODIMP CTRiASHPFeature::EvalEx(ITRiASObject *pIObj, SAFEARRAY **ppConstraints, VARIANT *pVal)
{
	return Eval (pIObj, pVal);		// ignore constraints	
}

// Wert der Objekteigenschaft modifizieren
STDMETHODIMP CTRiASHPFeature::Update(ITRiASObject *pIObj, VARIANT Val)
{
#if defined(_READWRITE)
HRESULT hr = TRIASDB_E_NOSUCHFEATURE;
	...
	CComVariant vArg;
		...
		THROW_FAILED_HRESULT(pField -> Eval (lObjCursor-1, V_BSTR(&vArg)));
...
STDMETHODIMP CTRiASHPFeature::Clone (ITRiASFeature **ppIFeat)
{
	return E_NOTIMPL;
}

// Objekt wird logisch freigegeben
STDMETHODIMP CTRiASHPFeature::OnFinalRelease()
{
	COM_TRY {
