/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imfile.c,v 1.44 1995/08/15 08:37:32 bduggan Exp $
...
#define HEADER	"	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imfile.c,v 1.44 1995/08/15 08:37:32 bduggan Exp $"
 ...
 **
 **  DESCRIPTION
 **	imfile.c contains the top level _read and _write routines of the
 ...
 **
 **	ImFileRead	f  _read an image file from a file descriptor
 **	ImFileFRead	f  _read an image file from a file pointer
 **	ImFileWrite	f  _write an image file to a file descriptor
 **	ImFileFWrite	f  _write an image file to a file pointer
 ...
 **
 **	imFileRead	f  do the real work of _read
 **	imFileSelectMap	f  select a good _write map to use
 ...
 **	Corrected problem that caused the wrong _write-map to be
 ...
 **	Fixed bug in fopen of temp files for _read and _write.
 **	
 ...
 **	conversions prior to calling format _write code.  It also does
 ...
 **	rearranged the names of all _read/_write calls from ImReadXXX to
 **	ImXXXRead, etc.
 **	
 ...
 **	Changed format arg to _read/_write calls to a character string.  Added
 ...
 **	arguments to the _read and _write calls.  Added two new variations
 **	of the _read and _write calls:  fd and fp _read.
 **	
...
{									\
	...
	location = _lseek( (fd), 0, L_INCR );				\
	_lseek( (fd), (at), L_SET );					\
	BinRead( (fd), (buf), IM_UCHAR_TYPE, 1, (size) );			\
	_lseek( (fd), location, L_SET );					\
}
...
{									\
	...
	fseek( (fp), (at), F_SET );					\
	BinFRead( (fp), (buf), IM_UCHAR_TYPE, 1, (size) );			\
 ...
 *	stream "on the fly".
 *
 *	For reads we copy the input stream to a file, then _read it.
 *	For writes we _write to a file, then copy it to the output stream.
 *
 ...
 *	seeking) by the lower level _read and _write code.
 *
 *	The following cases must be recognized (using input as an example):
 *
 *	Command line		Meaning			Classification
 *	tool file		_read from file		file
 *	tool < file		redirect from file	file
 *	tool < /dev/tty		_read from tty		pipe
 *	tool			_read from tty		pipe
 *	cat file | tool		_read from pipe		pipe
 *
...
#define IOTYPE(fd)							\
	((((_lseek( (fd), 0, L_INCR )==-1) && (errno==ESPIPE))||_isatty( (fd) ))?\
...
#define FIOTYPE(fp)							\
	(((fseek( (fp), 0, F_INCR )==-1)||_isatty( _fileno(fp) ))?	\
 ...
 *	file is _open and how it was opened (descriptor or pointer).
 *
 *	If input isn't a pipe, tty, or _write-only file, we check the magic
...
#endif
{
	return ( imFileQFormat( IOTYPE(fd), fd, NULL, fileName ) );
}
...
#endif
{
	return ( imFileQFormat( FIOTYPE(fp), -1, fp, fileName ) );
}
...
//	int            ioOperation;	/* Read or _write?		*/
	 ...
	 *	2.  If file is _open for writing only, NO.
	 *	3.  Otherwise, YES.
	 */
	if ( ioType & IMFILEIOPIPE )
		...
		if ( ioType & IMFILEIOFD )
			doMagic = FALSE;		// sicherheitshalber nicht!
		else
			doMagic = !IsWriteOnly(fp);
#else
		if ( ioType & IMFILEIOFD )
			ioOperation = fcntl( fd, F_GETFL, &ioOperation );
		else
			ioOperation = fcntl( _fileno(fp), F_GETFL, &ioOperation);
	...
	if ( doMagic )
	{
		 ...
		 *  but not at byte 0 in the file, then _read in and test
 ...
 *
 *	imFileImageTags looks for the image _write handling tags in the
...
/*
 *  FUNCTION
 *	ImFileRead	-  _read an image file from a file descriptor
 *	ImFileFRead	-  _read an image file from a file pointer
 *	imFileRead	-  do the real work of _read
 *
 *  DESCRIPTION
 ...
 *	_read function, and that function called.
 */
...
ImFileRead( fd, format, flagsTable, tagTable )
	int         fd;			/* File descriptor to _read from	*/
	char       *format;		/* File format to use		*/
	TagTable   *flagsTable;		/* Flags			*/
	TagTable   *tagTable;		/* Tag table to append to	*/
#endif
{
	return ( imFileRead( IOTYPE(fd), fd, NULL, format,
		flagsTable, tagTable ) );
}
...
ImFileFRead( fp, format, flagsTable, tagTable )
	FILE       *fp;			/* File pointer to _read from	*/
	char       *format;		/* File format to use		*/
	TagTable   *flagsTable;		/* Flags			*/
	TagTable   *tagTable;		/* Tag table to append to	*/
#endif
...
#endif
{
	...
	unsigned char buffer[1024];	/* Temporary _write buffer	*/
	...
	else
	{
		...
		/*
		 *  If the fd/fp is a file, or it's a pipe and the handler can _read
		 *  directly to a pipe, _read it.
		 */
		...
		else
		{
			 ...
			 *  return.  If errors occur, save them until the _unlink has been done.
			 */
			strcpy( tmpName, imMakeTempFile() );
			if ( ioType & IMFILEIOFD )
			{
				if ( (tmpFd = _open( tmpName, _O_RDWR|_O_CREAT, 0666 )) == -1 )
				{
					ImErrNo = IMESYS;
					ImErrorFatal( ImQError( ), -1, ImErrNo );
				}
				while ( (n = _read( fd, buffer, sizeof( buffer ) )) > 0 )
					_write( tmpFd, buffer, n );
				_lseek( tmpFd, 0, 0 );
...
/*
 *  FUNCTION
 *	imFileSelectMap	-  select a good _write map to use
 *
 *  DESCRIPTION
 *	The format's _write map is scanned to find the best match for output
 *	of the given VFB based upon the user's requests.
 */

static ImFileFormatWriteMap *		/* Returns _write map entry	*/
#ifdef __STDC__
...
#endif
{
	...
	ImFileFormatWriteMap *pBest;	/* Best _write map entry		*/
	int         i;			/* Counters			*/
	 ...
	 *
	 *  In evaluating a given format's selection of _write map entries,
	 ...
	 *
	 *  Among several _write map entries with the same preference value
	 *  (same incomming VFB and attributes but different channel numbers
	 *  and depths), the earliest one in the _write map is chosen.
	 */
...
/*
 *  FUNCTION
 *	ImFileWrite	-  _write an image file to a file descriptor
 *	ImFileFWrite	-  _write an image file to a file pointer
 *	imFileWrite	-  do the real work of _write
 *
 *  DESCRIPTION
 ...
 *	relevant flags.  The format table is searched for the format's
 *	_write map.  The _write map is scanned to find the best match, or
 *	the match requested by incomming flags.  If the match requires
 *	depth conversion of the VFB(s), it's done.  The match's _write
...
ImFileWrite( fd, format, flagsTable, tagTable )
	...
	char       *format;		/* File format to use		*/
	TagTable   *flagsTable;		/* Flags			*/
	TagTable   *tagTable;		/* Tag table to _read from	*/
#endif
{
...
#endif
{
	...
	/*
	 *  Get the _write map entry that matches this VFB best.
	 */
	        ...
	        !(origFields & IMVFBALPHA) ) )
	{
		...
		{
			/*
			 *  Make a CLT, attach it, _write it all out,
	...
	TagTableReplace( newTable, TagEntryQNthEntry( tmpEntry ),
		TagEntryAlloc( "image vfb", IM_POINTER_TYPE, &newVfb ) );
	...
	else
	{
		...
		newEntry = TagEntryAlloc( "image clt", IM_POINTER_TYPE, &newClt);
		if ( tmpEntry != TAGENTRYNULL )
			TagTableReplace( newTable, TagEntryQNthEntry( tmpEntry),
				newEntry );
		else
	...
	for ( i = 0; i < nEntry; i++ )
	{
		...
		if ( newClt != IMCLTNULL )
			TagTableAppend( newTable,
				TagEntryAlloc( "image clt", IM_POINTER_TYPE, &newClt ));
		TagTableAppend( newTable,
			TagEntryAlloc( "image vfb", IM_POINTER_TYPE, &newVfb ) );
	}
...
#endif
{
	...
	{  
		...
		/*
		 *  If the fd/fp is a file, or it's a pipe and the handler can _write
		 *  directly to a pipe, _write it.
		 */
		...
		if ( ioType & IMFILEIOFD )
		{
			...
			status = (*pMap->map_write)( pMap, IMFILEIOFD | IMFILEIOFILE,
					tmpFd, NULL, flagsTable, tagTable );
			_lseek( tmpFd, 0, 0 );
			while ( (n = _read( tmpFd, buffer, sizeof( buffer ) )) > 0 )
				_write( fd, buffer, n );
			_close( tmpFd );
			_unlink( tmpName );
			return ( status );
...
//	int ioOperation;	   /* Read or _write?	                    */
	...
	if (doMagic)
	{
		 ...
		 */
		  ...
		  * a magic number, but not at byte 0, then _read in
...
#endif
{
	 ...
	 * 2. Compress the temp file using the _write routine from our table.
...
#endif
{
	...
	/*
	 * Call the scheme's _read routine.
