...
STDMETHODIMP CSubIndicator::Restart(THIS_ ULONG uiMinPos, ULONG ulMaxPos, DWORD dwFlags)
{
	COM_TRY {
		WProgressIndicator PI;
		PI = m_wPI;
...
STDMETHODIMP_(int) CSubIndicator::SetPosition(THIS_ ULONG uiNewPos)
{
...
// #if _MSC_VER >= 1100
STDMETHODIMP CSubIndicator :: InitNew (LONG hWnd, DWORD dwReserved, LONG *phWnd)
// #else
...
STDMETHODIMP CSubIndicator :: SetPosition (ULONG ulNewPos, INT *piPercent)
{
	if (NULL == piPercent)
		return E_POINTER;
...
STDMETHODIMP CSubIndicator :: Restart (ULONG ulMinPos, ULONG ulMaxPos, PIFLAGS fFlags)
{
	TX_ASSERT (NULL != (IProgressIndicator2 *) m_wPI);
