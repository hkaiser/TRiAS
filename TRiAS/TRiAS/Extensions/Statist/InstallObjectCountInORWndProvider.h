...
//
// @doc
// @module InstallObjectCountORWndProvider.h | Declaration of the <c CInstallObjectCountProvider> class

#if !defined(_INSTALLOBJECTCOUNTORWNDPROVIDER_H__2BB45BEB_14F8_450D_A43A_DFB3646420FF__INCLUDED_)
#define _INSTALLOBJECTCOUNTORWNDPROVIDER_H__2BB45BEB_14F8_450D_A43A_DFB3646420FF__INCLUDED_
...
inline HRESULT 
InstallObjectCountInORWndProvider (CORWndStatistics *pTarget, 
	HTREEITEM hParent, HWND hWnd, OBJECTTYPE rgTypes, int iImage, 
	int iChildren, int iState, CallbackT *)
{
	COM_TRY {
	CComObject<CObjectCountInORWnd> *pProvider = NULL;
	WValueProvider Provider;

		THROW_FAILED_HRESULT(CComObject<CObjectCountInORWnd>::CreateInstance(&pProvider));
		Provider = pProvider;

	WTriasXtensionSite Site;

		THROW_FAILED_HRESULT(g_pTE -> GetXtensionSite(Site.ppi()));
		pProvider -> SetData(hWnd, rgTypes);
	...
	CallbackT *pValueProvider = new CallbackT(pTarget, hWnd, Provider, hItem);
...
template <typename CallbackT>
inline HRESULT 
InstallObjectCountInORWndClassProvider (CORWndStatistics *pTarget, 
	HTREEITEM hParent, HWND hWnd, HOBJECTS lIdent, OBJECTTYPE rgTypes, 
    int iImage, int iChildren, int iState, CallbackT *)
{
	COM_TRY {
	CComObject<CObjectCountInORWndClass> *pProvider = NULL;
	WValueProvider Provider;

		THROW_FAILED_HRESULT(CComObject<CObjectCountInORWndClass>::CreateInstance(&pProvider));
		Provider = pProvider;

	WTriasXtensionSite Site;

		THROW_FAILED_HRESULT(g_pTE -> GetXtensionSite(Site.ppi()));
		pProvider -> SetData(hWnd, lIdent, rgTypes);
	...
	HTREEITEM hItem = pTarget->InsertItem(OLE2A(bstrName), iImage, iImage, hParent, TVI_LAST);
	CallbackT *pValueProvider = new CallbackT(pTarget, hWnd, lIdent, Provider, hItem);
...
#endif // !defined(_INSTALLOBJECTCOUNTORWNDPROVIDER_H__2BB45BEB_14F8_450D_A43A_DFB3646420FF__INCLUDED_)
