/**	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imras.c,v 1.22 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imras.c,v 1.22 1995/06/29 00:28:04 bduggan Exp $"
...
{
	{ IM_UINT_TYPE, 4, 1 },			/* ras_magic			*/
	{ IM_UINT_TYPE, 4, 1 },			/* ras_width			*/
	{ IM_UINT_TYPE, 4, 1 },			/* ras_height			*/
	{ IM_UINT_TYPE, 4, 1 },			/* ras_depth			*/
	{ IM_UINT_TYPE, 4, 1 },			/* ras_length			*/
	{ IM_UINT_TYPE, 4, 1 },			/* ras_type			*/
	{ IM_UINT_TYPE, 4, 1 },			/* ras_maptype			*/
	{ IM_UINT_TYPE, 4, 1 },			/* ras_maplength		*/
	{ 0, 0, 0 },
};
...
#endif
{
	...
	case IMRMTEQUALRGB:	/* RGB CLT.				*/
		...
		if ( ImBinRead( ioType, fd, fp, cltBuffer, IM_UCHAR_TYPE, 1,
...
#endif
{
	...
	{
		if ( ImBinRead( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, n ) == -1 )
		{
			free( (char *)runBuffer );
			ImVfbFree( vfb );
			ImReturnBinError( );
...
#endif
{
	 ...
	 *  And read them in.
	 */
	if ( (n = ImBinRead( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, n )) == -1 )
	{
		free( (char *)runBuffer );
		ImVfbFree( vfb );
		ImReturnBinError( );
...
#endif
{
	...
	if ( ImBinWrite( ioType, fd, fp, cltBuffer, IM_UCHAR_TYPE, 1, 
		imRasHeader.ras_maplength ) == -1 )
	{
		free( (char *)cltBuffer );
		ImReturnBinError( );
...
#endif
{
	...
	for ( i = 0; i < imRasHeader.ras_height; i++ )
	{
		...
		if ( ImBinWrite( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, n ) == -1)
		{
...
#endif
{
	...
	for ( i = 0; i < imRasHeader.ras_height; i++ )
	{
		...
		if ( ImBinWrite( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, nBytes ) == -1 )
		{
			free( (unsigned char *)runBuffer );
			ImReturnBinError( );
		}
