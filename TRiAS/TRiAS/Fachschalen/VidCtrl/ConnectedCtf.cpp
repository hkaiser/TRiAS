...
// Created: 15.07.2002 09:03:03 
//
// @doc
// @module ConnectedCtf.cpp | Connected CTF for maintenance of coordinate 
...
HRESULT CConnectedCtf::SetXtsnSite (ITriasXtensionSite *pSite)
{
	COM_TRY {
		if (NULL != pSite) {
			...
			THROW_FAILED_HRESULT(AtlAdvise (m_Cont, GetUnknown(), 
...
HRESULT hr = S_OK;
	...
	WUnknown Unk (this);	// uns festhalten
		...
		hr = AtlUnadvise (m_Cont, __uuidof(ITRiASNotificationSink), 
			m_dwCookie);
		if (SUCCEEDED(hr)) {
			m_dwCookie = 0L;
