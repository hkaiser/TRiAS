...
BOOL lReturn = FALSE;		// RückgabeWert initialiseren
	...
	// Script starten
		THROW_FAILED_HRESULT(m_BScript -> RunCodeEx (m_hInst, m_uiIDScript, m_strScript.c_str(), "BeginAction", 0, &bs, FALSE));
...
STDMETHODIMP CScriptPropAct::DoAction (IDataObject *pIDataObj, DWORD)
{
	...
	// Script starten
		...
		THROW_FAILED_HRESULT(m_BScript -> ExecuteMethod (m_strScript.c_str(), "DoAction", 1, &bs, FALSE));
	...
	// Enumerator in Liste speichern
		m_OutObjs.push_back (EnumOut);
		if (1 == m_OutObjs.size())
			m_ObjsIter = m_OutObjs.begin();	// auf Anfang stellen
	...
	WDataObject IDO;
		...
		*ppDataObj = IDO.detach();
		...
		// Script starten: EndAction
			THROW_FAILED_HRESULT(m_BScript -> ExecuteMethod (m_strScript.c_str(), "EndAction", 0, &bs, FALSE));
