...
CSelectObjWndDlg *CSelectObjWndDlg :: CreateInstance (
		ResID uiDlg, LPCSTR pcCaption, DWORD dwFlags)
{
	try {
	CSelectObjWndDlg *pDlg = NULL;
	
		pDlg = new CSelectObjWndDlg (uiDlg, pcCaption, dwFlags);
...
bool FillTrees (bool fSelected, bool fActivated, HWND hORWnd, CEnumObjectNumbers *pEnum)
{
// Enumerator füllen
ENUMLONGKEY ELK;
	...
	LONG lONr = DEX_GetActiveObject (hORWnd);
		...
		return GetObjectsFromObjWnd (lONr, false, pEnum);
	}
...
// Enumerator füllen
	for (CObjWnds::const_iterator it = m_ObjWnds.begin();
		 it != m_ObjWnds.end(); it++)
	{
		FillTrees (m_fSelected, m_fActivated, (*it).second, pEnum);
	}
...
// Merken, ob alle Objekte oder nur selektierte
	...
	m_fActivated = m_rbActivated.GetState() ? true : false;
	m_fAllObjects = m_rbAllObjs.GetState() ? true : false;
