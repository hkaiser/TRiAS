...
HWND hCtrl = Handle();
	...
	TX_ASSERT(IsWin40() || !m_fIsBmpBtn);	// BS_BITMAP nur unter Win40 erlaubt
}
...
// erste Bitmap laden
	...
	m_pBitmap = new Bitmap (resBitmap);
...
bool fAllLoaded = true;
	...
	if (IsValid(resBitmapSel)) {
		m_pBitmapSel = new Bitmap (resBitmapSel);
	...
	if (IsValid(resBitmapFocus)) {
		m_pBitmapFocus = new Bitmap (resBitmapFocus);
	...
	if (IsValid(resBitmapDisabled)) {
		m_pBitmapDisabled = new Bitmap (resBitmapDisabled);
...
{
	TX_ASSERT(m_pBitmap != NULL && m_pBitmap -> Handle() != NULL);     // required
...
string strButtonName (cbBuffer);
	...
	TX_ASSERT(strButtonName.length() > 0);      // must provide a title
