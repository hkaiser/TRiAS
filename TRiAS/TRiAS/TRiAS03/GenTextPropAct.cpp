...
STDMETHODIMP CGenTextPropAct::BeginAction (IProgressIndicator *pPI)
{
	...
	CComVariant vData;
		...
		if (HACTCONNECTION == m_hPr && m_Status.m_bstrDataSource.Length() > 0) 
		{
			USES_CONVERSION;
			m_hPr = DEX_GetDataSourceHandleFromShortName (OLE2A(m_Status.m_bstrDataSource));
		}

	// JG 980914
		CPropertyActionExt::BeginAction(pPI);
		THROW_FAILED_HRESULT(m_pCfgDlg -> ResetEnumLONG());
...
// DataObject generieren und mit EnumObj füllen
	*ppDataObj = NULL;
	...
	WEnumLONG EnumObj;
		...
		THROW_FAILED_HRESULT(m_pCfgDlg -> GetEnumLONG (m_hPr, EnumObj.ppi()));
...
// es wird nur eine Objektmenge geliefert
...
ULARGE_INTEGER liOwnSize;
	...
	liOwnSize.QuadPart = UINT(5 * sizeof(LONG));

	liOwnSize.QuadPart += UINT(::GetSizeMax (m_Status.m_strDirectText));
	for (CDirectTextList::iterator it = m_Status.m_lstDirectText.begin(); 
		 it != m_Status.m_lstDirectText.end(); ++it)
	{
		liOwnSize.QuadPart += UINT(::GetSizeMax (*it));
	}
	liOwnSize.QuadPart += UINT(::GetSizeMax (m_Status.m_strObjProp));
...
//  ExistantOnly=1	Text nur bei existierender Objekteigenschaft erzeugen
...
// ExistantOnly=
	vParam.Clear();
	hr = pBag -> Read (CComBSTR(g_cbExistantOnly), &vParam, pLog);
	if (S_OK == hr && SUCCEEDED(vParam.ChangeType (VT_BOOL)))
	{
		m_Status.m_fOnlyExistant = V_BOOL(&vParam) ? TRUE : FALSE;
	}
	return S_OK;
}
