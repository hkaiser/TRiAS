...
static const char* rcsid = "@(#)$RCSfile: dh_str.c,v $ $Revision: 1.1.2.1 $ (DOWNHILL) $Date: 1997/04/17 06:17:25 $";
#endif
...
int strncasecmp(char* string_One,char* string_Two,int string_Length)
{
	...
	while (string_Index < string_Length)
	{
		...
		/* Stop on '\0', if diff, already caught it above. */
		if ( string_One[string_Index] == '\0' ) {
			return 0;
		}
		string_Index++;
	}
