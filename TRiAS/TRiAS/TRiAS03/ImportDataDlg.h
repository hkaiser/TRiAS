...
DefineSmartInterface(TRiASImportWizardPropExt);		// WTRiASImportWizardPropExt
DefineSmartInterface(TRiASImportWizardPropExt2);	// WTRiASImportWizardPropExt2
...
private:
	...
	unsigned int m_fCanCoordSystems : 1;

	unsigned int m_fShowErrors : 1;

	unsigned int m_fFilesChanged : 1;
	...
	WTRiASImportWizardPropExt m_ExtPages;

	HPROJECT m_hPr;
	UINT m_uiCnt;

	CSelectCoordSysActionDlg *m_pSelectCoordSysDlg;
...
// Ergebnisse abfragen und bearbeiten
	...
	bool GetFileNames(list<os_string> &rNames, LPCSTR pcBuffer, int iLen);
...
// zusätzliche Konfigurationsseiten
	static BOOL __stdcall AddPropSheetPageProc (HPROPSHEETPAGE hPSP, LPARAM lParam);
	BOOL LoadExtPages (IImportGeoData *pIImport, REFGUID rGuid);
	HRESULT ResetExtPages ();
	int GetPageIndex (CPropertySheet *pSheet);

	void SetCoordSysDlg(CSelectCoordSysActionDlg *pDlg)
	{
		m_pSelectCoordSysDlg = pDlg;
	}
};
