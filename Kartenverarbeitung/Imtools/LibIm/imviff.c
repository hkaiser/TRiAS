/**
 **     $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imviff.c,v 1.12 1995/06/29 00:28:04 bduggan Exp $
...
#define HEADER  "    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/imviff.c,v 1.12 1995/06/29 00:28:04 bduggan Exp $"
...
#define IMVFF_MAPTYP_COMPLEX      6       /* cells are complex IM_FLOAT_TYPE */
...
{
	{ IM_UCHAR_TYPE, 1, 1	},	/* viff_identifier		*/
	{ IM_UCHAR_TYPE, 1, 1	},	/* viff_file_type		*/
	{ IM_UCHAR_TYPE, 1, 1   },	/* viff_release			*/
	{ IM_UCHAR_TYPE, 1, 1	},	/* viff_version			*/
	{ IM_UCHAR_TYPE, 1, 1	},	/* viff_machine_dep		*/
	{ IM_UCHAR_TYPE, 1, 3	},	/* viff_trash			*/
	{ IM_UCHAR_TYPE, 1, 512	},	/* viff_comment			*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_row_size		*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_col_size		*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_subrow_size		*/
	{ IM_LONG_TYPE, 4, 1	},	/* viff_startx			*/
	{ IM_LONG_TYPE, 4, 1	},	/* viff_starty			*/
	{ IM_FLOAT_TYPE, 4, 1	},	/* viff_pixsizx			*/
	{ IM_FLOAT_TYPE, 4, 1	},	/* viff_pixsizy			*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_location_type		*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_location_dim		*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_num_of_images		*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_num_data_bands		*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_data_storage_type	*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_data_encode_scheme	*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_map_scheme		*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_map_storage_type	*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_map_row_size		*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_map_col_size		*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_map_subrow_size		*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_map_enable		*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_maps_per_cycle		*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_color_space_model	*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_ispare1			*/
	{ IM_ULONG_TYPE, 4, 1	},	/* viff_ipsare2			*/
	{ IM_FLOAT_TYPE, 4, 1	},	/* viff_fpsare1			*/
	{ IM_FLOAT_TYPE, 4, 1	},	/* viff_fpsare2			*/
	{ IM_CHAR_TYPE,  1, (IMVIFF_HEADERSIZE - (21 * 4)
		 - (520*1)
		 - (2 *4) - (4*4)
		 - (4))},/*viff_reserve*/
	{ IM_CHAR_TYPE ,  4, 1	},	/* viff_maps			*/
	{ IM_FLOAT_TYPE ,  4, 1	},	/* viff_location		*/
	{ IM_CHAR_TYPE ,  4, 1	},	/* viff_imagedata		*/
	{ 0, 0, 0}
};
...
//        long         *bufferl;          /* Run buffer (type long)       */
        ...
        unsigned long c,n,m,h,w,q;       /* convenient short names       */
...
/* I am pretty sure next lines are unneeded */
	   ...
	   {
                if (ImBinRead (ioType,fd,fp,buffer, IM_UCHAR_TYPE, 1, n*1)== -1)
                {
                   free ( (char *)buffer);
                   ImReturnBinError();
                }
	...
	TagTableAppend (tagTable,TagEntryAlloc("image name",IM_POINTER_TYPE,&viffName));        }
...
//        int     expblue;                /* The five blue bits           */
		...
		if (c==IMVFF_DES_RAW)
		{
                   ...
                   for (y =0; y<h; y++)
                   {
                      if (ImBinRead (ioType,fd,fp,buffer,IM_UCHAR_TYPE,1,1*w) == -1)
                      {
                        free ( (char *)buffer);
                        ImReturnBinError();
                      }
...
#endif
{
		...
		if (c==IMVFF_DES_RAW)
		{
                   ...
                   for (y =0; y<h; y++)
                   {
                      if (ImBinRead (ioType,fd,fp,buffer, IM_UINT16_TYPE, 2,w*1) == -1)
                      {
...
//        unsigned char   leadByte;       /* Read in the lead byte        */
	...
	switch (c)
	{
		case (IMVFF_DES_RAW):
		   ...
		   {
			count=0;
                      if (ImBinRead (ioType,fd,fp,buffer,IM_UCHAR_TYPE,1,regular) == -1)
                      {
                        free ( (char *)buffer);
                        ImReturnBinError();
                      }
...
//        TagEntry    *tagEntry;          /* Tmp table entry              */
	...
	for (y=0;y<h;y++)
	{
		...
		*pBuffer++=pixel;
            ...
            if (ImBinWrite (ioType, fd, fp, buffer, IM_UCHAR_TYPE,1, regular) ==-1)
            {
                   free ( (char *)buffer);
                   ImReturnBinError( );
            }
...
//        TagEntry    *tagEntry;          /* Tmp table entry              */
	...
	x = ImVfbQWidth(vfb);
        ...
        for (j=0; j<y; j++)
        {
              ...
              if (ImBinWrite (ioType, fd, fp, buffer, IM_UINT16_TYPE,2,pBuffer - buffer) ==-1)
                {
                        free ( (sdsc_uint16 *)buffer);
                        ImReturnBinError( );
                }
...
//        ImVfbPtr     pptr;     /* Pixel pointer                */
	...
	header.viff_identifier=(char)171;
