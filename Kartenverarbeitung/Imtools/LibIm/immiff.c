/**
 **	$Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/immiff.c,v 1.6 1995/06/30 22:09:07 bduggan Exp $
...
#define HEADER	"    $Header: /sdsc/dev/vis/image/imtools/v3.0/libim/src/RCS/immiff.c,v 1.6 1995/06/30 22:09:07 bduggan Exp $"
...
#endif
{
	...
	while (moreImages)
	{
		...
		{
			if( (retVal = ImBinRead( ioType,fd,fp,&c,IM_CHAR_TYPE,sizeof(char),1))<0)
				ImReturnBinError( );
			if (retVal==0)
				break;
		 ...
		 */
		if( ImBinRead( ioType,fd,fp,&c,IM_CHAR_TYPE,sizeof(char),1)<=0)
			ImReturnBinError( );
...
#endif
{
	...
	rbufptr = rbuf;
	if ( ImBinRead (ioType, fd, fp, rbuf, IM_UCHAR_TYPE, 1, 3 * cltSize) == -1)
	{
	...
	switch (numChans)
	 {
		case 1 : /* Color indexed */
			...
			while ( ImBinRead (ioType, fd, fp, rbuf, IM_UCHAR_TYPE, 1, 2) > 0)
			{
			...
			while ( ImBinRead (ioType, fd, fp, rbuf, IM_UCHAR_TYPE, 1, 3) > 0)
			{
				uintval = rbuf[1];
				uintval |= rbuf[0] << 8;
		...
		case 3 : /* RGB */
			...
			while ( ImBinRead (ioType, fd, fp, rbuf, IM_UCHAR_TYPE, 1, 4) > 0)
			{  
		...
		case 4 : /* RGBA */
			...
			while ( ImBinRead (ioType, fd, fp, rbuf, IM_UCHAR_TYPE, 1, 5) > 0)
			{  
...
// ImVfbPtr vfbptr;       /* points into a vfb   */
 ...
 for (curImage=0;curImage<numImages;curImage++)
 {
	 ...
	 */
	 ...
	 if ( ImBinWrite( ioType, fd, fp, header, IM_CHAR_TYPE, 1, strlen(header))==-1)
	 {
		ImReturnBinError( );
	 }
...
#endif
{
 ...
 for (i=0; i<y; i++)  /* i goes through the rows */
 {
	...
	if ( ImBinWrite( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, x * numChans)==-1)
	{
		ImReturnBinError( );
	}
 }
...
#define imMiffDumpRun3(r,g,b,len)	                            \
	...
	dumpBuf[3] = len;					    \
	if ( ImBinWrite( ioType, fd, fp, dumpBuf, IM_UCHAR_TYPE, 1, 4)==-1) \
	{							    \
		ImReturnBinError( );				    \
	}		
...
#define imMiffDumpRun4(r,g,b,a,len)	                            \
	...
	if ( ImBinWrite( ioType, fd, fp, dumpBuf, IM_UCHAR_TYPE, 1, 5)==-1) \
	{							    \
		ImReturnBinError( );				    \
	}		
...
#define imMiffDumpRun1(v,len)		                            \
	dumpBuf[0] = v; 				            \
	dumpBuf[1] = len;					    \
	if ( ImBinWrite( ioType, fd, fp, dumpBuf, IM_UCHAR_TYPE, 1, 2)==-1) \
	{							    \
		ImReturnBinError( );				    \
	}		
...
#define imMiffDumpRun2(v,len)		                            \
	...
	dumpBuf[1] = (v & 0x00ff); 			            \
	dumpBuf[2] = len;					    \
	if ( ImBinWrite( ioType, fd, fp, dumpBuf, IM_UCHAR_TYPE, 1, 3)==-1) \
	{							    \
		ImReturnBinError( );				    \
	}		
 ...
 for (curImage=0;curImage<numImages;curImage++)
 {
	 ...
	 */
	 ...
	 if (pMap->map_outAttributes & IMCLTYES)
	 {
		 ...
		 if ( ImBinWrite( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, (numColors+numZeros) * 3)==-1)
		 {
			ImReturnBinError( );
		 }
		 free(runBuffer);
...
#endif
{
 ...
 for (i=0; i<y; i++)  /* i goes through the rows */
 {
	...
	if ( ImBinWrite( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, x )==-1)
	{
		ImReturnBinError( );
	}
 }
...
#endif
{
 ...
 for (i=0; i<y; i++)  /* i goes through the rows */
 {
	...
	if ( ImBinWrite( ioType, fd, fp, runBuffer, IM_UCHAR_TYPE, 1, x*2 )==-1)
	{
		ImReturnBinError( );
	}
 }
