...
$Archive: /MfcExt/Source/CoolControlManager.h $
$Workfile: CoolControlManager.h $
...
#if !defined (__CoolControlManager_h)
#define __CoolControlManager_h
...
class CCoolControlsManager {
   public:
      ...
      class CCMHeaderCtrl : public CCMControl {
         ...
         private:
            int m_nItem;        // Recently selected item
      };
   ...
   public:
      ...
      // Installs WH_CALLWNDPROC or WH_CALLWNDPROCRET hook
      // which automatically handles all dialogs in the current thread
      void InstallHook( BOOL bCallWndProcRet = TRUE, BOOL bDialogOnly = TRUE );  
      ...
      BOOL IsHooked() const;                // Returns TRUE if control manager is hooked
   ...
   protected:
      ...
      friend LRESULT CALLBACK CCM_CallWndProcRet( int nCode, WPARAM wParam, LPARAM lParam );
...
inline
BOOL CCoolControlsManager::IsHooked() const
{
   return (NULL != m_hkWndProc) ? TRUE : FALSE;
}
