...
#if _TRiAS_VER < 0x0300		// für alten Compiler
#include "Ctfx.cxx"		// 'alte' Variante laden
#else
...
// Initialisierung dieser CoordTransX
	...
	WTRiASProject Prj;
		...
		e;
		_ASSERTE(SUCCEEDED(_COM_ERROR(e)));
		return;
	}
}
...
bool CCoordTransServiceX::Scale (const double &rdX, const double &rdY, EPunkt &outPt)
{
...
bool CCoordTransServiceX::UnScale (const EPunkt &Pt, double *pX, double *pY)
{
	if (NULL == pX || NULL == pY) 
		return false;
...
double CCoordTransServiceX::CoordTransAspectRatio (ObjRechteck &OC)
{
	if (m_fIsInitialized) {
		try {
...
//			THROW_FAILED_HRESULT(m_CTF -> DistancePoint(&pt1, &pt2, outPt.ppi()));
			THROW_FAILED_HRESULT(m_CTF -> Distance (&pt1, &pt2, outPt.ppi()));	// 16.06.99
...
bool CCoordTransServiceX::Native (const double &rdX, const double &rdY, EPunkt &outPt)
{
...
bool CCoordTransServiceX::Metric (const EPunkt &Pt, double *pX, double *pY)
{
...
bool CCoordTransServiceX::GetMetricParam (const EPunkt &rPt, LPVOID *ppData)
{
...
bool CCoordTransServiceX::MetricEx (const EPunkt &rPt, double *pX, double *pY, LPVOID pData)
{
...
bool CCoordTransServiceX::NativeEx (const double &rdX, const double &rdY, EPunkt &outPt, LPVOID pData)
{
...
bool CCoordTransServiceX::TransformEx (DoublePair *pPoints, int iNum, 
...
bool CCoordTransServiceX::InvTransformEx (DoublePair *pPoints, int iNum, 
	LPVOID pData, IUnknown *pICS)
{
	if (m_fIsInitialized) {
		COM_TRY {
