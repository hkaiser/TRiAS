    ...
    ULARGE_INTEGER* pcbSize)
{
    ...
    if( SUCCEEDED(hr) )
    {
        pcbSize->LowPart = sizeof(DWORD) +  // NDR data rep.
                           ...
                           wcslen(m_pwszDisplayName) * sizeof(OLECHAR) +  // Display name
                           sizeof(OLECHAR); // NULL
        pcbSize->HighPart = 0;
    }
    ...
    IMoniker*   pmkOtherMoniker)
{
        ...
        SUCCEEDED(hr = pmkOtherMoniker->GetClassID(&clsidOther)) )
    {
        if( clsid == clsidOther )
        {
            SIP(IROTData)    prd = pmkOtherMoniker;
            if( prd )
            {
                byte	rgb[2][1024];
                ULONG	cbData[2];
...
//        pwsz++;
//    }
	*pdwHash = LHashValOfName (LOCALE_SYSTEM_DEFAULT, m_pwszDisplayName);
    return hr;
}
    ...
    ULONG*  pcbData)
{
    ...
    if( !m_pwszDisplayName && FAILED(CacheDisplayName()))	// #HK990525 ADDED: CacheDisplayName
		return E_UNEXPECTED;

    ULONG&  cbData = *pcbData;
    cbData = wcslen(m_pwszDisplayName)*sizeof(wchar_t);		// #HK000204 ADDED: sizeof(wchar_t)
    ...
    const wchar_t** ppwszDisplayParam)
{
    ...
    if( (nColon = wcslen(ProgID()))&&
        (wcsnicmp(pwszDisplayName, ProgID(), nColon) == 0) &&
        (pwszDisplayName[nColon] == ':') )
    {
        // Fall through
    }
    else if( (nColon = wcslen(VersionIndependentProgID())) &&
             (wcsnicmp(pwszDisplayName, VersionIndependentProgID(), nColon) == 0) &&
             (pwszDisplayName[nColon] == ':') )
    {
        // Fall through
    }
