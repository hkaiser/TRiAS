...
//    PNAME_INFO p;
    ...
    while ((t = McGetToken( TRUE )) != MCTOK_END_OF_FILE) {
        ...
        case MCTOK_MESSAGEID_KEYWORD:
			...
			char cbFExt[_MAX_EXT];
			char cbBuffer[_MAX_PATH];
			time_t lTime = 0;

				_splitpath (HeaderFileName, NULL, NULL, cbFName, cbFExt);
				_makepath (cbBuffer, "", "", cbFName, cbFExt);
                FirstMessageDefinition = FALSE;
                McFlushComments();

				time(&lTime);
                fprintf( HeaderFile, "// DO NOT EDIT - ALWAYS GENERATED\r\n" );
				fprintf (HeaderFile, "// File generated by MCONST at %s\r\n", ctime (&lTime));
                fprintf( HeaderFile, "// @doc MESSAGECONSTANTS\r\n" );
                fprintf( HeaderFile, "// @module %s | Definition der Message-Konstanten für die Dokumentation\r\n", cbBuffer);
                fprintf( HeaderFile, "//\r\n\r\n" );
                }
    ...
    while ((t = McGetToken( TRUE )) != MCTOK_END_OF_FILE) {
        ...
        case MCTOK_LANGUAGE_KEYWORD:
            ...
            if (MessageInfo->SymbolicName) {
                fprintf( HeaderFile, "// @constv %s | %s | 0x%lx | \r\n",
                                     NULL != MessageIdTypeName ? MessageIdTypeName : "HRESULT", 
            ...
            if (!strcmp( src, ".\r\n" )) {
                if (MessageText->Length == 0) {
                    if (MessageInfo->SymbolicName) {
                        ...
                        sprintf( dst, "No symbolic name defined for 0x%08lXL" );
                        }
